...
class DataIter {
   public:
    ...
    inline static const char* TypeName() { return "DataIter"; }
...
class MXDataIter : public DataIter {
   public:
    /*! \return typename from R side. */
    inline static const char* TypeName() { return "MXNativeDataIter"; }
    // implement the interface
    virtual void Reset();
    virtual bool Next();
    virtual int NumPad() const;
    virtual Rcpp::List Value() const;
    virtual ~MXDataIter() { MX_CALL(MXDataIterFree(handle_)); }
   ...
   private:
    ...
    // constructor
    MXDataIter() {}
    explicit MXDataIter(DataIterHandle handle) : handle_(handle) {}
    /*!
...
class ArrayDataIter : public DataIter {
   public:
    /*! \return typename from R side. */
    inline static const char* TypeName() { return "MXArrayDataIter"; }
    /*!
    ...
    ArrayDataIter(const Rcpp::NumericVector& data,
                  ...
                  const Rcpp::NumericVector& unif_rnds, int batch_size,
                  bool shuffle);
    virtual void Reset() { counter_ = 0; }
    virtual bool Next();
    virtual int NumPad() const;
    virtual Rcpp::List Value() const;
...
class DataIterCreateFunction : public ::Rcpp::CppFunction {
   public:
    ...
    virtual SEXP get_formals() { return Rcpp::List::create(Rcpp::_["alist"]); }
...
inline bool is<mxnet::R::DataIter>(SEXP x) {
    return is<mxnet::R::MXDataIter>(x) || is<mxnet::R::ArrayDataIter>(x);
}
}  // namespace Rcpp
