...
class RLogFatal {
   public:
    ...
    inline std::ostringstream& stream() { return log_stream_; }
    ~RLogFatal()
#if DMLC_USE_CXX11
        noexcept(false)
#endif
...
class MXNetMovable {
   public:
    ...
    inline static Class* XPtr(const Rcpp::RObject& obj) {
        ...
        RCHECK(!has_been_moved) << "Passed in a moved " << Class::TypeName()
                                << " as parameter."
...
struct Context {
    ...
    inline RObjectType RObject() const {
        ...
        Rcpp::List ret = Rcpp::List::create(
            Rcpp::Named("device") = dev_name, Rcpp::Named("device_id") = dev_id,
    ...
    inline static void InitRcppModule() {
        ...
        function("mx.cpu", &CPU, List::create(_["dev.id"] = 0),
                 "Create a CPU context.");
        function("mx.gpu", &GPU, List::create(_["dev.id"] = 0),
 ...
 */
inline std::string toPyString(const std::string& key,
                              const Rcpp::RObject& val) {
    std::ostringstream os;
    int len = Rf_length(val);
    if (len != 1 ||
        key.substr(std::max(5, static_cast<int>(key.size())) - 5) ==
            std::string("shape")) {
...
/*!
 * \brief Get names from list, return vector of empty strings if names do not
 * present
...
template <typename T>
bool is__module__object_fix(SEXP x) {
