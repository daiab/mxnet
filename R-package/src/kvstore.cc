...
void KVStore::Init(const std::vector<int>& keys, const Rcpp::List& weights) {
    ...
    std::vector<NDArrayHandle> handles =
        NDArray::GetHandles(weights, "weights");
    MX_CALL(MXKVStoreInit(handle_, static_cast<mx_uint>(handles.size()),
                          dmlc::BeginPtr(keys), dmlc::BeginPtr(handles)));
}

void KVStore::Push(const std::vector<int>& keys, const Rcpp::List& weight_lists,
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        MX_CALL(MXKVStorePush(handle_, static_cast<mx_uint>(vals.size()),
                              dmlc::BeginPtr(group_keys), dmlc::BeginPtr(vals),
                              priority.size() == 0 ? 0 : priority[i]));
    }
}

void KVStore::Pull(const std::vector<int>& keys, const Rcpp::List& out_lists,
                   const std::vector<int>& priority) {
    RCHECK(keys.size() == priority.size() || priority.size() == 0)
        << "The length of keys should be same as length of priority";
...
extern "C" void KVUpdaterCallback(int key, NDArrayHandle recv,
...
void KVStore::SetOptimizer(const Rcpp::List& optimizer) {
    ...
    RCHECK(names.size() == 2 && names[0] == "create.state" &&
           names[1] == "update")
        << "Invalid optimizer";
    fcreate_state_ = optimizer[0];
    fupdate_ = optimizer[1];
    optimizer_set_ = true;
    MX_CALL(MXKVStoreSetUpdater(handle_, KVUpdaterCallback, this));
}

NDArray KVStore::CreateState(int index, const NDArray& weight) const {
    RCHECK(optimizer_set_) << "Need to call set.optimizer for KVStore "
                           << type();
    // TODO(KK) review this
    // Use R Internal API here
    Rcpp::Shield<SEXP> call(
        Rf_lang3(fcreate_state_, Rcpp::wrap(index), weight.RObject()));
...
void KVStore::InitRcppModule() {
    ...
    function("mx.kv.create", &KVStore::Create,
             List::create(_["type"] = "local"), "Create a new kvstore");
}
}  // namespace R
}  // namespace mxnet
