...
class KVStore {
   public:
     ...
     *
     *  This operation will do a aggregation first on weight_lists, the push
     * things out.
     *
     ...
     */
    void Push(const std::vector<int>& keys, const Rcpp::List& weight_lists,
     ...
     */
    void Pull(const std::vector<int>& keys, const Rcpp::List& out_lists,
              const std::vector<int>& priority);
    ...
    // destructor
    ~KVStore() { MX_CALL(MXKVStoreFree(handle_)); }
