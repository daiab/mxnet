...
void MXDataIter::Reset() { MX_CALL(MXDataIterBeforeFirst(handle_)); }
...
ArrayDataIter::ArrayDataIter(const Rcpp::NumericVector& data,
    ...
    RCHECK(label_.size() == data_.size()) << "Datasize not consistent";
}
...
void ArrayDataIter::Convert(const Rcpp::NumericVector& src,
    ...
    for (size_t begin = 0; begin < order.size(); begin += batch_size) {
        ...
        MX_CALL(MXNDArraySyncCopyFromCPU(NDArray(ret)->handle,
...
DataIterCreateFunction::DataIterCreateFunction(DataIterCreator handle)
    : handle_(handle) {
    const char* name;
    ...
    MX_CALL(MXDataIterGetIterInfo(handle_, &name, &description, &num_args,
...
SEXP DataIterCreateFunction::operator()(SEXP* args) {
    ...
    MX_CALL(MXDataIterCreateIter(handle_, static_cast<mx_uint>(str_keys.size()),
                                 dmlc::BeginPtr(c_str_keys),
                                 dmlc::BeginPtr(c_str_vals), &out));
...
void DataIter::InitRcppModule() {
    ...
    class_<MXDataIter>("MXNativeDataIter").derives<DataIter>("MXDataIter");

    class_<ArrayDataIter>("MXArrayDataIter").derives<DataIter>("MXDataIter");
