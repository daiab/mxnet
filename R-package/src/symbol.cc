...
void Symbol::Compose(const Rcpp::List &kwargs, const std::string &name) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        RCHECK((keys[i].length() == 0) == positional)
            << "Input symbols need to be either positional or key=value style, "
               "not both\n";
    }
    ...
    MX_CALL(NNSymbolCompose(handle_, name.c_str(),
...
void Symbol::setAttrs(Rcpp::List attr) {
    ...
    for (size_t i = 0; i < attr.size(); i++) {
        RCHECK(TYPEOF(attr[i]) == STRSXP)
            << "Attribute values must be characters.\n";
    }
    for (size_t i = 0; i < attr.size(); i++) {
...
inline Rcpp::List BuildShapeData(mx_uint shape_size, const mx_uint *shape_ndim,
...
SEXP Symbol::InferShape(const Rcpp::List &kwargs) const {
    RCHECK(HasName(kwargs)) << "Need to pass parameters in key=value style.\n";
    ...
    MX_CALL(MXSymbolInferShape(
        ...
        &in_shape_size, &in_shape_ndim, &in_shape_data, &out_shape_size,
        &out_shape_ndim, &out_shape_data, &aux_shape_size, &aux_shape_ndim,
    ...
    if (complete != 0) {
        return Rcpp::List::create(
            ...
            Rcpp::Named("aux.shapes") =
                BuildShapeData(aux_shape_size, aux_shape_ndim, aux_shape_data,
                               ListAuxiliaryStates()));
    } else {
        return R_NilValue;
    }
...
SymbolFunction::SymbolFunction(OpHandle handle, std::string name)
    ...
    name_hint_ = name;
    std::transform(name_hint_.begin(), name_hint_.end(), name_hint_.begin(),
                   ::tolower);
    if (name[0] == '_') {
...
SEXP SymbolFunction::operator()(SEXP *args) {
    ...
    for (size_t i = 0; i < kwargs.size(); ++i) {
        ...
        if (!isSimple(kwargs[i]) && Rcpp::is<Symbol>(kwargs[i])) {
            ...
            RCHECK(keys[i].length() != 0) << "Non Symbol parameters is only "
                                             "accepted via key=value style.";
    ...
    MX_CALL(NNSymbolCreateAtomicSymbol(
        ...
        dmlc::BeginPtr(c_str_keys), dmlc::BeginPtr(c_str_vals), &shandle));
...
void Symbol::InitRcppModule() {
    ...
    class_<Symbol>("MXSymbol")
        ...
        .method("save", &Symbol::Save, "Save symbol to file")
    ...
    function("mx.symbol.Variable", &Symbol::Variable, List::create(_["name"]),
             "Create a symbolic variable with specified name.");
    function("mx.symbol.load", &Symbol::Load, List::create(_["file.name"]),
             "Load a symbol from file.");
    function("mx.symbol.load.json", &Symbol::LoadJSON,
             List::create(_["json.str"]), "Load a symbol from json string.");
    function("mx.varg.symbol.internal.Group", &Symbol::Group,
             List::create(_["slist"]),
