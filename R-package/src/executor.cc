...
void Executor::UpdateArgArray(const Rcpp::List& array, bool match_name,
                              bool skip_null) {
    UpdateArray("arg.arrays", array, arg_arrays_, match_name, skip_null);
}

void Executor::UpdateAuxArray(const Rcpp::List& array, bool match_name,
                              bool skip_null) {
    UpdateArray("aux.arrays", array, aux_arrays_, match_name, skip_null);
}

void Executor::UpdateGradArray(const Rcpp::List& array, bool match_name,
                               bool skip_null) {
    UpdateArray("grad.arrays", array, grad_arrays_, match_name, skip_null);
}

void Executor::UpdateArray(const char* array_name, const Rcpp::List& from,
                           Rcpp::List* to, bool match_name, bool skip_null) {
    if (!match_name) {
        ...
        for (size_t i = 0; i < from.size(); ++i) {
            if (to->at(i) != R_NilValue) {
                if (from[i] != R_NilValue) {
                    ...
                    RCHECK(skip_null) << "Position " << i
                                      << " expected to be not NULL";
                }
            } else {
                RCHECK(from[i] == R_NilValue) << "Position " << i
                                              << " expected to be NULL";
            }
        }
    } else {
        if (from.size() == 0) return;
        RCHECK(HasName(from)) << "match.name is set to TRUE, the input list "
                                 "must have names in all elements";
        std::vector<std::string> names = from.names();
        for (size_t i = 0; i < names.size(); ++i) {
            RCHECK(names[i].length() != 0) << "match.name is set to TRUE, the "
                                              "input list must have names in "
                                              "all elements";
            RCHECK(to->containsElementNamed(names[i].c_str()))
                << "cannot find key " << names[i] << " in the array "
                << array_name;
            ...
            if (to->at(index) != R_NilValue) {
                if (from[i] != R_NilValue) {
                    ...
                    RCHECK(skip_null) << "Element " << names[i]
                                      << " expected to be not NULL";
                }
            } else {
                RCHECK(from[i] == R_NilValue) << "Element " << names[i]
                                              << " expected to be NULL";
            }
        }
    }
...
Rcpp::List Executor::CloneArray(const Rcpp::List& src) {
    ...
    for (size_t i = 0; i < src.size(); ++i) {
        if (src[i] != R_NilValue) {
            RCHECK(Rcpp::is<NDArray>(src[i])) << "Expected exec to be "
                                              << Executor::TypeName();
...
void Executor::Forward(bool is_train, const Rcpp::List& kwargs) {
...
inline Rcpp::List* CreateArrayList(const Rcpp::List& source_array,
    ...
    try {
        ...
        for (size_t i = 0; i < source_array.size(); ++i) {
            RCHECK(Rcpp::is<NDArray>(source_array[i])) << "Expect input " << key
                                                       << " to be list of "
                                                       << NDArray::TypeName();
    ...
    try {
        ...
        for (size_t i = 0; i < grad_reqs.size(); ++i) {
            if (Rcpp::as<std::string>(grad_reqs[i]) != "null" &&
                ...
                RLOG_FATAL
                    << "grad_req must be one of 'null', 'write' or 'add'";
            }

            if (Rcpp::as<std::string>(grad_reqs[i]) != "null") {
                ret->at(i) = NDArray::Empty(
                    NDArray::FromRObject(source_array[i]).dim(), ctx);
                handles->at(i) = NDArray::FromRObject(ret->at(i))->handle;
                grad_req_type->at(i) =
                    req_map[Rcpp::as<std::string>(grad_reqs[i])];
            }
        }
    } catch (const Rcpp::exception& ex) {
        delete ret;
...
inline Rcpp::List* CreateOutList(mx_uint out_size, NDArrayHandle* out_arr,
                                 const std::vector<std::string>& names) {
...
Executor::RObjectType Executor::Bind(const Symbol::RObjectType& symbol,
    ...
    try {
        ...
        exec->arg_arrays_ =
            CreateArrayList(arg_arrays, "arg_arrays", sym->ListArguments(),
                            context, &arg_handles);
        exec->aux_arrays_ =
            CreateArrayList(aux_arrays, "aux_arrays",
                            sym->ListAuxiliaryStates(), context, &aux_handles);
        exec->grad_arrays_ =
            CreateGradList(arg_arrays, grad_reqs, sym->ListArguments(), context,
                           &grad_handles, &grad_req_type);
        Context ctx(context);
        MX_CALL(MXExecutorBind(sym->handle_, ctx.dev_type, ctx.dev_id,
                               static_cast<mx_uint>(arg_handles.size()),
                               dmlc::BeginPtr(arg_handles),
                               dmlc::BeginPtr(grad_handles),
                               dmlc::BeginPtr(grad_req_type),
                               static_cast<mx_uint>(aux_handles.size()),
                               dmlc::BeginPtr(aux_handles), &(exec->handle_)));
        mx_uint out_size;
        NDArrayHandle* out_arr;
        MX_CALL(MXExecutorOutputs(exec->handle_, &out_size, &out_arr));
        exec->out_arrays_ = CreateOutList(out_size, out_arr, sym->ListOuputs());
    } catch (const Rcpp::exception& ex) {
        delete exec;
        throw ex;
    }
...
void Executor::InitRcppModule() {
    ...
    class_<Executor>("MXExecutor")
        .method("update.aux.arrays", &Executor::UpdateAuxArray,
                "Update auxilary states array of executor, this will mutate "
                "the executor")
        .method(
            "update.arg.arrays", &Executor::UpdateArgArray,
            "Update arguments array of executor, this will mutate the executor")
        .method(
            "update.grad.arrays", &Executor::UpdateGradArray,
        ...
        .method("backward", &Executor::Backward,
                "Peform a backward operation on exec, this will set the "
                "gradients requested.")
    ...
    function("mx.symbol.bind", &Executor::Bind,
             List::create(_["symbol"], _["ctx"], _["arg.arrays"],
                          _["aux.arrays"], _["grad.reqs"]),
             "Bind the symbol on argument arrays, generate gradient array "
             "according to grad_reqs");
}
