...
inline void ConvertLayout(InputIter it, const mx_uint* ishape,
                          const size_t* ostride, int dim, size_t size,
                          mx_float* out_data) {
...
inline void ConvertLayout(const mx_float* in_data, const mx_uint* ishape,
                          const size_t* ostride, int dim, size_t size,
                          OutputIter it) {
    for (size_t i = 0; i < size; ++i, ++it) {
        size_t offset = 0;
        ...
        RCHECK(offset < size) << "offset=" << offset << ", size=" << size;
        *it = in_data[offset];
    }
}

inline std::vector<size_t> GetReverseStride(
...
template <typename InputIter>
inline void ColToRowMajor(InputIter begin, const std::vector<mx_uint>& ishape,
                          size_t size, mx_float* out_data) {
    int ndim = static_cast<int>(ishape.size());
...
inline void RowToColMajor(const mx_float* in_data,
                          const std::vector<mx_uint>& ishape, size_t size,
                          OutputIter begin) {
    int ndim = static_cast<int>(ishape.size());
    std::vector<size_t> out_stride = GetReverseStride(ishape);
    // manual unroll special constants
...
void NDArrayPacker::Push(const NDArray::RObjectType& nd) {
    ...
    if (shape_.size() == 0) {
        ...
        for (size_t i = 0; i < shape_.size() - 1; ++i) {
            RCHECK(shape_[i] == rshape[i]) << "The dimension besides last need "
                                              "to be consistent for arrays "
                                              "pushed";
        }
    ...
    MX_CALL(MXNDArraySyncCopyToCPU(arr->handle, dmlc::BeginPtr(data_) + begin,
                                   size));
}
...
Rcpp::Dimension NDArray::dim() const {
    ...
    MX_CALL(MXNDArrayGetShape(ptr_->handle, &ndim, &pshape));
...
Rcpp::NumericVector NDArray::AsNumericVector() const {
    ...
    MX_CALL(MXNDArraySyncCopyToCPU(ptr_->handle, dmlc::BeginPtr(temp),
                                   temp.size()));
...
void NDArray::Save(const Rcpp::List& data_lst, const std::string& filename) {
    ...
    MX_CALL(MXNDArraySave(filename.c_str(), num_args, dmlc::BeginPtr(handles),
                          dmlc::BeginPtr(keys)));
}
...
Rcpp::List NDArray::Load(const std::string& filename) {
    ...
    MX_CALL(MXNDArrayLoad(filename.c_str(), &out_size, &out_arr, &out_name_size,
                          &out_names));
...
NDArray::RObjectType NDArray::Empty(const Rcpp::Dimension& rshape,
...
std::vector<NDArrayHandle> NDArray::GetHandles(const Rcpp::List& array_list,
    ...
    for (size_t i = 0; i < ret.size(); ++i) {
        if (array_list[i] == R_NilValue) {
            RCHECK(allow_null) << "Expect " << list_name
                               << " to be list of non-NULL "
                               << NDArray::TypeName();
            ret[i] = nullptr;
        } else {
            RCHECK(TYPEOF(array_list[i]) == EXTPTRSXP) << "Expect " << list_name
                                                       << " to  be list of "
                                                       << NDArray::TypeName();
            Rcpp::RObject obj = array_list[i];
            Rcpp::XPtr<NDBlob> ptr(obj);
            Rcpp::RObject attr = ptr.attr("class");
            RCHECK(attr != R_NilValue &&
                   Rcpp::as<std::string>(attr) == "MXNDArray")
                << "Expect " << list_name << " to  be list of "
                << NDArray::TypeName();
            if (move_old_array) {
                RCHECK(ptr->writable)
...
void NDArray::CopyFromTo(const NDArray& from, NDArray* to) {
    ...
    RCHECK(from_handle != to_handle) << "Attempt to copy NDArray to itself";
    NDArrayHandle* p_output_vars = &to_handle;
    int num_output = 1;
    MX_CALL(MXImperativeInvoke(copy_handle, 1, &from_handle, &num_output,
                               &p_output_vars, 0, nullptr, nullptr));
}

NDArray::RObjectType NDArray::Array(const Rcpp::RObject& src,
    ...
    MX_CALL(MXNDArraySyncCopyFromCPU(NDArray(ret)->handle, dmlc::BeginPtr(temp),
                                     rdata.size()));
    return ret;
}
...
NDArrayFunction::NDArrayFunction(OpHandle handle, std::string name)
    ...
    MX_CALL(MXSymbolGetAtomicSymbolInfo(
        handle_, &real_name, &description, &num_args, &arg_names,
        &arg_type_infos, &arg_descriptions, &key_var_num_args, &ret_type));
    ...
    for (mx_uint i = 0; i < num_args; ++i) {
        ...
        // check data type.
        if (dtype.substr(0, 7) == "NDArray" || dtype.substr(0, 6) == "Symbol") {
...
SEXP NDArrayFunction::operator()(SEXP* args) {
    ...
    MXImperativeInvoke(handle_, static_cast<int>(nd_args.size()),
                       dmlc::BeginPtr(nd_args), &num_output, &p_output_vars,
                       static_cast<int>(param_keys.size()),
                       dmlc::BeginPtr(param_keys), dmlc::BeginPtr(param_vals));
...
*/
inline bool ParseNDArrayArg(SEXP sexp, NDArrayHandle* handle,
                            std::string* value) {
    switch (TYPEOF(sexp)) {
        case REALSXP: {
        ...
        case EXTPTRSXP: {
            ...
            RCHECK(attr != R_NilValue &&
                   Rcpp::as<std::string>(attr) == "MXNDArray")
                << "MXNDArray binary operations only support NDArray and "
                   "numeric values";
            RCHECK(!ptr->moved) << "Passing in an NDArray that has been moved";
            *handle = ptr->handle;
            return true;
        }
        default: {
...
inline NDArrayHandle BinaryOp(OpHandle op, NDArrayHandle* handles) {
    int num_output = 0;
    NDArrayHandle* p_output_vars = nullptr;
    MX_CALL(MXImperativeInvoke(op, 2, handles, &num_output, &p_output_vars, 0,
                               nullptr, nullptr));
    RCHECK(num_output == 1);
    return p_output_vars[0];
}

inline NDArrayHandle BinaryScalarOp(OpHandle op, NDArrayHandle handle,
    ...
    MX_CALL(MXImperativeInvoke(op, 1, &handle, &num_output, &p_output_vars, 1,
                               &skey, &svalue));
    RCHECK(num_output == 1);
    return p_output_vars[0];
}
...
NDArray::RObjectType DispatchOps(SEXP op, SEXP lhs, SEXP rhs) {
    ...
    static OpHandle rminus_scalar =
        NDArrayFunction::FindHandle("_rminus_scalar");
...
NDArray::RObjectType Slice(const NDArray::RObjectType& src, mx_uint begin,
                           mx_uint end) {
    NDArray nd(src);
    Rcpp::Dimension dim = nd.dim();
    size_t ndim = dim.size();
    RCHECK(dim[ndim - 1] >= end) << "end=" << end
                                 << ", max-dim=" << dim[ndim - 1];
...
void NDArrayFunction::InitRcppModule() {
    ...
    RCHECK(scope != nullptr) << "Init Module need to be called inside scope";
