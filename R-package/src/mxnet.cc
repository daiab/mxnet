...
namespace mxnet {
namespace R {
void SetSeed(int seed) { MX_CALL(MXRandomSeed(seed)); }

void NotifyShutdown() { MX_CALL(MXNotifyShutdown()); }

void ProfilerSetConfig(int mode, const std::string &filename) {
    MX_CALL(MXSetProfilerConfig(mode, filename.c_str()));
}

void ProfilerSetState(int state) { MX_CALL(MXSetProfilerState(state)); }
