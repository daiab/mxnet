...
class NDArray {
   public:
    ...
    inline static const char* TypeName() { return "MXNDArray"; }
     ...
     */
    NDArray(const NDArray& other) : ptr_(other.ptr_) {}
    /*!
     * \brief constructor from R SEXP
     * \param src The source SEXP
     */
    explicit NDArray(SEXP src) : ptr_(src) {}
    /*!
     * \brief Constructor
     * \param handle The handle
     */
    ...
    inline RObjectType RObject() const { return ptr_; }
    /*!
    ...
    inline NDBlob* operator->() { return ptr_.get(); }
    inline const NDBlob* operator->() const { return ptr_.get(); }
    /*!
     ...
     */
    static void Save(const Rcpp::List& data, const std::string& filename);
    /*!
...
class NDArrayFunction : public ::Rcpp::CppFunction {
   public:
    ...
    virtual int nargs() { return static_cast<int>(arg_names_.size()); }
    ...
    virtual const char* get_name() { return name_.c_str(); }

    virtual SEXP get_formals() { return formals_; }
...
inline bool is<mxnet::R::NDArray>(SEXP x) {
    ...
    return attr != R_NilValue && Rcpp::as<std::string>(attr) == "MXNDArray";
    return true;
}
}  // namespace Rcpp
