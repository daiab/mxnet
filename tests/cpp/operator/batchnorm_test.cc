...
class BatchNormValidator : public test::op::Validator<DType, AccReal> {
    ...
    static void checkBatchNorm1D(const TBlob* blob) {
        ...
        for (size_t j = 0; j < channels; ++j) {
            ...
            if (itemCount > 1) {
                // Due to eps, for a small number of entries, the error will be
                // a bit higher for one pass
            ...
            for (size_t i = 0; i < num; ++i) {
                for (size_t k = 0; k < height; ++k) {
                    for (size_t l = 0; l < width; ++l) {
                        const AccReal data =
                            test::data_at<DType>(blob, {i, j, k, l});
                        sum += data;
                        var += data * data;
                        ++itemCount;
                    }
            ...
            for (size_t i = 0; i < num; ++i) {
                for (size_t d = 0; d < depth; ++d) {
                    for (size_t k = 0; k < height; ++k) {
                        for (size_t l = 0; l < width; ++l) {
                            const AccReal data =
                                test::data_at<DType>(blob, {i, j, d, k, l});
                            sum = sum + data;
                            var = var + (data * data);
                            ++itemCount;
                        }
   ...
   public:
    ...
    static void validateForward(const BNOperatorProp& data) {
        const TBlob& outputBlob =
            data.c_.blob_output_vec_[mxnet::op::batchnorm::kData];
    ...
    static void compare(
        const test::op::OpInfo<PropType1, DType, AccReal>& info_1,
        ...
        // Output
        EXPECT_TRUE(
            compare(*info_1.data_, *info_2.data_,
...
#if MXNET_USE_CUDNN != \
    1 /* CUDNN takes a slightly different approach here on first pass */
        // Aux
        EXPECT_TRUE(compare(*info_1.data_, *info_2.data_,
   ...
   public:
    ...
    void resetForward() override {
        ...
        MSHADOW_TYPE_SWITCH(
            ...
            DTypeX, {
                const TBlob& blob =
                    this->c_.blob_input_vec_[mxnet::op::batchnorm::kGamma];
        ...
        MSHADOW_TYPE_SWITCH(
            ...
            DTypeX, {
                const TBlob& blob =
                    this->c_.blob_input_vec_[mxnet::op::batchnorm::kBeta];
                if (!hasWeightAndBias_) {
                    test::fill(blob, DTypeX(0));
                } else {  // This will cause forward pass check to fail when
                          // calculating sum == 0
                    test::fill(blob, DTypeX(1));
                    if (blob.size(0) > 0) {
        ...
        MSHADOW_TYPE_SWITCH(
            this->c_.blob_aux_states_[mxnet::op::batchnorm::kMovingMean]
                .type_flag_,
            DTypeX, {
                test::fill(
                    this->c_
                        .blob_aux_states_[mxnet::op::batchnorm::kMovingMean],
                    DTypeX(0));
            });
        MSHADOW_TYPE_SWITCH(
            this->c_.blob_aux_states_[mxnet::op::batchnorm::kMovingVar]
                .type_flag_,
            DTypeX, {
                test::fill(
                    this->c_.blob_aux_states_[mxnet::op::batchnorm::kMovingVar],
    ...
    void resetBackward() override {
        ...
        if (mxnet::op::batchnorm::kGamma < this->c_.blob_out_grad_.size()) {
            MSHADOW_TYPE_SWITCH(
                this->c_.blob_out_grad_[mxnet::op::batchnorm::kGamma]
                    .type_flag_,
                DTypeX, {
                    test::try_fill(this->c_.blob_out_grad_,
                                   mxnet::op::batchnorm::kGamma, DTypeX(0.1));
                });
        }
        ...
        if (mxnet::op::batchnorm::kBeta < this->c_.blob_out_grad_.size()) {
            MSHADOW_TYPE_SWITCH(
                ...
                DTypeX, {
                    test::try_fill(this->c_.blob_out_grad_,
                                   mxnet::op::batchnorm::kBeta, DTypeX(0.1));
                });
        }
        ...
        MSHADOW_TYPE_SWITCH(
            ...
            DTypeX, {
                test::try_fill(this->c_.blob_in_grad_,
                               mxnet::op::batchnorm::kData, DTypeX(0));
            });
        ...
        if (mxnet::op::batchnorm::kGamma < this->c_.blob_in_grad_.size()) {
            MSHADOW_TYPE_SWITCH(
                ...
                DTypeX, {
                    test::try_fill(this->c_.blob_in_grad_,
                                   mxnet::op::batchnorm::kGamma, DTypeX(0));
                });
        }
        ...
        if (mxnet::op::batchnorm::kBeta < this->c_.blob_in_grad_.size()) {
            MSHADOW_TYPE_SWITCH(
                ...
                DTypeX, {
                    test::try_fill(this->c_.blob_in_grad_,
                                   mxnet::op::batchnorm::kBeta, DTypeX(0));
                });
        }
    }

    const bool
        hasWeightAndBias_;  // This will cause forward pass validation to fail
};
...
static StreamType& PRT(
    StreamType* os, const test::op::BasicOperatorData<DType, AccReal>& obj,
    const typename test::op::BasicOperatorData<DType, AccReal>::BlobVectorType
        bvt,
    const size_t idx) {
    *os << test::op::BasicOperatorData<DType, AccReal>::bvt2String(bvt) << ": "
        << idx << ": ";
    const TBlob& blob = obj.getBlobVect(bvt)[idx];
    MSHADOW_REAL_TYPE_SWITCH(blob.type_flag_, DTypeX,
                             { test::print_blob<DTypeX>(os, blob); });
    return *os;
}
...
static StreamType& dumpF(StreamType* os,
    ...
    if (test::debugOutput) {
        *os << std::endl;
        ...
        typedef
            typename test::op::BasicOperatorData<DType, AccReal>::BlobVectorType
                BlobVectorType;
...
static test::op::OpInfo<OperatorProp, DType, AccReal>
TestBatchNormOperatorForward(
    bool isGPU, const TShape& inputShape,
    ...
    test::op::OpInfo<OperatorProp, DType, AccReal> info =
        test::op::createOpAndInfoF<OperatorProp, BNOperatorData<DType, AccReal>,
                                   DType, AccReal>(isGPU, inputShape, kwargs);
...
testForwardAndBackward(const bool isGPU1, const bool isGPU2,
                       const TShape& inputShape,
                       const test::op::kwargs_t& kwargs, const bool dumpC,
                       const size_t count = 1, const size_t cycleCount = 5) {
    test::op::OpInfo<OperatorProp1, DType, AccReal> info_1 =
        TestBatchNormOperatorForward<OperatorProp1, DType, AccReal>(
            isGPU1, inputShape, kwargs, count);

    test::op::OpInfo<OperatorProp2, DType, AccReal> info_2 =
        TestBatchNormOperatorForward<OperatorProp2, DType, AccReal>(
...
template <typename OperatorProp1, typename OperatorProp2, typename DType,
          typename AccReal>
static test::op::OpInfoPair<OperatorProp1, OperatorProp2, DType, AccReal>
testForwardAndBackward(const bool isGPU, const TShape& inputShape,
    ...
    return testForwardAndBackward<OperatorProp1, OperatorProp2, DType, AccReal>(
        isGPU, isGPU, inputShape, kwargs, dumpC);
}

template <typename DType, typename AccReal>
static test::op::OpInfoPair<op::BatchNormV1Prop, op::BatchNormProp, DType,
                            AccReal>
testBNForwardAndBackward2D(const bool isGPU, const TShape& inputShape,
                           const test::op::kwargs_t kwargs,
                           const bool dumpC = false) {
    CHECK_EQ(inputShape.ndim(), 4);  // V1 can only handle 2D
    return testForwardAndBackward<op::BatchNormV1Prop, op::BatchNormProp, DType,
                                  AccReal>(isGPU, isGPU, inputShape, kwargs,
                                           dumpC);
}
...
static const std::vector<int> v2_types = {mshadow::kFloat32, mshadow::kFloat64,
                                          mshadow::kFloat16};
...
static void timingTest(const std::string& label, const bool isGPU,
                       const bool stochastic, const test::op::kwargs_t& kwargs,
                       const int dim = 0, const size_t count = 1) {
    ...
    for (size_t i = 0; i < COUNT; ++i) {
        ...
        do {
            batchSize = stochastic ? test::rangedRand(1U, BATCH_SIZE * 2U)
                                   : TIMING_BATCH_SIZE;
            channels = stochastic ? test::rangedRand(1U, CHANNELS * 2U)
                                  : TIMING_CHANNELS;
            depth =
                stochastic ? test::rangedRand(1U, DEPTH * 2U) : TIMING_DEPTH;
        ...
        switch (D) {
            ...
            case 2:
                info = TestBatchNormOperatorForward<PropType, DType, AccReal>(
                    isGPU, {batchSize, channels, depth, height, width}, kwargs,
                    count);
                break;
            default:
...
TEST(BATCH_NORM, TestStochasticTiming_2D) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        timingTest<op::BatchNormProp, DType, AccReal>(
            "RANDOM: BatchNormProp<cpu>", false, true, blank_kwargs_nocudnn,
            GPU_TEST_DIMENSIONS);
    });
#if MXNET_USE_CUDA
    if (test::unitTestsWithCuda) {
        MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
            timingTest<op::BatchNormProp, DType, AccReal>(
                "RANDOM: BatchNormProp<gpu>", true, true, blank_kwargs_nocudnn,
                GPU_TEST_DIMENSIONS);
        });
    }
#endif
...
TEST(BATCH_NORM, TestTiming_2D) {
    ...
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        ...
        prefix = "MKL ";
#endif
        timingTest<op::BatchNormV1Prop, DType, AccReal>(
            "BatchNormV1Prop<cpu> 2D", false, false, blank_kwargs, 2,
            THISCOUNT);
        timingTest<op::BatchNormProp, DType, AccReal>(
            prefix + "BatchNormProp<cpu> 2D", false, false,
            blank_kwargs_nocudnn, 2, THISCOUNT);
#if MXNET_USE_CUDA
        if (test::unitTestsWithCuda) {
            timingTest<op::BatchNormV1Prop, DType, AccReal>(
                "BatchNormV1Prop<gpu> 2D", true, false, blank_kwargs, 2,
                THISCOUNT);
            timingTest<op::BatchNormProp, DType, AccReal>(
                "BatchNormProp<gpu> 2D", true, false, blank_kwargs_nocudnn, 2,
                THISCOUNT);
#if MXNET_USE_CUDNN == 1 && CUDNN_MAJOR >= 5
            timingTest<op::BatchNormProp, DType, AccReal>(
                "CUDNN BatchNormProp<gpu> 2D", true, false, blank_kwargs, 2,
                THISCOUNT);
#endif
        }
#endif
...
TEST(BATCH_NORM, TestBackward2D_Simple) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        ...
        test::op::OpInfoPair<op::BatchNormV1Prop, op::BatchNormProp, DType,
                             AccReal>
...
TEST(BATCH_NORM, TestIterAll) {
    TShape shapes[] = {TShape({BATCH_SIZE, CHANNELS, DH}),
                        ...
                        for (int g2 = 0; g2 < 2U; ++g2) {
                            for (int type : v2_types) {
                                MSHADOW_REAL_TYPE_SWITCH_EX(
                                    type, DType, AccReal, {
                                        test::op::OpInfoPair<op::BatchNormProp,
                                                             op::BatchNormProp,
                                                             DType, AccReal>
                                            bi = testForwardAndBackward<
                                                op::BatchNormProp,
                                                op::BatchNormProp, DType,
                                                AccReal>(
                                                g1 != 0, g2 != 0, shape, kwargs,
                                                false);  // Keep it simple
                                        if (shape.ndim() == 4 &&
                                            type == mshadow::kFloat32 && !x3) {
                                            test::op::OpInfoPair<
                                                op::BatchNormV1Prop,
                                                op::BatchNormProp, DType,
                                                AccReal>
...
template <typename DType, typename AccReal>
static void test_V1_V2_2D(const test::op::kwargs_t& kwargs,
                          const size_t count) {
    ...
    for (size_t xx = 0; xx < sizeof(tf) / sizeof(tf[0]); ++xx) {
        for (size_t yy = 0; yy < sizeof(tf) / sizeof(tf[0]); ++yy) {
            ...
            test::op::OpInfo<op::BatchNormV1Prop, DType, AccReal> info_1 =
                test::op::createOpAndInfoF<op::BatchNormV1Prop,
                                           BNOperatorData<DType, AccReal>,
                                           DType, AccReal>(gpu_V1, inputShape,
                                                           kwargs);

            test::op::OpInfo<op::BatchNormProp, DType, AccReal> info_2 =
                test::op::createOpAndInfoF<op::BatchNormProp,
                                           BNOperatorData<DType, AccReal>,
                                           DType, AccReal>(gpu_V2, inputShape,
                                                           kwargs);
            ...
            TBlob& blob1 =
                info_1.data_->c_.blob_input_vec_[op::batchnorm::kData];
            ...
            TBlob& blob2 =
                info_2.data_->c_.blob_input_vec_[op::batchnorm::kData];
            ...
            test::data_ref<DType>(
                &info_1.data_->c_.blob_input_vec_[op::batchnorm::kGamma], {1}) =
                3;
            test::data_ref<DType>(
                &info_2.data_->c_.blob_input_vec_[op::batchnorm::kGamma], {1}) =
                3;

            test::data_ref<DType>(
                &info_1.data_->c_.blob_input_vec_[op::batchnorm::kBeta], {0}) =
                3;
            test::data_ref<DType>(
                &info_2.data_->c_.blob_input_vec_[op::batchnorm::kBeta], {0}) =
                3;
...
TEST(BATCH_NORM, TestBackward2D_SimpleNFG) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        ...
        test::op::OpInfoPair<op::BatchNormV1Prop, op::BatchNormProp, DType,
                             AccReal>
            bi = testForwardAndBackward<op::BatchNormV1Prop, op::BatchNormProp,
                                        DType, AccReal>(false, inputShape,
                                                        nonfixgamma_kwargs);
    });
}
    ...
    static void test() {
...
#if MXNET_DUMP_C
        bi.info_1_.data_->dumpC(&std::cerr,
                                "Test2DBackward2DPlusLoadAndCompareLogic");
#endif
        ...
        static const std::vector<std::vector<std::vector<DType> > >
            ___Test2DBackward2DPlusLoadAndCompareLogic_data_shape_1_1_2_1___ = {
                ...
                {/* kOutGrad */
                 {0.999f, 1.999f}}};
        // Expected data state when running forward+backward starting with
        // default values
        ...
        test::op::OpInfo<op::BatchNormProp, DType, AccReal> info_checkLoad =
            test::op::createOpAndInfoF<op::BatchNormProp,
                                       BNOperatorData<DType, AccReal>, DType,
                                       AccReal>(false, inputShape,
                                                blank_kwargs);
        info_checkLoad.data_->initForward(*info_checkLoad.prop_,
                                          &info_checkLoad.in_type_);
        info_checkLoad.data_->initBackward(*info_checkLoad.prop_,
                                           &info_checkLoad.in_type_);
        info_checkLoad.data_->load(
            ___Test2DBackward2DPlusLoadAndCompareLogic_data_shape_1_1_2_1___);
...
void compare(const bool isGPU,
    ...
    test::op::OpInfo<PropType, DType, AccReal> info_checkLoad =
        test::op::createOpAndInfoF<PropType, BNOperatorData<DType, AccReal>,
                                   DType, AccReal>(
...
TEST(BATCH_NORM, TestBackward1D_Simple) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DTypeX, AccReal, {
        const TShape inputShape({1, 1, 2});
        test::op::OpInfo<op::BatchNormProp, DTypeX, AccReal> info =
            TestBatchNormOperatorForward<op::BatchNormProp, DTypeX, AccReal>(
...
TEST(BATCH_NORM, Test2DBackwardMixedV1_gpu_cpu) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        ...
        testForwardAndBackward<op::BatchNormV1Prop, op::BatchNormV1Prop, DType,
                               AccReal>(false, true, inputShape, blank_kwargs,
                                        false);
    });
}
    ...
    for (int type : v2_types) {
        MSHADOW_REAL_TYPE_SWITCH_EX(type, DType, AccReal, {
            ...
            testForwardAndBackward<op::BatchNormProp, op::BatchNormProp, DType,
                                   AccReal>(false, true, inputShape,
                                            blank_kwargs, false);
            testForwardAndBackward<op::BatchNormProp, op::BatchNormProp, DType,
                                   AccReal>(false, true, inputShape,
                                            blank_kwargs_nocudnn, false);
        });
    }
}
...
TEST(BATCH_NORM, Test2DBackwardMixedV1V2Complex_cpu_cpu_nfg) {
    MSHADOW_REAL_TYPE_SWITCH_EX(mshadow::kFloat32, DType, AccReal, {
        ...
        testForwardAndBackward<op::BatchNormV1Prop, op::BatchNormProp, DType,
                               AccReal>(false, false, inputShape,
                                        nonfixgamma_kwargs, false);
    });
}
...
TEST(BATCH_NORM, Test2DBackwardMixed_gpu_cpu_nfg) {
    for (int type : v2_types) {
        MSHADOW_REAL_TYPE_SWITCH_EX(type, DType, AccReal, {
            ...
            testForwardAndBackward<op::BatchNormProp, op::BatchNormProp, DType,
                                   AccReal>(false, true, inputShape,
                                            nonfixgamma_kwargs_nocudnn, false);
        });
    }
}
