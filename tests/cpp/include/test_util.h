...
template <typename DType>
inline void try_fill(const std::vector<TBlob> &container, size_t index,
                     const DType value) {
...
/*! \brief Return reference to data at position indexes */
template <typename DType>
inline const DType &data_at(const TBlob *blob,
...
inline StreamType &print_blob(StreamType *_os, const TBlob &blob,
    ...
    if (dim == 1) {
        ...
        TBlob changed(blob.dptr<DType>(), TShape(3), blob.dev_mask(),
                      blob.dev_id());
        changed.shape_[0] = 1;
        changed.shape_[1] = 1;
        ...
        // probably a tensor (mshadow::Tensor is deprecated)
        TBlob changed(blob.dptr<DType>(), TShape(4), blob.dev_mask(),
                      blob.dev_id());
        changed.shape_[0] = 1;
        changed.shape_[1] = 1;
        changed.shape_[2] = blob.shape_[0];
    ...
    for (size_t r = 0; r < height; ++r) {
        for (size_t thisBatch = 0; thisBatch < batchSize; ++thisBatch) {
            ...
            for (size_t thisChannel = 0; thisChannel < channels;
                 ++thisChannel) {
                for (size_t c = 0; c < width; ++c) {
                    ...
                    for (size_t dd = 0; dd < depth; ++dd) {
                        ...
                        switch (dim) {
                            case 3:
                                val = data_at<DType>(
                                    &blob, {thisBatch, thisChannel, c});
                                break;
                            case 4:
                                val = data_at<DType>(
                                    &blob, {thisBatch, thisChannel, r, c});
                                break;
                            case 5:
                                val = data_at<DType>(
                                    &blob, {thisBatch, thisChannel, dd, r, c});
                                break;
                            default:
                                LOG(FATAL) << "Unsupported blob dimension"
                                           << dim;
                                val = DType(0);
                                break;
                        }
                        os << repeatedStr("(", dd);
                        os << std::fixed << std::setw(7)
                           << std::setprecision(MPRINT_PRECISION) << std::right
                           << val << " ";
...
static inline void patternFill(TBlob *blob, GetNextData getNextData) {
    ...
    CHECK_LE(dim, 5U)
        << "Will need to handle above 3 dimensions (another for loop)";
                ...
                if (dim > 2) {
                    for (size_t d = 0; d < depth; ++d) {
                        if (dim > 3) {
                            ...
                            const size_t idx =
                                test::offset(blob->shape_, {n, ch, d});
                            CHECK_LT(idx, numberOfIndexes);
                            DType &f = blob->dptr<DType>()[idx];
                            f = getNextData();
                        }
...
struct ScopeSet {
    ...
    inline ~ScopeSet() { var_ = saveValue_; }
    T &var_;
    T saveValue_;
};
