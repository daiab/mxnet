...
inline uint64_t getTickCount() { return getMicroTickCount() / 1000; }
...
class TimedScope {
   ...
   public:
    explicit inline TimedScope(const char *msg = NULL, size_t count = 1,
    ...
    explicit inline TimedScope(const std::string &msg, size_t count = 1,
                               const bool start = true)
    ...
    inline ~TimedScope() { print(); }

    inline void start() { startTime_ = getMicroTickCount(); }
    ...
    inline float elapsedMilliseconds() const {
        const uint64_t elapsed = stopTime_ ? stopTime_ - startTime_
                                           : getMicroTickCount() - startTime_;
        return MICRO2MSF(elapsed);
    }

    inline uint64_t elapsedMicroseconds() const {
        return stopTime_ ? stopTime_ - startTime_
                         : getMicroTickCount() - startTime_;
    }
    ...
    void print() const {
        ...
        ss << "elapsed time: " << std::setprecision(4) << std::fixed
           << MICRO2MSF(diff) << " ms";
...
/*! \brief Accumulate separate timing values mapped by label/id -> total time
 * spent */
class TimingInstrument {
   public:
    ...
    void stopTiming(int id, const size_t subIterationCount = 1) {
        std::unique_lock<std::recursive_mutex> lk(mutex_);
        std::unordered_map<int, Info>::iterator i = data_.find(id);
        CHECK_NE(i == data_.end(), true)
            << "Can't stop timing on an object that we don't know about";
        if (i != data_.end()) {
            CHECK_NE(i->second.nestingCount_, 0U)
                << "While stopping timing, invalid nesting count of 0";
            if (!--i->second.nestingCount_) {
                CHECK_NE(i->second.baseTime_, 0U) << "Invalid base time";
                i->second.duration_.fetch_add(getMicroTickCount() -
                                              i->second.baseTime_);
    ...
    void print(StreamType *os, const std::string &label_,
               bool doReset = false) {
        std::unique_lock<std::recursive_mutex> lk(mutex_);
        // Sorted output
        std::map<int, Info> data(data_.begin(), data_.end());
        for (std::map<int, Info>::const_iterator i = data.begin(),
                                                 e = data.end();
             i != e; ++i) {
            const Info &info = i->second;
            const uint64_t duration = getDuration(i->first);
            *os << /*std::endl <<*/ label_ << ": " << name_ << " Timing ["
                << info.name_ << "] " << (info.nestingCount_.load() ? "*" : "")
    ...
    void reset() {
        std::unique_lock<std::recursive_mutex> lk(mutex_);
        for (std::unordered_map<int, Info>::iterator i = data_.begin(),
                                                     e = data_.end();
             i != e; ++i) {
    ...
    TimingInstrument &operator+=(const TimingInstrument &o) {
        for (std::unordered_map<int, Info>::const_iterator i = o.data_.begin(),
                                                           e = o.data_.end();
             i != e; ++i) {
   ...
   private:
    struct Info {
        ...
        std::atomic<uint64_t>
            cycleCount_;  // Note that nesting may skew averages
