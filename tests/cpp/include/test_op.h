 ...
 *
 * These classes offer a framework for developing, testing and debugging
 * operators
 ...
 * stored scalar values (which can be generated by this code from an existing
 * operator via
 * BasicOperatorData::dumpC(), as well as against each other (ie check that
 * GPU, CPU, MKL, and CUDNN operators produce the same output given the same
 * input.
 *
    ...
    struct GPUStreamScope {
        ...
        inline ~GPUStreamScope() {
            if (opContext_.run_ctx.stream) {
                mshadow::DeleteStream<gpu>(static_cast<mshadow::Stream<gpu>*>(
                    opContext_.run_ctx.stream));
   ...
   public:
    ...
    BasicOperatorData(const bool isGPU, const TShape& topShape)
        ...
        : isGPU_(isGPU)
          ...
          ,
          initializeForward_(
              0)  // unit testing may call inits in any order based
          ,
          initializeBackward_(0)  // upon its use-case (ie may not want to run
                                  // forward pass first)
          ,
    ...
    virtual bool initForward(const OperatorProperty& opProp,
                             std::vector<int>* in_type) {
        if (!initializeForward_++) {
            shape_input_vec_.resize(opProp.ListArguments().size());
            op_.reset(opProp.CreateOperatorEx(getContext(), &shape_input_vec_,
                                              in_type));
            if (op_) {
                ...
                for (size_t x = 0, n = shape_input_vec_.size(); x < n; ++x) {
                    ...
                    if (x < in_type->size()) {
                        ...
                        type = x ? mshadow::DataType<AccReal>::kFlag
                                 : mshadow::DataType<DType>::kFlag;
                    }

                    allocateBlob(&c_.blob_input_vec_, shape_input_vec_[x],
                                 false, type);
                }
    ...
    /*! \brief Initialize auxiliary and output blobs */
    virtual bool initBackward(const OperatorProperty& opProp,
                              std::vector<int>* in_type) {
        initForward(opProp, in_type);
        if (!initializeBackward_++) {
            for (size_t x = 0,
                        n = static_cast<size_t>(opProp.NumVisibleOutputs());
                 x < n; ++x) {
    ...
    void forward(const size_t count = 1) {
        ...
        MXNET_CUDA_ONLY(std::unique_ptr<GPUOpData> gpuData(
            isGPU_ ? new GPUOpData(c_, &opContext_) : nullptr));
        perf::TimingItem timeF(&timing_, Forward, "Forward", count);
        if (!isGPU_) {
            VTuneResume profile;  // VTune sample only this scope
            for (size_t x = 0; x < count; ++x) {
                op()->Forward(opContext_, c_.blob_input_vec_,
                              {kWriteTo, kWriteTo, kWriteTo},
                              c_.blob_output_vec_, c_.blob_aux_states_);
            }
        } else {
            for (size_t x = 0; x < count; ++x) {
                MXNET_CUDA_ONLY(op()->Forward(
                    opContext_, gpuData->blob_input_vec_,
                    {kWriteTo, kWriteTo, kWriteTo}, gpuData->blob_output_vec_,
    ...
    void backward(const size_t count = 1) {
        ...
        if (!isGPU_) {
            VTuneResume profile;  // VTune sample only this scope
            for (size_t x = 0; x < count; ++x) {
                op()->Backward(opContext_, c_.blob_out_grad_,
                               c_.blob_input_vec_, c_.blob_output_vec_,
                               {kWriteTo, kWriteTo, kWriteTo}, c_.blob_in_grad_,
                               c_.blob_aux_states_);
            }
        } else {
            for (size_t x = 0; x < count; ++x) {
                MXNET_CUDA_ONLY(op()->Backward(
                    opContext_, gpuData->blob_out_grad_,
                    gpuData->blob_input_vec_, gpuData->blob_output_vec_,
                    {kWriteTo, kWriteTo, kWriteTo}, gpuData->blob_in_grad_,
                    gpuData->blob_aux_states_));
            }
        }
    }
...
#undef CASE_STR
    ...
    inline const std::vector<TBlob>& getBlobVect(
        const BlobVectorType bvt) const {
        switch (bvt) {
            case kInput:
    ...
    /*! \brief Dump an operator's data set into compilable C++ data code for
     * runtime validation
     * When writing an operator test, you can generate a "known good operator
     * data state" in C++
     * code with this function, and then use load() to load the blob states into
     * this
     * class (BasicOperatorData).
     * After that, you can compare with the "actual" operator state
     * (BasicOperatorData) of
     * the operator that you are testing.
     */
    template <typename Stream>
    inline void dumpC(Stream* _os, const std::string& label) {
        Stream& os = *_os;
        os << "static const std::vector< std::vector< std::vector<float> > > "
              "___"
           << label << "_data_shape_";
        ...
        for (size_t x = 0; x < kBlobVectorTypeCount; ++x) {
            os << "  { /* " << bvt2String(BlobVectorType(x)) << " */"
               << std::endl;
    ...
    struct OpData {
        ...
        std::vector<TBlob>
            blob_out_grad_;  // Remaining err (loss) pushing back upstream
    ...
    class GPUOpData : public OpData {
       ...
       public:
        inline GPUOpData(const OpData& cpuData, OpContext* opContext)
            ...
            CHECK_EQ(cpuData_.all_blob_vects_.size(),
                     this->all_blob_vects_.size());
            for (size_t bvt = 0, nbvt = cpuData_.all_blob_vects_.size();
                 bvt < nbvt; ++bvt) {
                ...
                for (size_t i = 0, n = bv_src.size(); i < n; ++i) {
                    ...
                    TBlob* destBlob =
                        allocateBlob(&gpuBlobs_, &bvt_dest, srcBlob.shape_,
                    ...
                    mxnet::ndarray::Copy<cpu, gpu>(
                        srcBlob, destBlob, cpu_ctx, gpu_ctx,
                        allocGPUStream_.opContext_.run_ctx);
                }
            }
        ...
        inline ~GPUOpData() {
            // Copy GPU->CPU
            cudaDeviceSynchronize();
            for (size_t bvt = 0, nbvt = this->all_blob_vects_.size();
                 bvt < nbvt; ++bvt) {
                ...
                for (size_t i = 0, n = bv_src.size(); i < n; ++i) {
                    ...
                    mxnet::ndarray::Copy<gpu, cpu>(
                        srcBlob, destBlob, gpu_ctx, cpu_ctx,
                        allocGPUStream_.opContext_.run_ctx);
                }
            }
   ...
   protected:
    ...
    void allocateResources(const std::vector<ResourceRequest>& reqs) {
        ...
        for (const ResourceRequest& req : reqs) {
            if (req.type == ResourceRequest::kTempSpace) {
                ...
                opContext_.requested.push_back(
    ...
    /*! \brief Locally allocate a managed TBlob and insert into the supplied
     * vector */
    static TBlob* allocateBlob(
        std::list<std::unique_ptr<test::StandaloneBlob>>* standalone_blobs,
        std::vector<TBlob>* dest, const TShape& shape, const bool isGPU,
        const int dtype) {
        test::StandaloneBlob* blob =
            new test::StandaloneBlob(shape, isGPU, dtype);
        CHECK_NE(blob, static_cast<TBlob*>(nullptr));
        standalone_blobs->push_back(
...
/*! \brief Pair of op info objects, generally for validating ops against each
 * other */
...
class Validator {
   public:
    ...
    static inline DType ErrorBound(const TBlob* blob) {
        // Due to eps, for a small number of entries, the error will be a bit
        // higher for one pass
        if (blob->shape_.ndim() >= 3) {
            return (blob->Size() / blob->shape_[1]) <= 4 ? (ERROR_BOUND() * 15)
                                                         : ERROR_BOUND();
        } else {
    ...
    static inline DType ErrorBound(const TBlob* blob, const DTypeX v1,
                                   const DTypeX v2) {
        const DType initialErrorBound = ErrorBound(blob);
        DType kErrorBound = initialErrorBound;  // This error is based upon the
                                                // range [0.1x, 0.9x]
    ...
    static void on_failure(const size_t i, const size_t n, const Type1 v1,
                           const Type1 v2, const Type2 kErrorBound) {
    ...
    static bool compare(const TBlob& b1, const TBlob& b2) {
        if (b1.shape_ == b2.shape_) {
            MSHADOW_REAL_TYPE_SWITCH(b1.type_flag_, DTypeX, {
                ...
                for (size_t i = 0, n = b1.Size(), warningCount = 0; i < n;
                     ++i) {
                    const DTypeX v1 = *d1++;
                    const DTypeX v2 = *d2++;
    ...
    static bool compare(const TBlob& b1, const DTypeX* valuePtr) {
        ...
        for (size_t i = 0, n = b1.Size(), warningCount = 0; i < n; ++i) {
            ...
            EXPECT_NEAR(v1, v2, kErrorBound);
            if (!isNear(v1, v2, kErrorBound) && !warningCount++) {
                LOG(WARNING) << "Near test failure: " << i << ", " << n
                             << std::endl
                             << std::flush;
            }
        }
    ...
    static bool compare(const test::op::BasicOperatorData<DType, AccReal>& i1,
                        const test::op::BasicOperatorData<DType, AccReal>& i2,
                        const typename test::op::BasicOperatorData<
                            DType, AccReal>::BlobVectorType bvt,
        ...
        if (print && test::debugOutput) {
            MSHADOW_REAL_TYPE_SWITCH(b1.type_flag_, DTypeX, {
                test::print_blob<DTypeX>(&(std::cout << "Blob 1:"), b1, true,
                                         true);
                test::print_blob<DTypeX>(&(std::cout << "Blob 2:"), b2, true,
                                         true);
            });
        }
...
/*! \brief Create operator data, prop, the operator itself and init default
 * forward input */
template <typename OperatorProp, typename OperatorData, typename DType,
          typename AccReal>
static test::op::OpInfo<OperatorProp, DType, AccReal> createOpAndInfoF(
    const bool isGPU, const TShape& inputShape, const kwargs_t& kwargs) {
