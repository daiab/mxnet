...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_nativeLibInit(JNIEnv *env,
                                                                jobject obj) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayCreateEx(
    ...
    int ret = MXNDArrayCreateEx(reinterpret_cast<mx_uint *>(shapeArr),
                                static_cast<mx_uint>(ndim), devType, devId,
                                delayAlloc, dtype, &out);
...
Java_ml_dmlc_mxnet_LibInfo_mxNDArrayWaitAll(JNIEnv *env, jobject obj) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxFuncDescribe(
    ...
    int type;
    int ret = MXFuncDescribe(reinterpret_cast<FunctionHandle>(funcPtr),
                             &numUseVars, &numScalars, &numMutateVars, &type);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxFuncGetInfo(
    ...
    int ret =
        MXFuncGetInfo(reinterpret_cast<FunctionHandle>(funcPtr), &cName, &cDesc,
                      &cNumArgs, &cArgNames, &cArgTypes, &cArgDescs);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxImperativeInvoke(
    ...
    if (numParams > 0) {
        ...
        for (int i = 0; i < numParams; i++) {
            ...
            jstring jval = reinterpret_cast<jstring>(
                env->GetObjectArrayElement(paramVals, i));
    ...
    int ret = MXImperativeInvoke(
        reinterpret_cast<AtomicSymbolCreator>(funcPtr),
        static_cast<int>(numInputs), reinterpret_cast<NDArrayHandle *>(cInputs),
        &numOutputs, &cOutputs, static_cast<int>(numParams), cParamKeys,
        cParamVals);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxFuncInvokeEx(
    JNIEnv *env, jobject obj, jlong funcPtr, jlongArray useVars,
    jfloatArray scalarArgs, jlongArray mutateVars, jint numParams,
    jobjectArray paramKeys, jobjectArray paramVals) {
    jlong *cUseVars = env->GetLongArrayElements(useVars, NULL);
    jfloat *cScalarArgs = env->GetFloatArrayElements(scalarArgs, NULL);
    jlong *cMutateVars = env->GetLongArrayElements(mutateVars, NULL);
    ...
    if (numParams > 0) {
        ...
        for (int i = 0; i < numParams; i++) {
            jbyteArray jkey = reinterpret_cast<jbyteArray>(
                env->GetObjectArrayElement(paramKeys, i));
            jbyte *cParamKey = env->GetByteArrayElements(jkey, NULL);
            cParamKeys[i] = cParamKey;
            env->DeleteLocalRef(jkey);
            jbyteArray jval = reinterpret_cast<jbyteArray>(
                env->GetObjectArrayElement(paramVals, i));
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArraySaveRawBytes(
    ...
    size_t length;
    const char *pdata;
    int ret = MXNDArraySaveRawBytes(reinterpret_cast<NDArrayHandle>(ndArrayPtr),
                                    &length, &pdata);
    ...
    for (size_t i = 0; i < length; ++i) {
        jobject data =
            env->NewObject(byteClass, newByte, static_cast<jbyte>(pdata[i]));
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayGetShape(
    JNIEnv *env, jobject obj, jlong ndArrayPtr, jobject ndimRef,
    jobject dataBuf) {
    mx_uint ndim;
    const mx_uint *pdata;
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArraySyncCopyToCPU(
    ...
    int ret =
        MXNDArraySyncCopyToCPU(reinterpret_cast<NDArrayHandle>(ndArrayPtr),
                               reinterpret_cast<void *>(pdata), size);
    env->ReleaseByteArrayElements(data, pdata,
                                  0);  // copy back to java array automatically
    return ret;
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArraySlice(
    JNIEnv *env, jobject obj, jlong ndArrayPtr, jint start, jint end,
    jobject slicedHandle) {
    NDArrayHandle out;
    int ret = MXNDArraySlice(reinterpret_cast<NDArrayHandle>(ndArrayPtr), start,
                             end, &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayAt(
    ...
    NDArrayHandle out;
    int ret =
        MXNDArrayAt(reinterpret_cast<NDArrayHandle>(ndArrayPtr), idx, &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayReshape(
    ...
    int ret = MXNDArrayReshape(reinterpret_cast<NDArrayHandle>(ndArrayPtr),
                               ndim, reinterpret_cast<int *>(pdims), &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArraySyncCopyFromCPU(
    JNIEnv *env, jobject obj, jlong arrayPtr, jfloatArray sourceArr,
    jint arrSize) {
    jfloat *sourcePtr = env->GetFloatArrayElements(sourceArr, NULL);
    int ret = MXNDArraySyncCopyFromCPU(
        reinterpret_cast<NDArrayHandle>(arrayPtr),
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayGetContext(
    JNIEnv *env, jobject obj, jlong arrayPtr, jobject devTypeId,
    jobject devId) {
    int outDevType;
    int outDevId;
    int ret = MXNDArrayGetContext(reinterpret_cast<NDArrayHandle>(arrayPtr),
                                  &outDevType, &outDevId);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArraySave(
    JNIEnv *env, jobject obj, jstring jfname, jlongArray jhandles,
    jobjectArray jkeys) {
    ...
    if (jkeys != NULL) {
        ...
        for (int i = 0; i < numArgs; i++) {
            jstring jkey =
                reinterpret_cast<jstring>(env->GetObjectArrayElement(jkeys, i));
            const char *key = env->GetStringUTFChars(jkey, 0);
            keys[i] = key;
            env->DeleteLocalRef(jkey);
        }
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxNDArrayGetDType(
    ...
    int dtype;
    int ret =
        MXNDArrayGetDType(reinterpret_cast<NDArrayHandle>(jhandle), &dtype);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxInitPSEnv(
    ...
    for (int i = 0; i < paramSize; i++) {
        ...
        jstring value =
            reinterpret_cast<jstring>(env->GetObjectArrayElement(jvals, i));
...
extern "C" void KVStoreServerControllerFunc(int head, const char *body,
                                            void *handle) {
    ...
    jmethodID ctrlFunc =
        env->GetMethodID(ctrlClass, "invoke", "(ILjava/lang/String;)V");
...
extern "C" void KVStoreUpdaterCallbackFunc(int key, NDArrayHandle recv,
    ...
    jmethodID updtFunc =
        env->GetMethodID(updtClass, "update",
                         "(ILml/dmlc/mxnet/NDArray;Lml/dmlc/mxnet/NDArray;)V");

    // find java NDArray constructor
    jclass ndObjClass = env->FindClass("ml/dmlc/mxnet/NDArray");
    jmethodID ndObjConstructor =
        env->GetMethodID(ndObjClass, "<init>", "(JZ)V");
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStorePush(
    ...
    jint *keyArray = env->GetIntArrayElements(keys, NULL);
    jlong *valueArray = env->GetLongArrayElements(values, NULL);
    int ret = MXKVStorePush(
        reinterpret_cast<KVStoreHandle>(kvStorePtr), static_cast<mx_uint>(len),
        static_cast<const int *>(keyArray),
        reinterpret_cast<NDArrayHandle *>(valueArray), priority);
    env->ReleaseIntArrayElements(keys, keyArray, 0);
    env->ReleaseLongArrayElements(values, valueArray, 0);
    return ret;
}
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStorePull(
    ...
    int ret = MXKVStorePull(
        reinterpret_cast<KVStoreHandle>(kvStorePtr), static_cast<mx_uint>(len),
        static_cast<const int *>(keyArray),
        reinterpret_cast<NDArrayHandle *>(outArray), priority);
    env->ReleaseIntArrayElements(keys, keyArray, 0);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreGetType(
    ...
    const char *type;
    int ret =
        MXKVStoreGetType(reinterpret_cast<KVStoreHandle>(kvStorePtr), &type);
    jclass refStringClass = env->FindClass("ml/dmlc/mxnet/Base$RefString");
    jfieldID valueStr =
        env->GetFieldID(refStringClass, "value", "Ljava/lang/String;");
...
JNIEXPORT jint JNICALL
Java_ml_dmlc_mxnet_LibInfo_mxKVStoreSendCommmandToServers(
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreGetGroupSize(
    ...
    int size;
    int ret = MXKVStoreGetGroupSize(reinterpret_cast<KVStoreHandle>(kvStorePtr),
                                    &size);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreGetRank(
    ...
    int rank;
    int ret =
        MXKVStoreGetRank(reinterpret_cast<KVStoreHandle>(kvStorePtr), &rank);
    SetIntField(env, rankRef, rank);
    return ret;
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreGetNumDeadNode(
    JNIEnv *env, jobject obj, jlong kvStorePtr, jint nodeId,
    jobject numberRef) {
    int number;
    int ret =
        MXKVStoreGetNumDeadNode(reinterpret_cast<KVStoreHandle>(kvStorePtr),
                                static_cast<const int>(nodeId), &number);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreSetBarrierBeforeExit(
    ...
    return MXKVStoreSetBarrierBeforeExit(
        reinterpret_cast<KVStoreHandle>(kvStorePtr),
        static_cast<const int>(doBarrier));
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxKVStoreFree(JNIEnv *env,
                                                                jobject obj,
                                                                jlong ptr) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorOutputs(
    ...
    mx_uint outSize;
    NDArrayHandle *out;
    int ret = MXExecutorOutputs(reinterpret_cast<ExecutorHandle>(executorPtr),
                                &outSize, &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorFree(JNIEnv *env,
                                                                 jobject obj,
                                                                 jlong ptr) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorForward(
    ...
    return MXExecutorForward(reinterpret_cast<ExecutorHandle>(ptr),
                             static_cast<int>(isTrain));
}
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorPrint(
    ...
    int ret =
        MXExecutorPrint(reinterpret_cast<ExecutorHandle>(ptr), &retDebugStr);
    SetStringField(env, debugStr, retDebugStr);
    return ret;
}

extern "C" void ExecutorMonitorCallbackFunc(const char *name, NDArrayHandle arr,
                                            void *handle) {
    ...
    jmethodID callbackFunc =
        env->GetMethodID(callbackClass, "invoke", "(Ljava/lang/String;J)V");

    // invoke java callback method
    jstring jname = env->NewStringUTF(name);
    env->CallVoidMethod(callbackFuncObjGlb, callbackFunc, jname,
                        reinterpret_cast<jlong>(arr));
    env->DeleteLocalRef(jname);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorSetMonitorCallback(
    ...
    return MXExecutorSetMonitorCallback(
        reinterpret_cast<ExecutorHandle>(executorPtr),
        ExecutorMonitorCallbackFunc,
        reinterpret_cast<void *>(callbackFuncObjGlb));
}

JNIEXPORT jstring JNICALL
Java_ml_dmlc_mxnet_LibInfo_mxGetLastError(JNIEnv *env, jobject obj) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterCreateIter(
    ...
    int ret = MXDataIterCreateIter(reinterpret_cast<DataIterCreator>(creator),
                                   ...
                                   static_cast<const char **>(keys),
                                   static_cast<const char **>(vals), &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterGetIterInfo(
    JNIEnv *env, jobject obj, jlong creator, jobject jname, jobject jdesc,
    jobject jargNames, jobject jargTypeInfos, jobject jargDescs) {
    const char *name;
    ...
    int ret = MXDataIterGetIterInfo(reinterpret_cast<DataIterCreator>(creator),
                                    &name, &description, &numArgs, &argNames,
                                    &argTypeInfos, &argDescs);
    ...
    for (size_t i = 0; i < numArgs; i++) {
        env->CallObjectMethod(jargNames, listAppend,
                              env->NewStringUTF(argNames[i]));
        env->CallObjectMethod(jargTypeInfos, listAppend,
                              env->NewStringUTF(argTypeInfos[i]));
        env->CallObjectMethod(jargDescs, listAppend,
                              env->NewStringUTF(argDescs[i]));
    }
    return ret;
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterFree(JNIEnv *env,
                                                                 jobject obj,
                                                                 jlong handle) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterNext(JNIEnv *env,
                                                                 jobject obj,
                                                                 jlong handle,
                                                                 jobject out) {
    int cout;
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterGetLabel(
    ...
    NDArrayHandle out;
    int ret =
        MXDataIterGetLabel(reinterpret_cast<DataIterHandle>(handle), &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDataIterGetIndex(
    ...
    uint64_t *coutIndex;
    uint64_t coutSize;
    int ret = MXDataIterGetIndex(reinterpret_cast<DataIterHandle>(handle),
                                 &coutIndex, &coutSize);
    // set field
...
// Symbol functions
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolFree(JNIEnv *env,
                                                               jobject obj,
                                                               jlong ptr) {
...
JNIEXPORT jint JNICALL
Java_ml_dmlc_mxnet_LibInfo_mxSymbolListAtomicSymbolCreators(
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolCreateAtomicSymbol(
    ...
    for (int i = 0; i < paramSize; i++) {
        jstring key =
            reinterpret_cast<jstring>(env->GetObjectArrayElement(paramKeys, i));
        const char *rawKey = env->GetStringUTFChars(key, 0);
        keys[i] = rawKey;
        env->DeleteLocalRef(key);

        jstring value =
            reinterpret_cast<jstring>(env->GetObjectArrayElement(paramVals, i));
    ...
    int ret = MXSymbolCreateAtomicSymbol(
        reinterpret_cast<AtomicSymbolCreator>(symbolPtr),
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolSetAttr(
    ...
    int ret = MXSymbolSetAttr(reinterpret_cast<SymbolHandle>(symbolPtr), ckey,
                              cvalue);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolCompose(
    ...
    int ret = MXSymbolCompose(reinterpret_cast<SymbolHandle>(symbolPtr), name,
                              static_cast<mx_uint>(argSize), keys,
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolGetAttr(
    JNIEnv *env, jobject obj, jlong symbolPtr, jstring jkey, jobject retRef,
    jobject successRef) {
    const char *out;
    int success;
    const char *key = env->GetStringUTFChars(jkey, 0);
    int ret = MXSymbolGetAttr(reinterpret_cast<SymbolHandle>(symbolPtr), key,
                              &out, &success);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolListArguments(
    ...
    int ret = MXSymbolListArguments(reinterpret_cast<SymbolHandle>(symbolPtr),
                                    &outSize, &outStrArray);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolListOutputs(
    ...
    mx_uint outSize;
    const char **outStrArray;
    int ret = MXSymbolListOutputs(reinterpret_cast<SymbolHandle>(symbolPtr),
                                  &outSize, &outStrArray);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolCopy(
    ...
    int ret =
        MXSymbolCopy(reinterpret_cast<SymbolHandle>(symbolPtr), &clonedSymbol);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolCreateGroup(
    ...
    int ret = MXSymbolCreateGroup(
        numSymbols, reinterpret_cast<SymbolHandle *>(symbols), &handle);
    env->ReleaseLongArrayElements(jsymbols, symbols, 0);
    SetLongField(env, out, reinterpret_cast<jlong>(handle));
    return ret;
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolPrint(JNIEnv *env,
                                                                jobject obj,
                                                                jlong symbolPtr,
                                                                jobject out) {
    const char *outStr;
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolGetInternals(
    ...
    SymbolHandle out;
    int ret =
        MXSymbolGetInternals(reinterpret_cast<SymbolHandle>(symbolPtr), &out);
    SetLongField(env, jout, reinterpret_cast<jlong>(out));
    return ret;
}

JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolInferType(
    JNIEnv *env, jobject obj, jlong symbolPtr, jobjectArray jkeys,
    jintArray jvals, jobject jargTypeData, jobject joutTypeData,
    jobject jauxTypeData, jobject jcomplete) {
    ...
    int ret = MXSymbolInferType(reinterpret_cast<SymbolHandle>(symbolPtr),
                                ...
                                static_cast<const int *>(vals), &inTypeSize,
                                &inTypeData, &outTypeSize, &outTypeData,
                                &auxTypeSize, &auxTypeData, &complete);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolSaveToJSON(
    ...
    const char *out;
    int ret =
        MXSymbolSaveToJSON(reinterpret_cast<SymbolHandle>(symbolPtr), &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolSaveToFile(
    ...
    const char *fname = env->GetStringUTFChars(jfname, 0);
    int ret =
        MXSymbolSaveToFile(reinterpret_cast<SymbolHandle>(symbolPtr), fname);
    env->ReleaseStringUTFChars(jfname, fname);
    return ret;
}
...
int FillSymbolInferShape(JNIEnv *env, jmethodID listAppend, jobject joutData,
                         mx_uint shapeSize, const mx_uint *shapeNdim,
                         const mx_uint **shapeData) {
    for (size_t i = 0; i < shapeSize; ++i) {
        ...
        env->SetIntArrayRegion(jshape, 0, shapeNdim[i],
                               reinterpret_cast<const jint *>(shapeData[i]));
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxSymbolInferShape(
    JNIEnv *env, jobject obj, jlong symbolPtr, jint jnumArgs,
    ...
    int ret = MXSymbolInferShape(
        reinterpret_cast<SymbolHandle>(symbolPtr),
        static_cast<mx_uint>(jnumArgs), keys,
        reinterpret_cast<const mx_uint *>(argIndPtr),
        reinterpret_cast<const mx_uint *>(argShapeData), &inShapeSize,
        &inShapeNdim, &inShapeData, &outShapeSize, &outShapeNdim, &outShapeData,
        &auxShapeSize, &auxShapeNdim, &auxShapeData, &complete);
    ...
    if (FillSymbolInferShape(env, listAppend, joutShapeData, outShapeSize,
                             outShapeNdim, outShapeData)) {
        // TODO(Yizhi): out of memory error thrown, return a specific error code
        // ?
        return -1;
    }
    if (FillSymbolInferShape(env, listAppend, jauxShapeData, auxShapeSize,
                             auxShapeNdim, auxShapeData)) {
        // TODO(Yizhi): out of memory error thrown, return a specific error code
        // ?
        return -1;
    }
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorBindX(
    JNIEnv *env, jobject obj, jlong symbolPtr, jint deviceTypeId, jint deviceID,
    jint numCtx, jobjectArray jctxMapKeys, jintArray jctxMapDevTypes,
    jintArray jctxMapDevIDs, jint numArgs, jlongArray jargsHandle,
    jlongArray jargsGradHandle, jintArray jreqsArray, jlongArray jauxArgsHandle,
    jobject jexecOut) {
    ExecutorHandle out;
    ...
    for (int i = 0; i < numCtx; i++) {
        jstring jkey = reinterpret_cast<jstring>(
            env->GetObjectArrayElement(jctxMapKeys, i));
        const char *key = env->GetStringUTFChars(jkey, 0);
        mapKeys[i] = key;
        env->DeleteLocalRef(jkey);
    }
    ...
    int ret =
        MXExecutorBindX(reinterpret_cast<SymbolHandle>(symbolPtr), deviceTypeId,
                        deviceID, static_cast<mx_uint>(numCtx), mapKeys,
                        mapDevTypes, mapDevIDs, static_cast<mx_uint>(numArgs),
                        ...
                        reinterpret_cast<NDArrayHandle *>(auxStates), &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxExecutorBindEX(
    ...
    jintArray jctxMapDevIDs, jint numArgs, jlongArray jargsHandle,
    jlongArray jargsGradHandle, jintArray jreqsArray, jlongArray jauxArgsHandle,
    jlong jsharedExec, jobject jexecOut) {
    ExecutorHandle out;
    int auxStatesLen = env->GetArrayLength(jauxArgsHandle);
    ExecutorHandle sharedExec = nullptr;
    if ((int32_t)jsharedExec != 0)
        sharedExec = reinterpret_cast<ExecutorHandle>(jsharedExec);
    ...
    jint *mapDevTypes = env->GetIntArrayElements(jctxMapDevTypes, NULL);
    jint *mapDevIDs = env->GetIntArrayElements(jctxMapDevIDs, NULL);
    int ret = MXExecutorBindEX(
        reinterpret_cast<SymbolHandle>(symbolPtr), deviceTypeId, deviceID,
        static_cast<mx_uint>(numCtx), mapKeys, mapDevTypes, mapDevIDs,
        static_cast<mx_uint>(numArgs),
        reinterpret_cast<NDArrayHandle *>(inArgs),
        reinterpret_cast<NDArrayHandle *>(argGradStore),
        reinterpret_cast<mx_uint *>(gradReqType),
        static_cast<mx_uint>(auxStatesLen),
        reinterpret_cast<NDArrayHandle *>(auxStates), sharedExec, &out);
    env->ReleaseIntArrayElements(jctxMapDevIDs, mapDevIDs, 0);
    env->ReleaseIntArrayElements(jctxMapDevTypes, mapDevTypes, 0);
    env->ReleaseLongArrayElements(jargsGradHandle, argGradStore, 0);
    env->ReleaseLongArrayElements(jargsHandle, inArgs, 0);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxRandomSeed(JNIEnv *env,
                                                               jobject obj,
                                                               jint seed) {
...
JNIEXPORT jint JNICALL
Java_ml_dmlc_mxnet_LibInfo_mxNotifyShutdown(JNIEnv *env, jobject obj) {
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxRtcCreate(
    ...
    for (int i = 0; i < num_input; i++) {
        jstring jinname = reinterpret_cast<jstring>(
            env->GetObjectArrayElement(jinputNames, i));
    ...
    for (int i = 0; i < num_output; i++) {
        jstring joutname = reinterpret_cast<jstring>(
    ...
    int ret = MXRtcCreate(
        name, static_cast<mx_uint>(num_input), static_cast<mx_uint>(num_output),
        inputNames, outputNames, reinterpret_cast<NDArrayHandle *>(inputs),
        reinterpret_cast<NDArrayHandle *>(outputs), kernel, &out);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxRtcPush(
    ...
    int ret = MXRtcPush(
        handle, static_cast<mx_uint>(num_input),
        static_cast<mx_uint>(num_output),
        reinterpret_cast<NDArrayHandle *>(inputs),
        reinterpret_cast<NDArrayHandle *>(outputs),
        static_cast<mx_uint>(gridDimX), static_cast<mx_uint>(gridDimY),
        static_cast<mx_uint>(gridDimZ), static_cast<mx_uint>(blockDimX),
        static_cast<mx_uint>(blockDimY), static_cast<mx_uint>(blockDimZ));
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxRtcFree(JNIEnv *env,
                                                            jobject obj,
                                                            jlong jhandle) {
    RtcHandle handle = reinterpret_cast<RtcHandle>(jhandle);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxCustomOpRegister(
    ...
    auto creatorLambda = [](const char *opType, const int numKwargs,
        ...
        if (globalOpPropMap.find(opPropKey) == globalOpPropMap.end()) {
            ...
            _jvm->AttachCurrentThread(reinterpret_cast<void **>(&env), NULL);
            jclass opPropClass =
                env->GetObjectClass(globalOpPropMap.at(opPropKey));
            jmethodID midInit =
                env->GetMethodID(opPropClass, "init",
            ...
            if (NULL == midInit) {
                ...
                jobjectArray keysArr =
                    env->NewObjectArray(numKwargs, strCls, NULL);
                jobjectArray valuesArr =
                    env->NewObjectArray(numKwargs, strCls, NULL);
                ...
                env->CallVoidMethod(globalOpPropMap.at(opPropKey), midInit,
                                    keysArr, valuesArr);
        ...
        auto opPropListArgument = [](char ***args, void *state) {
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                JNIEnv *env;
                _jvm->AttachCurrentThread(reinterpret_cast<void **>(&env),
                                          NULL);
                jclass opPropClass =
                    env->GetObjectClass(globalOpPropMap.at(key));
                jmethodID midListArguments = env->GetMethodID(
                    opPropClass, "listArguments", "()[Ljava/lang/String;");
                if (NULL == midListArguments) {
                    LOG(WARNING)
                        << "could not find opProp method listArguments.";
                    success = false;
                } else {
                    ...
                    *args = new char *[len + 1];
                    for (int i = 0; i < len; ++i) {
                        jstring jarg = reinterpret_cast<jstring>(
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                if (NULL == midListOutputs) {
                    ...
                    jobjectArray joutputs =
                        (jobjectArray)(env->CallObjectMethod(
                            globalOpPropMap.at(key), midListOutputs));
                    int len = env->GetArrayLength(joutputs);
                    *outputs = new char *[len + 1];
                    for (int i = 0; i < len; ++i) {
                        jstring joutput = reinterpret_cast<jstring>(
                            env->GetObjectArrayElement(joutputs, i));
        ...
        auto opPropListAuxStates = [](char ***auxs, void *state) {
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                jclass opPropClass =
                    env->GetObjectClass(globalOpPropMap.at(key));
                jmethodID midListAuxStates =
                    env->GetMethodID(opPropClass, "listAuxiliaryStates",
                                     "()[Ljava/lang/String;");
                if (NULL == midListAuxStates) {
                    LOG(WARNING)
                        << "could not find opProp method listAuxiliaryStates.";
                    success = false;
                } else {
                    auto obj = env->CallObjectMethod(globalOpPropMap.at(key),
                                                     midListAuxStates);
                    if (obj != NULL) {
                        ...
                        *auxs = new char *[len + 1];
                        for (int i = 0; i < len; ++i) {
                            jstring jaux = reinterpret_cast<jstring>(
                                env->GetObjectArrayElement(jauxs, i));
        ...
        auto opPropDeclareBkDep = [](const int *outGrad, const int *inData,
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                if (NULL == midDeclareBkDep) {
                    ...
                    env->SetIntArrayRegion(outGradArr, (jsize)0, (jsize)outLen,
                                           outGrad);
                    jintArray inDataArr = env->NewIntArray(intLen);
                    env->SetIntArrayRegion(inDataArr, (jsize)0, (jsize)intLen,
                                           inData);
                    jintArray outDataArr = env->NewIntArray(outLen);
                    env->SetIntArrayRegion(outDataArr, (jsize)0, (jsize)outLen,
                                           outData);

                    auto obj = env->CallObjectMethod(
                        globalOpPropMap.at(key), midDeclareBkDep, outGradArr,
        ...
        auto opPropInferShape = [](int numInput, int *ndims, unsigned **shapes,
                                   void *state) {
            int success = true;
            std::string key(reinterpret_cast<char *>(state));
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                jclass opPropClass =
                    env->GetObjectClass(globalOpPropMap.at(key));
                jmethodID midInferShape = env->GetMethodID(
                    opPropClass, "inferShapeEntry", "(I[[I)[[I");
                if (NULL == midInferShape) {
                    LOG(WARNING)
                        << "could not find opProp method inferShapeEntry.";
                    success = false;
                } else {
                    jmethodID midListArguments = env->GetMethodID(
                        opPropClass, "listArguments", "()[Ljava/lang/String;");
                    ...
                    jobjectArray tensorShapes =
                        env->NewObjectArray(intLen, arrayClass, NULL);
                    for (int i = 0; i < intLen; ++i) {
                        ts[i] = env->NewIntArray(ndims[i]);
                        env->SetIntArrayRegion(
                            ts[i], (jsize)0, (jsize)ndims[i],
                            reinterpret_cast<int *>(shapes[i]));
                        env->SetObjectArrayElement(tensorShapes, i,
                                                   (jobject)(ts[i]));
                    }
                    jobjectArray ret = (jobjectArray)(env->CallObjectMethod(
                        globalOpPropMap.at(key), midInferShape, numInput,
                        tensorShapes));
                    for (int i = 0; i < numInput; ++i) {
                        jintArray jarr = reinterpret_cast<jintArray>(
                            env->GetObjectArrayElement(ret, i));
                        int len = env->GetArrayLength(jarr);
                        jint *arr = env->GetIntArrayElements(jarr, NULL);
                        ndims[i] = len;
                        shapes[i] = new unsigned[len];
                        for (int j = 0; j < len; ++j)
                            shapes[i][j] = (unsigned)(arr[j]);
        ...
        auto opPropInferType = [](int numInput, int *types, void *state) {
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                jclass opPropClass =
                    env->GetObjectClass(globalOpPropMap.at(key));
                jmethodID midInferType =
                    env->GetMethodID(opPropClass, "inferTypeEntry", "(I[I)[I");
                if (NULL == midInferType) {
                    LOG(WARNING)
                        << "could not find opProp method inferTypeEntry.";
                    success = false;
                } else {
                    jmethodID midListArguments = env->GetMethodID(
                        opPropClass, "listArguments", "()[Ljava/lang/String;");
                    ...
                    jintArray ret = (jintArray)(env->CallObjectMethod(
                        globalOpPropMap.at(key), midInferType, numInput, ts));
        ...
        auto opPropCreateOp = [](const char *ctx, int numInputs,
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                jmethodID midCreateOp = env->GetMethodID(
                    opPropClass, "createOperator",
                    "(Ljava/lang/String;[[I[I)Lml/dmlc/mxnet/CustomOp;");
                if (NULL == midCreateOp) {
                    LOG(WARNING)
                        << "could not find opProp method createOperator.";
                    success = false;
                } else {
                    ...
                    jobjectArray inputShapes =
                        env->NewObjectArray(numInputs, arrayClass, NULL);
                    for (int i = 0; i < numInputs; ++i) {
                        ts[i] = env->NewIntArray(ndims[i]);
                        env->SetIntArrayRegion(
                            ts[i], (jsize)0, (jsize)ndims[i],
                            reinterpret_cast<int *>(shapes[i]));
                        env->SetObjectArrayElement(inputShapes, i,
                                                   (jobject)(ts[i]));
                    }
                    jintArray jdtypes = env->NewIntArray(numInputs);
                    env->SetIntArrayRegion(jdtypes, (jsize)0, (jsize)numInputs,
                                           dtypes);
                    // get operator
                    jobject jOp = env->CallObjectMethod(globalOpPropMap.at(key),
                                                        midCreateOp, jctx,
                                                        inputShapes, jdtypes);
                    ...
                    auto forwardEntry = [](int size, void **ptrs, int *tags,
                                           const int *reqs, const int isTrain,
                                           void *state) {
                        std::string key(reinterpret_cast<char *>(state));
                        int success = true;
                        if (globalOpMap.find(key) == globalOpMap.end()) {
                            ...
                            JNIEnv *env;
                            _jvm->AttachCurrentThread(
                                reinterpret_cast<void **>(&env), NULL);
                            jclass opClass =
                                env->GetObjectClass(globalOpMap.at(key));
                            jmethodID midForward = env->GetMethodID(
                                opClass, "forwardEntry", "(I[J[I[IZ)Z");
                            if (NULL == midForward) {
                                LOG(WARNING)
                                    << "could not find op method forwardEntry.";
                                success = false;
                            } else {
                                jintArray tagsArr = env->NewIntArray(size);
                                env->SetIntArrayRegion(tagsArr, (jsize)0,
                                                       (jsize)size, tags);
                                int reqSize = 0;
                                ...
                                env->SetIntArrayRegion(reqsArr, (jsize)0,
                                                       (jsize)reqSize, reqs);
                                jlongArray ptrsArr = env->NewLongArray(size);
                                env->SetLongArrayRegion(
                                    ptrsArr, (jsize)0, (jsize)size,
                                    reinterpret_cast<jlong *>(ptrs));
#if MXNET_USE_CUDA
                                mxnet::NDArray *tmp =
                                    reinterpret_cast<mxnet::NDArray *>(ptrs[0]);
                                CUDA_CALL(cudaSetDevice(tmp->ctx().dev_id));
#endif
                                bool is_train = true;
                                if (isTrain == 0) is_train = false;
                                success = env->CallBooleanMethod(
                                    globalOpMap.at(key), midForward, size,
                                    ptrsArr, tagsArr, reqsArr, is_train);
                    ...
                    auto backwardEntry = [](int size, void **ptrs, int *tags,
                        ...
                        if (globalOpMap.find(key) == globalOpMap.end()) {
                            ...
                            jclass opClass =
                                env->GetObjectClass(globalOpMap.at(key));
                            jmethodID midBackward = env->GetMethodID(
                                opClass, "backwardEntry", "(I[J[I[IZ)Z");
                            if (NULL == midBackward) {
                                LOG(WARNING) << "could not find op method "
                                                "backwardEntry.";
                                success = false;
                            } else {
                                jintArray tagsArr = env->NewIntArray(size);
                                ...
                                if (isTrain == 0) is_train = false;
                                success = env->CallBooleanMethod(
                                    globalOpMap.at(key), midBackward, size,
                                    ptrsArr, tagsArr, reqsArr, is_train);
                                env->DeleteLocalRef(tagsArr);
                                env->DeleteLocalRef(reqsArr);
                                env->DeleteLocalRef(ptrsArr);
                    ...
                    // TODO(eric): Memory leak here. Refactor later and delete
                    // in delEntry
                    ret->num_callbacks = 3;
                    ret->callbacks = new MXGenericCallback[ret->num_callbacks];
                    ret->callbacks[kCustomOpDelete] =
                        reinterpret_cast<int (*)(void)>(
                            static_cast<int (*)(void *)>(delEntry));
                    ret->callbacks[kCustomOpForward] = reinterpret_cast<int (*)(
                        void)>(
                        static_cast<int (*)(int, void **, int *, const int *,
                                            const int, void *)>(forwardEntry));
                    ret->callbacks[kCustomOpBackward] =
                        reinterpret_cast<int (*)(void)>(
                            static_cast<int (*)(int, void **, int *,
                                                const int *, const int,
                                                void *)>(backwardEntry));
        ...
        auto opPropDel = [](void *state) {
            ...
            if (globalOpPropMap.find(key) == globalOpPropMap.end()) {
                ...
                _jvm->DetachCurrentThread();
                for (auto it = globalOpPropMap.begin();
                     it != globalOpPropMap.end();) {
                    if (it->first == key) {
                ...
                for (auto it = globalOpPropCountMap.begin();
                     it != globalOpPropCountMap.end();) {
                    if (it->first == key) {
        ...
        ret->num_callbacks = 8;
        ret->callbacks = new MXGenericCallback[ret->num_callbacks];
        ret->callbacks[kCustomOpPropDelete] = reinterpret_cast<int (*)(void)>(
        ...
        ret->callbacks[kCustomOpPropInferShape] =
            reinterpret_cast<int (*)(void)>(
                static_cast<int (*)(int, int *, unsigned **, void *)>(
                    opPropInferShape));
        ret->callbacks[kCustomOpPropDeclareBackwardDependency] =
            reinterpret_cast<int (*)(void)>(
                static_cast<int (*)(const int *, const int *, const int *,
                                    int *num_deps, int **, void *)>(
                    opPropDeclareBkDep));
        ret->callbacks[kCustomOpPropCreateOperator] =
            reinterpret_cast<int (*)(void)>(
                static_cast<int (*)(const char *, int, unsigned **, int *,
                                    int *, MXCallbackList *, void *)>(
                    opPropCreateOp));
    ...
    CustomOpPropCreator creator =
        static_cast<int (*)(const char *, const int, const char **,
                            const char **, MXCallbackList *)>(creatorLambda);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_mxnet_LibInfo_mxDumpProfile(JNIEnv *env,
                                                                jobject obj) {
