...
class CallbackOnComplete {
   public:
    ...
    inline void operator()() const { (*callback_)(engine_, param_); }
...
class MXNET_API Engine {
   public:
     ...
     */
    virtual void Push(OprHandle op, Context exec_ctx, int priority = 0,
                      bool profiling = false) = 0;
    /*!
     ...
     */
    virtual void DeleteVariable(SyncFn delete_fn, Context exec_ctx,
                                VarHandle var) = 0;
    /*!
     * \brief Wait for a variable.
     * \param var The variable we should wait for. This function returns when
     * the
     *            variable is ready.
     */
     ...
     */
    inline CallbackOnComplete CreateCallback(void (*callback)(Engine*, void*),
                                             void* param) {
    ...
    inline void DeduplicateVarHandle(
