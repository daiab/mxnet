 ...
 *  Operator defines basic operation unit of optimized computation graph in
 * mxnet.
 *  This interface relies on pre-allocated memory in TBlob, the caller need to
 * set
 *  the memory region in TBlob correctly before calling Forward and Backward.
 *
 *  Operator is generated by OperatorProperty.
 *  To add new operator(aka. layers of neural nets) to mxnet, developer need to
 * create
...
class Operator {
   public:
    ...
    enum ExecType {
         ...
         *  That indicates copy across devices, the input and output can sit on
         * different device.
...
/*!
 * \brief OperatorProperty is a object that stores all information about
 * Operator.
...
class OperatorProperty {
   public:
     ...
     */
    virtual std::vector<std::string> ListArguments() const { return {"data"}; }
    /*!
     ...
     */
    virtual std::vector<std::string> ListAuxiliaryStates() const { return {}; }
    /*! \return number of real return values of the Operator */
    virtual int NumOutputs() const { return this->ListOutputs().size(); }
    /*!
     ...
     *  For example, Dropout will return [data, mask], with NumVisibleOutputs()
     * == 1.
     *  So when user call sym = Dropout(input), only data is presented in sym.
     *  But all the returns will be presented in out_data parameter of Backward
     * if requested.
     *
     * \return number of default return values
     */
    virtual int NumVisibleOutputs() const { return NumOutputs(); }
    /*!
     * \brief infer the shapes of outputs and unknown input arguments
     * \param in_shape the shape of input arguments of the operator
     *     this should be of same length as the vector returned by DescribeArgs
     *     in_shape allows unknown elements, which are checked by shape.ndim()
     * == 0.
     *     For unknown shapes, InferShape will try to fill in the correct Shape
     * in in_shape
     *     For known shapes, InferShape will check shape consistency
     *
     *     common practice: set the shape of data input, and usually weight's
     ...
     *     InferShape will modify the vector to fill output TShape
     * \return true if the shape inference is successful, false if there is not
     * enough information.
     ...
     *     this should be of same length as the vector returned by DescribeArgs
     *     in_type allows unknown elements, which are checked by type.ndim() ==
     * 0.
     *     For unknown types, Infertype will try to fill in the correct type in
     * in_type
     *     For known types, Infertype will check type consistency
     *
     *     common practice: set the type of data input, and usually weight's
     ...
     *     Infertype will modify the vector to fill output Ttype
     * \return true if the type inference is successful, false if there is not
     * enough information.
     ...
     * \param in_shape The input shape to the operator, corresponds to shapes of
     * in_data.
     ...
     *  If this function is not overriden, all the variables will be valid in
     * Backward.
     *
     * \code
     *  // The following code declares Backward need out_grad[0],
     * in_data[0],in_data[1]
     ...
     *  vector<pair<int, void*> > ForwardInplaceOption(const vector<int>
     * &in_data,
     *                                                 const vector<void*>
     * &out_data) const {
     ...
     *  This function will select the result list of T according to
...
struct OperatorPropertyReg
     ...
     */
    inline OperatorPropertyReg &set_key_var_num_args(
...
// See also MXNET_REGISTER_SIMPLE_OP in operator_util.h for registering simple
// ops.
//---------------------------------------------------------------------------------
/*!
...
#define MXNET_REGISTER_OP_PROPERTY(name, OperatorPropertyType)                \
    DMLC_REGISTRY_REGISTER(::mxnet::OperatorPropertyReg, OperatorPropertyReg, \
                           name)                                              \
