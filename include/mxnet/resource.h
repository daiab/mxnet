...
struct Resource {
     ...
     *  So the caller need to serialize the calls when using the conflicted
     * space.
     *  The old space can get freed, however, this will incur a synchronization,
     *  when running on device, so the launched kernels that depend on the temp
     * space
    ...
    inline mshadow::Tensor<xpu, ndim, DType> get_space_typed(
        ...
        return mshadow::Tensor<xpu, ndim, DType>(
            reinterpret_cast<DType *>(
                get_space_internal(shape.Size() * sizeof(DType))),
    ...
    inline mshadow::Tensor<cpu, ndim, DType> get_host_space_typed(
        ...
        return mshadow::Tensor<cpu, ndim, DType>(
            reinterpret_cast<DType *>(
                get_host_space_internal(shape.Size() * sizeof(DType))),
