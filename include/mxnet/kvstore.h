...
class KVStore {
   public:
     ...
     *   - 'device' or 'local_allreduce_device' : same to local but use gpus for
     * kv
     *       allreduce
     ...
     *
     * If a key appears mulitple times in \a keys, then the according values
     * will
     ...
     * \endcode
     *
     * One can set a user-defined updater by \ref set_updater. The default
     * updater
     * is Assign.
     *
    ...
    virtual void Push(const std::vector<int>& keys,
                      const std::vector<NDArray>& values, int priority = 0) = 0;
    /*!
     * \brief pull a list of key-value pairs from the store
     *
     * One must call Init() on \a key before. And \a value should be
     * pre-allocated
     *
     ...
     *
     * \param keys the list of keys
     * \param values the list of buffers for the pulled data, they should be
     * preallocated
     * \param priority Priority of the action.
     */
     ...
     */
    static void InitPSEnv(
        const std::unordered_map<std::string, std::string>& envs) {
#if MXNET_USE_DIST_KVSTORE
        ps::Environment::Init(envs);
#else
        LOG(FATAL) << "compile with USE_DIST_KVSTORE=1 to init parameter "
