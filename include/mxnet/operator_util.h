 ...
 *  [Deprecated]
 *  Use the register functions in this file when possible to simplify operator
 * creations.
 *  Operators registered in this file will be exposed to both NDArray API and
 * symbolic API.
 *
 * \author Tianqi Chen
 */
...
#ifdef _MSC_VER
#pragma warning( \
    disable : 4503)  // disable warning: decorated name length exceeded.
#endif
 ...
 */
typedef void (*SourceFunction)(const EnvArguments& env, TBlob* ret,
                               OpReqType req, RunContext ctx);
 ...
 * \param ctx Runtime context to execute the function.
 */
typedef void (*UnaryFunction)(const TBlob& src, const EnvArguments& env,
                              TBlob* ret, OpReqType req, RunContext ctx);
/*!
...
/*!
 * \brief Gradient function that takes output value of function and computes
...
typedef void (*UnaryGradFunctionT0)(const OutputGrad& out_grad,
                                    const EnvArguments& env, TBlob* in_grad,
                                    OpReqType req, RunContext ctx);
/*!
 * \brief Gradient function that takes output value of function and computes
 * gradient wrt to input.
...
typedef void (*UnaryGradFunctionT1)(const OutputGrad& out_grad,
                                    ...
                                    OpReqType req, RunContext ctx);
/*!
 * \brief Gradient function that takes input value of function and computes
 * gradient wrt to input.
 * \param out_grad the gradient wrt to output of the function.
 ...
 * \param ctx Runtime context to execute the function.
 */
typedef void (*BinaryFunction)(const TBlob& lhs, const TBlob& rhs,
                               const EnvArguments& env, TBlob* ret,
                               OpReqType req, RunContext ctx);
 ...
 * \return The inferred result shape.
 */
typedef TShape (*BinaryShapeFunction)(const TShape& lhs, const TShape& rhs,
                                      const EnvArguments& env);
/*!
 * \brief Gradient function that takes only output gradient and computes
 * gradient wrt to input.
...
typedef void (*BinaryGradFunctionT0)(const OutputGrad& out_grad,
                                     const EnvArguments& env, TBlob* lhs_grad,
                                     TBlob* rhs_grad, OpReqType req_lhs_grad,
                                     OpReqType req_rhs_grad, RunContext ctx);
/*!
 * \brief Gradient function that takes inputs of function anod computes gradient
 * wrt to input.
 * \param out_grad the gradient wrt to output of the function.
...
typedef void (*BinaryGradFunctionT1)(const OutputGrad& out_grad,
                                     const Input0& lhs, const Input1& rhs,
                                     const EnvArguments& env, TBlob* lhs_grad,
                                     TBlob* rhs_grad, OpReqType req_lhs_grad,
                                     OpReqType req_rhs_grad, RunContext ctx);
...
class SimpleOpRegEntry {
   public:
    ...
    virtual TSelf& set_function(
        int dev_mask, SourceFunction fsource,
     ...
     */
    virtual TSelf& set_function(
        int dev_mask, UnaryFunction funary,
     ...
     */
    virtual TSelf& set_function(
        int dev_mask, BinaryFunction fbinary,
     ...
     */
    virtual TSelf& set_gradient(int dev_mask, UnaryGradFunctionT0 fgrad,
     ...
     * \param inplace_out_in_grad whether out_grad and in_grad can share memory.
     */
    virtual TSelf& set_gradient(int dev_mask, UnaryGradFunctionT1 fgrad,
                                SimpleOpInplaceOption inplace_out_in_grad) = 0;
    /*!
     * \brief set gradient of the function of this function.
     * \param dev_mask The device mask of the function can act on.
     ...
     * \param inplace_out_in_grad whether out_grad and in_grad can share memory.
     */
    virtual TSelf& set_gradient(int dev_mask, UnaryGradFunctionT2 fgrad,
                                SimpleOpInplaceOption inplace_out_in_grad) = 0;
    /*!
     * \brief set gradient of the function of this function.
     * \param dev_mask The device mask of the function can act on.
     * \param fgrad The gradient function to be set.
     * \param inplace_out_lhs_grad whether out_grad and lhs_grad can share
     * memory.
     */
    virtual TSelf& set_gradient(int dev_mask, BinaryGradFunctionT0 fgrad,
     ...
     * memory.
     */
    virtual TSelf& set_gradient(int dev_mask, BinaryGradFunctionT1 fgrad,
                                SimpleOpInplaceOption inplace_out_lhs_grad) = 0;
    /*!
...
/*!
* \brief Maximum ndim supported for special operators like broadcasting with non
* contiguous lhs/rhs
*/
