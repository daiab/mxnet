...
class NDArray {
   public:
    ...
    inline const TBlob &data() const {
        ...
        MSHADOW_TYPE_SWITCH(dtype_, DType, {
            tblob_ = TBlob(static_cast<DType *>(ptr_->shandle.dptr) + offset_,
                           ...
                           ptr_->shandle.ctx.dev_id, Mkl_mem_);
        });
#else
        MSHADOW_TYPE_SWITCH(dtype_, DType, {
            tblob_ = TBlob(static_cast<DType *>(ptr_->shandle.dptr) + offset_,
                           shape_, ptr_->shandle.ctx.dev_mask(),
                           ptr_->shandle.ctx.dev_id);
        });
#endif
        return tblob_;
    }
    ...
    inline TBlob raw_data(index_t offset, index_t length) const {
        ...
        MSHADOW_TYPE_SWITCH(dtype_, DType, {
            res = TBlob(
                static_cast<DType *>(ptr_->shandle.dptr) + offset_ + offset,
                raw_shape, ptr_->shandle.ctx.dev_mask(),
                ptr_->shandle.ctx.dev_id);
        });
#if MKL_EXPERIMENTAL == 1
     ...
     */
    inline Context ctx() const { return ptr_->shandle.ctx; }
    /*!
     ...
     * \param size the memory size we want to copy into, in sizeof(DType) not
     * raw btyes.
     */
     ...
     * This is an internal function used by system that normal user should not
     * use
     */
    inline void CheckAndAlloc() const { ptr_->CheckAndAlloc(); }
    /*!
     ...
     */
    static void Load(dmlc::Stream *fi, std::vector<NDArray> *data,
   ...
   private:
    ...
    struct Chunk {
        ...
        Chunk(const TBlob &data, int dev_id)
            ...
            shandle.size =
                data.shape_.Size() * mshadow::mshadow_sizeof(data.type_flag_);
        }
        ...
        ~Chunk() {
            if (static_data || delay_alloc) {
                Engine::Get()->DeleteVariable([](RunContext s) {}, shandle.ctx,
                                              var);
            } else {
                Storage::Handle h = this->shandle;
                Engine::Get()->DeleteVariable(
                    [h](RunContext s) { Storage::Get()->Free(h); }, shandle.ctx,
                    var);
            }
        }
    };
 ...
 * \return a new result ndarray
 */
NDArray operator*(const NDArray &lhs,
                  const NDArray &rhs); /*!
                                        * \brief elementwise multiplication
                                        * \param lhs left operand
                                        * \param rhs right operand
...
typedef std::function<void(NDArray **used_vars, real_t *scalars,
                           NDArray **mutate_vars, int num_params,
...
struct NDArrayFunctionReg
     ...
     */
    NDArrayFunctionReg()
        : num_use_vars(0), num_mutate_vars(0), num_scalars(0), type_mask(0) {}
    /*!
    ...
    inline NDArrayFunctionReg &set_function(void (*fsetvalue)(const real_t &rhs,
                                                              NDArray *out)) {
        body = [fsetvalue](NDArray **used_vars, real_t *s,
                           NDArray **mutate_vars, int num_params,
                           char **param_keys, char **param_vals) {
    ...
    */
    inline NDArrayFunctionReg &set_function(void (*fternary)(const NDArray &lhs,
                                                             ...
                                                             const NDArray &rhs,
                                                             NDArray *out)) {
        body = [fternary](NDArray **used_vars, real_t *s, NDArray **mutate_vars,
                          int num_params, char **param_keys,
                          char **param_vals) {
            (*fternary)(*used_vars[0], *used_vars[1], *used_vars[2],
                        mutate_vars[0]);
        };
        num_use_vars = 3;
        num_mutate_vars = 1;
...
#define MXNET_REGISTER_NDARRAY_FUN(name)                                    \
    DMLC_REGISTRY_REGISTER(::mxnet::NDArrayFunctionReg, NDArrayFunctionReg, \
                           name)
