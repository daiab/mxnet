...
class OpMap {
    ...
    */
    inline OpMap() {
        ...
        int r = MXSymbolListAtomicSymbolCreators(&num_symbol_creators,
                                                 &symbol_creators);
        CHECK_EQ(r, 0);
        for (mx_uint i = 0; i < num_symbol_creators; i++) {
            ...
            r = MXSymbolGetAtomicSymbolInfo(
                symbol_creators[i], &name, &description, &num_args, &arg_names,
    ...
    */
    inline AtomicSymbolCreator GetSymbolCreator(const std::string &name) {
        if (symbol_creators_.count(name) == 0) return GetOpHandle(name);
