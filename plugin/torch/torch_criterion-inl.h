...
struct TorchCriterionParam : public dmlc::Parameter<TorchCriterionParam> {
    ...
    DMLC_DECLARE_PARAMETER(TorchCriterionParam) {
        DMLC_DECLARE_FIELD(lua_string)
            .describe(
                "lua string that is called to generate the torch criterion "
                "object");
...
class TorchCriterionOp : public Operator {
   ...
   public:
    ...
    ~TorchCriterionOp() { delete this->torchState_; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        for (index_t i = 0; i < in_data.size(); ++i) {
            THGeneralTensor th =
                TorchTensor::TBlobToTHTensor(torchState_, in_data[i]);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        CHECK_EQ(req[0], kWriteTo)
            << "Torch Criterion only supports write to in_grad";
        CHECK_EQ(req[1], kNullOp)
            << "Torch Criterion cannot back prop to label";
        Stream<xpu> *s = ctx.get_stream<xpu>();
        torchState_->SetStream(s);
        lua_rawgeti(L, LUA_REGISTRYINDEX, lua_reference_);
        THGeneralTensor th =
            TorchTensor::TBlobToTHTensor(torchState_, in_grad[0]);
...
class TorchCriterionProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "TorchCriterion"; }
    ...
    std::vector<int> DeclareBackwardDependency(
        ...
        // Ensure that the backward and forward cannot be called at the same
        // time
