...
void TorchRunOp(std::vector<NDArray> arr_in, std::vector<NDArray> arr_out,
    ...
    for (size_t i = 0; i < format.size(); ++i) {
        ...
        switch (format[i]) {
            case 'n': {
                CHECK(idx < arr.size())
                    << "Too few NDArray arguments for Torch." << OP::fname;
                luaT_pushudata(L, TorchTensor::TBlobToTHTensor(torchState,
                                                               arr[idx].data()),
            ...
            case 'b':
                ...
                break;
            default:
                LOG(FATAL) << "Unknown argument type " << format[i]
                           << " for Torch." << OP::fname;
        }
    }
    CHECK_EQ(lua_pcall(L, format.size(), 0, 0), 0) << "Lua Error: "
                                                   << lua_tostring(L, -1);
}
...
void TorchOp(NDArray** u, real_t* s, NDArray** out,
    ...
    if (OP::num_inputs) {
        ...
        for (int i = 0; i < OP::num_inputs; ++i) {
            CHECK_EQ(ctx, u[i]->ctx())
                << "Context of all oprands must be the same.";
            CHECK_EQ(type_flag, u[i]->dtype())
                << "Data type of all oprands must be the same.";
        }
    } else {
        CHECK(param.count("ctx"))
            << "Must provide keyword argument ctx for TorchOp with 0 inputs";
    ...
    var_out.resize(std::unique(var_out.begin(), var_out.end()) -
                   var_out.begin());
    std::set_difference(var_in.begin(), var_in.end(), var_out.begin(),
                        var_out.end(),
...
struct TorchFirstShape {
    static std::vector<mshadow::TShape> GetShape(
        NDArray** u, const std::map<std::string, std::string>& param) {
        return {u[0]->shape()};
    }
};
...
#define MXNET_REGISTER_TORCH_CONSTRUCTOR_FUN(name, func) \
    struct TorchConstructorOpDesc_##name##_##func        \
        : public TorchConstructorShape {                 \
        static constexpr const char* fname = #func;      \
    };                                                   \
