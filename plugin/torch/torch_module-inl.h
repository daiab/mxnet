...
struct TorchModuleParam : public dmlc::Parameter<TorchModuleParam> {
    ...
    DMLC_DECLARE_PARAMETER(TorchModuleParam) {
        DMLC_DECLARE_FIELD(lua_string)
            .describe(
                "lua string that is called to generate the torch module "
                "object");
...
class TorchModuleOp : public Operator {
   ...
   public:
    explicit TorchModuleOp(TorchModuleParam p, TorchState* torchState)
        : torchState_(torchState) {
        this->param_ = p;
        ...
        if (param_.num_params != 0) {
            ...
            lua_pushnil(L);
            while (lua_next(L, -2)) {
                CHECK(luaT_isudata(L, -1,
                                   TorchTensor::TensorType(xpu::kDevMask)));
                void* udata =
                    luaT_toudata(L, -1, TorchTensor::TensorType(xpu::kDevMask));
                TorchTensor::FreeInternal(torchState_,
                                          static_cast<THGeneralTensor>(udata),
                                          xpu::kDevMask);
                lua_pop(L, 1);
            }
    ...
    virtual void Forward(const OpContext& ctx,
        ...
        if (param_.num_params != 0) {
            ...
            lua_pushnil(L);
            std::vector<TBlob>::const_iterator it =
            ...
            while (lua_next(L, -2)) {
                ...
                TorchTensor::SetInternal(
                    torchState_, static_cast<THGeneralTensor>(udata), *(it));
                it++;
                lua_pop(L, 1);
            }
            lua_pop(L, 1);  // pop the parameter table
        ...
        CHECK_EQ(err, 0) << lua_tostring(L, -1);
        TorchTensor::CheckOutput(torchState_, out_data.begin(),
                                 out_data.begin() + param_.num_outputs,
    ...
    virtual void Backward(const OpContext& ctx,
        ...
        if (param_.num_params != 0) {
            ...
            while (lua_next(L, -3)) {
                TorchTensor::SetInternal(
                    torchState_, static_cast<THGeneralTensor>(luaT_toudata(
                                     L, -1, TorchTensor::TensorType(*it))),
                    *it);
                it++;
                lua_pop(L, 1);
            }
        ...
        TorchTensor::TBlobVectorAsTable(torchState_, in_data.begin(),
                                        in_data.begin() + param_.num_data);
        TorchTensor::TBlobVectorAsTable(torchState_, out_grad.begin(),
                                        out_grad.end());
        // call
        ...
        CHECK_EQ(err, 0) << lua_tostring(L, -1);
        TorchTensor::CheckOutput(torchState_, in_grad.begin(),
...
class TorchModuleProp : public OperatorProperty {
   ...
   public:
    TorchModuleProp()
        : OperatorProperty(), torchState_(NULL), lua_reference_(-1) {}
    ...
    bool InferShape(std::vector<TShape>* in_shape,
        ...
        if (param_.num_data == 1) {
            THLongStorage* thshape =
                THLongStorage_newWithSize((*in_shape)[0].ndim());
        ...
        if (param_.num_params != 0) {
            ...
            while (lua_next(L, -3)) {
                THFloatTensor* param = reinterpret_cast<THFloatTensor*>(
                    luaT_toudata(L, -1, TorchTensor::TensorType(
                                            mshadow::cpu::kDevMask)));
                long int* size = param->size;  // NOLINT(*)
                (*in_shape)[index++] =
                    TShape(size, size + THFloatTensor_nDimension(param));
                lua_pop(L, 1);
            }
            lua_pop(L, 2);
        }
        ...
        if (param_.num_outputs == 0) {
            ...
            THFloatTensor* output =
                reinterpret_cast<THFloatTensor*>(luaT_toudata(
                    L, -1, TorchTensor::TensorType(mshadow::cpu::kDevMask)));
            long int* size = output->size;  // NOLINT(*)
            (*out_shape)[0] =
                TShape(size, size + THFloatTensor_nDimension(output));
        } else {
            for (uint32_t data_index = 0; data_index < param_.num_outputs;
                 ++data_index) {
                lua_pushnil(L);
                int index = 0;
                while (lua_next(L, -2)) {
                    ...
                    (*out_shape)[index++] =
                        TShape(size, size + THFloatTensor_nDimension(out));
                }
            }
        }
        lua_pop(L, 2);
    ...
    std::string TypeString() const override { return "TorchModule"; }
