...
class TorchState {
   public:
    ...
    int Serialize(THCharStorage** chunk) {
        ...
        *chunk = reinterpret_cast<THCharStorage*>(
            luaT_toudata(L, -1, "torch.CharStorage"));
        THCharStorage_retain(
            *chunk);  // keep the chunk even when lua side deletes
        lua_pop(L, 2);
        return 0;
    }
};
...
class TorchTensor {
   public:
    ...
    static THGeneralTensor TBlobToTHTensor(TorchState* torchState, TBlob data) {
        ...
        CHECK_EQ(data.type_flag_, mshadow::kFloat32)
            << "Torch Interface only support float32";
        switch (data.dev_mask_) {
            case cpu::kDevMask: {
                THFloatStorage* storage = THFloatStorage_newWithData(
                    static_cast<real_t*>(data.dptr_), size);
                THFloatStorage_clearFlag(storage, TH_STORAGE_FREEMEM);
                tensor = (THGeneralTensor)THFloatTensor_newWithStorage(
                    storage, 0, thshape, NULL);
            ...
            case gpu::kDevMask: {
                ...
                THCudaStorage* storage = THCudaStorage_newWithData(
                    state, static_cast<real_t*>(data.dptr_), size);
                // a bug in cutorch
                THFloatStorage_clearFlag(
                    reinterpret_cast<THFloatStorage*>(storage),
                    TH_STORAGE_FREEMEM);
                tensor = (THGeneralTensor)THCudaTensor_newWithStorage(
                    state, storage, 0, thshape, NULL);
    ...
    static void FreeInternal(TorchState* torchState, THGeneralTensor tensor,
                             int dev_mask) {
        switch (dev_mask) {
            case cpu::kDevMask: {
                THFloatStorage* original =
                    static_cast<THFloatTensor*>(tensor)->storage;
            ...
            case gpu::kDevMask: {
                THCState* state = torchState->CudaState();
                THCudaStorage* original =
                    static_cast<THCudaTensor*>(tensor)->storage;
    ...
    static void SetInternal(TorchState* torchState, THGeneralTensor tensor,
                            const TBlob& blob) {
        size_t size = blob.Size();
        switch (blob.dev_mask_) {
            case cpu::kDevMask: {
                THFloatStorage* storage = THFloatStorage_newWithData(
                    static_cast<real_t*>(blob.dptr_), size);
                THFloatStorage_clearFlag(storage, TH_STORAGE_FREEMEM);
                THFloatStorage* original =
                    static_cast<THFloatTensor*>(tensor)->storage;
            ...
            case gpu::kDevMask: {
                THCState* state = torchState->CudaState();
                THCudaStorage* storage = THCudaStorage_newWithData(
                    state, static_cast<real_t*>(blob.dptr_), size);
    ...
    static std::vector<THGeneralTensor> TBlobVectorAsTable(
        TorchState* torchState, const std::vector<TBlob>::const_iterator begin,
        ...
        if (num > 1) {
            ...
            int index = 1;
            for (std::vector<TBlob>::const_iterator it = begin; it != end;
                 ++it) {
                THGeneralTensor th =
                    TorchTensor::TBlobToTHTensor(torchState, *it);
                res.push_back(th);
            ...
            THGeneralTensor th =
                TorchTensor::TBlobToTHTensor(torchState, *begin);
            res.push_back(th);
    ...
    static void CopyIfDifferent(TorchState* torchState, TBlob dst,
                                THGeneralTensor th_dst) {
        lua_State* L = torchState->L;
        if (luaT_isudata(L, -1, TorchTensor::TensorType(cpu::kDevMask))) {
            ...
            if (src->storage != static_cast<THCudaTensor*>(th_dst)->storage) {
                THCudaTensor_copy(torchState->CudaState(),
                                  static_cast<THCudaTensor*>(th_dst), src);
            }
#endif  // MXNET_USE_CUDA
        } else {
            LOG(FATAL) << "Unsupported Torch tensor type "
                       << luaT_typename(L, -1);
        }
    }

    static void CheckOutput(
        TorchState* torchState, std::vector<TBlob>::const_iterator begin,
