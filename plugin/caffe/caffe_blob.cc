...
template <>
void SetDataGradToBlob<mshadow::cpu, float>(
    caffeMemoryTypes memType,
...
template <>
void SetDataGradToBlob<mshadow::cpu, double>(
    caffeMemoryTypes memType,
...
template <>
void SetDataGradToBlob<mshadow::gpu, float>(
    caffeMemoryTypes memType,
    std::vector<::caffe::Blob<float> *>::iterator blob,
    std::vector<TBlob>::const_iterator itr) {
    float *data_ptr = reinterpret_cast<float *>((*itr).dptr_);
...
template <>
void SetDataGradToBlob<mshadow::gpu, double>(
    caffeMemoryTypes memType,
    std::vector<::caffe::Blob<double> *>::iterator blob,
    std::vector<TBlob>::const_iterator itr) {
    double *data_ptr = reinterpret_cast<double *>((*itr).dptr_);
...
TShape Vector2TShape(const std::vector<int> &vec_int) {
    ...
    for (uint32_t i = 0; i < vec_int.size(); ++i) vec.push_back(vec_int[i]);
    // 0-dim represents scalar in caffe
    if (vec_int.size() == 0) vec.push_back(1);
    return {vec.begin(), vec.end()};
}

std::vector<int> TShape2Vector(const TShape &tshape) {
    std::vector<int> s;
    for (uint32_t i = 0; i < tshape.ndim(); ++i) s.push_back(tshape[i]);
    return s;
}
