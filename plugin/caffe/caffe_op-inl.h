...
struct CaffeOpParam : public dmlc::Parameter<CaffeOpParam> {
    ...
    DMLC_DECLARE_PARAMETER(CaffeOpParam) {
        ...
        DMLC_DECLARE_FIELD(num_weight).set_default(0).describe("Weight number");
        DMLC_DECLARE_FIELD(num_out)
            .set_default(1)
...
class CaffeOp : public Operator {
   public:
    explicit CaffeOp(CaffeOpParam p)
        : param_(p), init_w_(false), init_wd_(false), setup_(false) {
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        for (uint32_t i = 0; i < req.size(); ++i) CHECK_EQ(req[i], kWriteTo);
        ...
        caffe::TBlob2CaffeBlob<xpu, Dtype>(caffe::Data, bot_.begin(),
                                           in_data.begin(), param_.num_data);
        caffe::TBlob2CaffeBlob<xpu, Dtype>(caffe::Data, top_.begin(),
                                           out_data.begin(), param_.num_out);
        CaffeOpSetup();
        // Init caffe's weight pointer
        if (!init_w_) {
            init_w_ = true;
            caffe::TBlob2CaffeBlob<xpu, Dtype>(
                caffe::Data, wei_.begin(), in_data.begin() + param_.num_data,
                param_.num_weight);
        ...
        for (uint32_t i = 0; i < top_.size(); ++i) top_[i]->gpu_data();
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        int expected_num_data = param_.num_weight + param_.num_data;
        CHECK(in_data.size() == expected_num_data &&
              in_grad.size() == expected_num_data);
        ...
        caffe::TBlob2CaffeBlob<xpu, Dtype>(caffe::Grad, bot_.begin(),
                                           in_grad.begin(), param_.num_data);
        caffe::TBlob2CaffeBlob<xpu, Dtype>(caffe::Grad, top_.begin(),
                                           out_grad.begin(), param_.num_out);

        // Init caffe's gradient pointer
        if (!init_wd_) {
            init_wd_ = true;
            caffe::TBlob2CaffeBlob<xpu, Dtype>(
                caffe::Grad, wei_.begin(), in_grad.begin() + param_.num_data,
                param_.num_weight);
        }
        ...
        // Set BP flag
        for (int i = 0; i < param_.num_data; ++i) flags_[i] = req[i] != kNullOp;

        caffeOp_->Backward(top_, flags_, bot_);

#if defined(__CUDACC__)
        // Sync cpu diff to gpu diff
        for (uint32_t i = 0; i < bot_.size(); ++i) bot_[i]->gpu_diff();

        CHECK_EQ(cudaStreamSynchronize(NULL), cudaSuccess);
#endif  // __CUDACC__
    }

    void HandleOpReq(mshadow::Stream<xpu> *s, OpReqType req,
                     const TBlob &in_g) {
...
class CaffeOpProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (caffeOp_ == NULL)
            caffeOp_ =
                caffe::LayerRegistry<float>::CreateLayer(param_.prototxt);
        using namespace mshadow;
        using ::caffe::Blob;
        using std::vector;
        ...
        for (auto blob_ptr : bot_blobs) delete blob_ptr;
        for (auto blob_ptr : top_blobs) delete blob_ptr;
        return true;
    }
    ...
    std::string TypeString() const override { return "CaffeOp"; }
