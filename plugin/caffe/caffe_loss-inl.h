...
struct CaffeLossParam : public dmlc::Parameter<CaffeLossParam> {
    ...
    DMLC_DECLARE_PARAMETER(CaffeLossParam) {
        ...
        DMLC_DECLARE_FIELD(grad_scale)
            .set_default(1.0f)
            .describe(
                "Scale the gradient by a float factor (a.k.a weight of this "
                "loss).");
    }
};
...
class CaffeLoss : public Operator {
   public:
    explicit CaffeLoss(CaffeLossParam p) : param_(p), setup_(false) {
...
class CaffeLossProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "CaffeLoss"; }
