...
class FieldEntry<caffe::LayerParameter>
    : public FieldEntryBase<FieldEntry<caffe::LayerParameter>,
                            caffe::LayerParameter> {
   public:
    // parent class
    typedef FieldEntryBase<FieldEntry<caffe::LayerParameter>,
                           caffe::LayerParameter>
        Parent;
    ...
    virtual void Set(void *head, const std::string &value) const {
        ...
        if (!ReadProtoFromTextContent(value, &net_param))
            CHECK(false) << "Caffe Net Prototxt: " << value
                         << "Initialized Failed";

        CHECK_EQ(net_param.layer_size(), 1) << "Prototxt" << value
                                            << " more than a layer";
        caffe::LayerParameter *layer_param =
            new caffe::LayerParameter(net_param.layer(0));
        this->Get(head) = (*layer_param);
    }

    virtual void PrintValue(std::ostream &os,
                            caffe::LayerParameter value) const {  // NOLINT(*)
    }
    ...
    virtual void PrintDefaultValueString(std::ostream &os) const {  // NOLINT(*)
        ...
        // pointer
        np.mutable_layer()->AddAllocated(
            const_cast<::caffe::LayerParameter *>(&default_value_));
    ...
    inline FieldEntry<caffe::LayerParameter> &set_default(
