...
// Gets the JPEG size from the array of data passed to the function, file
// reference: http://www.obrador.com/essentialjpeg/headerinfo.htm
bool get_jpeg_size(const unsigned char *data, mx_uint data_size, mx_uint *width,
                   mx_uint *height) {
...
bool get_png_size(const unsigned char *data, mx_uint data_size, mx_uint *width,
                  mx_uint *height) {
...
MXNET_DLL int MXCVImdecode(const unsigned char *img, const mx_uint len,
    ...
    NDArray ndout(TShape(dims, dims + 3), Context::CPU(), true,
                  mshadow::kUint8);
...
MXNET_DLL int MXCVResize(NDArrayHandle src, const mx_uint w, const mx_uint h,
    ...
    Engine::Get()->PushSync(
        [=](RunContext ctx) {
            ...
            cv::Mat dst(h, w, dims[2] == 3 ? CV_8UC3 : CV_8U,
                        ndout.data().dptr_);
...
MXNET_DLL int MXCVcopyMakeBorder(NDArrayHandle src, const int top,
                                 const int bot, const int left, const int right,
    ...
    Engine::Get()->PushSync(
        [=](RunContext ctx) {
            ...
            cv::Mat dst(top + h + bot, left + w + right,
                        c == 3 ? CV_8UC3 : CV_8U, ndout.data().dptr_);
            cv::copyMakeBorder(buf, dst, top, bot, left, right, type,
                               cv::Scalar(value));
            CHECK(!dst.empty());
        },
        ndout.ctx(), {ndsrc.var()}, {ndout.var()});
