...
class WarpCTCOp : public Operator {
   ...
   public:
    explicit WarpCTCOp(WarpCTCParam p) { this->param_ = p; }
    ...
    void removeBlank(const int *flat_labels, int *cpu_labels, int size,
                     int blank) {
        int k = 0;
        for (int i = 0; i < size; i++) {
            if (flat_labels[i] != blank) {
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        CHECK_EQ(data.shape_.ndim(), 2)
            << "input data shape should be 2 (t*n, p)";
...
#if MXNET_USE_CUDA
            cpu_raw_labels =
                reinterpret_cast<int *>(malloc(sizeof(int) * label.Size()));
            cuda_status = cudaMemcpyAsync(
                cpu_raw_labels, flat_labels, label.Size() * sizeof(int),
                cudaMemcpyDeviceToHost, ctx.get_stream<gpu>()->stream_);
            CHECK_EQ(cuda_status, cudaSuccess) << "cuda memcpy label error";
#endif
        }

        int total_label_length = 0;
        std::vector<int> label_lengths = labelLengths(
            cpu_raw_labels, minibatch, label.Size(), 0, &total_label_length);
        int *cpu_labels =
            reinterpret_cast<int *>(malloc(sizeof(int) * total_label_length));
        removeBlank(cpu_raw_labels, cpu_labels, label.Size(), 0);

        size_t alloc_bytes;
        throw_on_error(
            get_workspace_size(label_lengths.data(), input_lengths.data(),
                               alphabet_size, input_lengths.size(), info,
                               &alloc_bytes),
            "Error: get_workspace_size in inf_test");

        Tensor<xpu, 1> ctc_workspace =
            ctx.requested[warpctc_enum::kTmp].get_space<xpu>(
                mshadow::Shape1(alloc_bytes), s);

        std::vector<float> costs(minibatch);
        throw_on_error(compute_ctc_loss(
                           activations, grads, cpu_labels, label_lengths.data(),
                           input_lengths.data(), alphabet_size, minibatch,
                           costs.data(), ctc_workspace.dptr_, info),
...
class WarpCTCProp : public OperatorProperty {
   public:
    ...
    virtual std::vector<std::string> ListOutputs() const { return {"output"}; }
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        label_shape[0] =
            param_.label_length * (dshape[0] / param_.input_length);
    ...
    std::string TypeString() const override { return "WarpCTC"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {in_data[warpctc_enum::kData], in_data[warpctc_enum::kLabel],
