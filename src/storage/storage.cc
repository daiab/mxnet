...
Storage::Handle StorageImpl::Alloc(size_t size, Context ctx) {
    ...
    storage::StorageManager *manager = device.Get(ctx.dev_id, [ctx]() {
        ...
        switch (ctx.dev_type) {
            ...
            default:
                LOG(FATAL) << "Unimplemented device " << ctx.dev_type;
        }
        return ptr;
    });
...
void StorageImpl::Free(Storage::Handle handle) {
    ...
    auto &&device = storage_managers_.at(ctx.dev_type);
    storage::StorageManager *manager = device.Get(ctx.dev_id, []() {
