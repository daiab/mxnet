...
class DropoutOp : public Operator {
   public:
    explicit DropoutOp(DropoutParam param) { this->pkeep_ = 1.0f - param.p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> data =
            in_data[dropout::kData].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> out =
            out_data[dropout::kOut].FlatTo2D<xpu, DType>(s);
        if (ctx.is_train) {
            Tensor<xpu, 2, DType> mask =
                out_data[dropout::kMask].FlatTo2D<xpu, DType>(s);
#if defined(USE_MKL) && defined(_OPENMP)
...
#pragma omp parallel for
...
#else
            Random<xpu> *prnd =
                ctx.requested[dropout::kRandom].get_random<xpu, real_t>(s);
            mask = tcast<DType>(
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> grad =
            out_grad[dropout::kOut].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> mask =
            out_data[dropout::kMask].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> gdata =
            in_grad[dropout::kData].FlatTo2D<xpu, DType>(s);
#if defined(USE_MKL) && defined(_OPENMP)
...
class DropoutProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "Dropout"; }
