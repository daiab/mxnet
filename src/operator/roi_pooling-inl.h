...
struct ROIPoolingParam : public dmlc::Parameter<ROIPoolingParam> {
    ...
    DMLC_DECLARE_PARAMETER(ROIPoolingParam) {
        ...
        DMLC_DECLARE_FIELD(spatial_scale)
            .set_range(0.0, 1.0)
            .describe(
                "Ratio of input feature map height (or w) to raw image height "
                "(or w). "
                "Equals the reciprocal of total stride in convolutional "
                "layers");
    }
};
...
class ROIPoolingOp : public Operator {
   public:
    explicit ROIPoolingOp(ROIPoolingParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        CHECK_EQ(out_data[roipool::kOut].shape_[0],
                 in_data[roipool::kBox].shape_[0]);
        CHECK_EQ(out_data[roipool::kMaxIdx].shape_[0],
                 in_data[roipool::kBox].shape_[0]);
        Stream<xpu> *s = ctx.get_stream<xpu>();

        Tensor<xpu, 4, DType> data =
            in_data[roipool::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 2, DType> bbox =
            in_data[roipool::kBox].get<xpu, 2, DType>(s);
        Tensor<xpu, 4, DType> out =
            out_data[roipool::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> max_idx =
            out_data[roipool::kMaxIdx].get<xpu, 4, DType>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad_out =
            out_grad[roipool::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 2, DType> bbox =
            in_data[roipool::kBox].get<xpu, 2, DType>(s);
        Tensor<xpu, 4, DType> max_idx =
            out_data[roipool::kMaxIdx].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> grad_in =
            in_grad[roipool::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 2, DType> grad_roi =
            in_grad[roipool::kBox].get<xpu, 2, DType>(s);
        ...
        if (kAddTo == req[roipool::kData] || kWriteTo == req[roipool::kData]) {
            ...
            ROIPoolBackwardAcc(grad_in, grad_out, bbox, max_idx,
...
class ROIPoolingProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(bshape.ndim(), 2U)
            << "bbox should be a 2D tensor of shape [batch, 5]";
        CHECK_EQ(bshape[1], 5U)
            << "bbox should be a 2D tensor of shape [batch, 5]";

        // out: [num_rois, c, pooled_h, pooled_w]
        // max_idx: [num_rois, c, pooled_h, pooled_w]
        out_shape->clear();
        out_shape->push_back(Shape4(bshape[0], dshape[1], param_.pooled_size[0],
                                    param_.pooled_size[1]));
        out_shape->push_back(Shape4(bshape[0], dshape[1], param_.pooled_size[0],
                                    param_.pooled_size[1]));
        return true;
    }
    ...
    std::string TypeString() const override { return "ROIPooling"; }

    // decalre dependency and inplace optimization options
    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[roipool::kOut], in_data[roipool::kBox],
                out_data[roipool::kMaxIdx]};
    }
