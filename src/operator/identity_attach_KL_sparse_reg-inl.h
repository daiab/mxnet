...
struct IdentityAttachKLSparseRegParam
    : public dmlc::Parameter<IdentityAttachKLSparseRegParam> {
    float penalty;
...
// The regularization is based on the KL divergence of mean activation and
// target.
// More details: P11 of https://www.cs.toronto.edu/~hinton/absps/guideTR.pdf
// Please make sure that it is only paired with sigmoid activation, otherwise
// NaN may occur.
...
class IdentityAttachKLSparseRegOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2> data =
            in_data[sparsereg::kData].FlatTo2D<xpu, real_t>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2> grad_in =
            in_grad[sparsereg::kData].FlatTo2D<xpu, real_t>(s);
        Tensor<xpu, 2> data_in =
            in_data[sparsereg::kData].FlatTo2D<xpu, real_t>(s);
        Tensor<xpu, 2> grad_out =
            out_grad[sparsereg::kOut].FlatTo2D<xpu, real_t>(s);
        Tensor<xpu, 1> moving_avg =
            aux_args[sparsereg::kMovingAvg].get<xpu, 1, real_t>(s);
        Tensor<xpu, 1> avg =
            ctx.requested[sparsereg::kTempSpace].get_space<xpu>(
        ...
        Assign(grad_in, req[sparsereg::kData],
               grad_out +
                   param_.penalty *
                       (-param_.sparseness_target /
                            broadcast<1>(moving_avg, data_in.shape_) +
