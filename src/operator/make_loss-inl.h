    ...
    DMLC_DECLARE_PARAMETER(MakeLossParam) {
        ...
        DMLC_DECLARE_FIELD(normalization)
            ...
            .describe(
                ...
                "normalized. "
                "If this is set to batch, the output gradient will be divided "
                "by the batch size. "
                "If this is set to valid, the output gradient will be divided "
                "by the number of "
   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        CHECK_EQ(in_data.size(), 1U)
            << "MakeLoss can only be used to one input";
        CHECK_EQ(out_data.size(), 1U);
        if (req[make_loss_enum::kOut] != kWriteInplace) {
            Stream<xpu> *s = ctx.get_stream<xpu>();
            Tensor<xpu, 2, DType> data =
                in_data[make_loss_enum::kData].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> out =
                out_data[make_loss_enum::kOut].FlatTo2D<xpu, DType>(s);
            Assign(out, req[make_loss_enum::kOut],
                   F<mshadow_op::identity>(data));
        }
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> grad =
            in_grad[make_loss_enum::kData].FlatTo2D<xpu, DType>(s);
        if (param_.normalization == make_loss_enum::kValid) {
            Tensor<xpu, 2, DType> data =
                in_data[make_loss_enum::kData].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 1, DType> temp =
                ctx.requested[make_loss_enum::kTempSpace]
                    .get_space_typed<xpu, 1, DType>(mshadow::Shape1(1), s);
            temp = sumall_except_dim<0>(reduce_keepdim<red::sum, false>(
                F<mshadow_op::threshold>(ScalarExp<DType>(param_.valid_thresh),
                                         data),
                0));
            temp = F<mshadow_op::maximum>(ScalarExp<DType>(1.f),
                                          temp);  // avoid zero
            Assign(grad, req[make_loss_enum::kData],
                   ScalarExp<DType>(param_.grad_scale) /
                       broadcast<0>(broadcast_keepdim(temp, 0, grad.shape_[0]),
                                    grad.shape_));
...
class MakeLossProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "MakeLoss"; }
    ...
    std::vector<std::pair<int, void *> > ForwardInplaceOption(
        ...
        return {
            {in_data[make_loss_enum::kData], out_data[make_loss_enum::kOut]}};
    }
