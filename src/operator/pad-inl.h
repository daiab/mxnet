    ...
    DMLC_DECLARE_PARAMETER(PadParam) {
        ...
        DMLC_DECLARE_FIELD(pad_width)
            .describe(
                ...
                "axis. "
                "It is a tuple of integer padding widths for each axis of the "
                "format "
                "``(before_1, after_1, ... , before_N, after_N)``. "
                "It should be of length ``2*N`` where ``N`` is the number of "
...
class PadOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        // TODO(nswamy@): update the documentation and log below when support is
        // added for more than
        ...
        switch (rank) {
            case 4: {
                ...
                pad_image(out, data, param_.pad_width, param_.mode,
                          constant_value);
                break;
            }
            case 5: {
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if ((pad[0] != 0) || (pad[1] != 0) || (pad[2] != 0) || (pad[3] != 0)) {
            LOG(FATAL) << "Current implementation expects padding on the first "
                          "two axes to be zero.";
        }
        ...
        if (param_.mode == pad_enum::kReflect) {
            ...
            if ((pad[4] >= size[2]) || (pad[5] >= size[2]) ||
                ...
                LOG(FATAL) << "Current implementation of reflection padding "
                              "only supports padding sizes smaller than the "
                              "input size.";
            }
        }
        TShape oshape = dshape;
        for (size_t i = 0; i < dshape.ndim(); ++i) {
            oshape[i] = param_.pad_width[2 * i] + param_.pad_width[2 * i + 1] +
                        dshape[i];
        }
        out_shape->clear();
