 ...
 * \file sampling_op.h
 * \brief Function definitions of operators for sampling from multiple
 * distributions
 */
...
inline bool MultiSampleOpShape(const nnvm::NodeAttrs& attrs,
    ...
    CHECK_GT(in_attrs->size(), 0)
        << "sampling operator takes 1 or 2 arguments (" << in_attrs->size()
        << " given)";
    ...
    for (size_t i = 0; i < sshape.ndim(); ++i) {
        CHECK_GT(sshape[i], 0)
            << "shape parameter must be non-zero within each dimension";
    }
    ...
    if (tshape.ndim() > sshape.ndim()) {
        ...
        tshape = TShape(tshape.begin(),
                        tshape.begin() + (tshape.ndim() - sshape.ndim()));
    }
...
inline bool MultiSampleOpType(const nnvm::NodeAttrs& attrs,
    ...
    if (dtype != -1) {
        if (param.dtype != -1) {
            ...
            CHECK_EQ(dtype, param.dtype)
                << "Inferred output type does not match requested type: "
    ...
    CHECK_EQ(dtype_ok, true)
        ...
        << dtype << " vs " << mshadow::kFloat16 << " or " << mshadow::kFloat32
        << " or " << mshadow::kFloat64;
...
void MultiSampleOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
    ...
    // behaviour for single threaded cpu, this is taken from mshadow random
    // generator.
    ...
    MSHADOW_TYPE_SWITCH(in0.type_flag_, IType, {
        MSHADOW_REAL_TYPE_SWITCH(out.type_flag_, OType, {
            MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
                ...
                Tensor<xpu, 2, OType> samples =
                    out.get_with_shape<xpu, 2, OType>(Shape2(N, M), s);
                const IType *iptr1 = in0.dptr<IType>(),
                            *iptr2 = in1.dptr<IType>();

                // The seeds for the different generators are itself a random
                // sequence. We don't
                // want to create the same samples in case that we have two
                // samplers with same
                // input parameters.
                std::mt19937 seed_generator(seed);
                for (int i = 0; i < N; ++i) {
                    // Generate seed for this sampler. Must be mutexed as
                    // calling
                    // a random generator is not thread safe.
                    int seed = seed_generator();
                    typename generator::template Sampler<OType> sampler(
                        iptr1[i], iptr2[i], seed);
                    // Get the sub-tensor that will hold the results of this
                    // sampler.
                    Tensor<xpu, 1, OType> slice =
                        samples.Slice(i, i + 1).FlatTo1D();
                    for (int j = 0; j < M; ++j) {
