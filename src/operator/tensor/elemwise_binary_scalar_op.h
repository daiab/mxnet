...
void BinaryScalarCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
...
template <typename xpu, typename OP>
void BinaryScalarBackward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                          const std::vector<TBlob>& inputs,
                          const std::vector<OpReqType>& req,
                          const std::vector<TBlob>& outputs) {
    using namespace mshadow;
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {
        ...
        ASSIGN_DISPATCH(igrad, req[0],
                        ograd * F<OP>(lhs, scalar<DType>(DType(alpha))));
    });
}
