 ...
 * \brief CPU/GPU: Gradient accumulate of embedding matrix.
                   ...
                   Called when the batchsize of src is larger than the
 featuredim
...
inline void AddTakeGradLargeBatch(
...
template <typename IndexType, typename DType>
inline void AddTakeGradLargeBatch(
    mshadow::Tensor<gpu, 2, DType> dst,
...
inline bool EmbeddingOpType(const nnvm::NodeAttrs& attrs,
    ...
    if (dtype_in != -1 && dtype_out != -1) {
        ...
        CHECK_EQ(dtype_in, dtype_out)
            << "Input and output weights must have same type";
        dtype = dtype_in;
...
struct Take {
    ...
    MSHADOW_XINLINE static void Map(int i, DType* out_data,
                                    const DType* in_data, const IType* idx,
...
void EmbeddingOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
...
struct tcast_clip {
    ...
    MSHADOW_XINLINE static void Map(int i, OType* out_data,
                                    const IType* in_data, const OType K) {
...
void AddTakeGradLargeBatchCaller(
    const OpContext& ctx, mshadow::Tensor<xpu, 2, DType> dst,
    ...
    size_t sort_workspace_size =
        mxnet::op::SortByKeyWorkspaceSize<int, int, xpu>(index.shape_.Size());
    size_t addtake_workspace_size =
        mxnet::op::AddTakeGradLargeBatchWorkspaceSize<int, xpu>(
            index.shape_.Size());
    size_t temp_storage_size =
        std::max(sort_workspace_size, addtake_workspace_size);
    size_t workspace_size =
        2 * (index.shape_.Size() * sizeof(int)) + temp_storage_size;
    ...
    Tensor<xpu, 1, int> original_index(reinterpret_cast<int*>(&workspace[pos]),
                                       Shape1(index.shape_.Size()), s);
    pos += index.shape_.Size() * sizeof(int);
    Tensor<xpu, 1, char> temp_storage(&workspace[pos],
                                      Shape1(temp_storage_size), s);
    Kernel<tcast_clip, xpu>::Launch(s, index.shape_.Size(), sorted_data.dptr_,
                                    index.dptr_,
                                    static_cast<int>(dst.shape_[0]));
    original_index = range<int>(0, index.shape_.Size());
    int num_bits = ilog2((dst.shape_[0] - 1));
    mxnet::op::SortByKey(sorted_data, original_index, true, &temp_storage, 0,
                         num_bits);
    mxnet::op::AddTakeGradLargeBatch(dst, sorted_data, original_index, src,
                                     &temp_storage);
}

template <typename xpu>
void EmbeddingOpBackward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                         const std::vector<TBlob>& inputs,
                         const std::vector<OpReqType>& req,
                         const std::vector<TBlob>& outputs) {
    using namespace mshadow;
    ...
    MSHADOW_TYPE_SWITCH(outputs[1].type_flag_, DType, {
        MSHADOW_TYPE_SWITCH(inputs[1].type_flag_, IType, {
            Tensor<xpu, 1, IType> data =
                inputs[1].get_with_shape<xpu, 1, IType>(
                    Shape1(ishape.ProdShape(0, ishape.ndim())), s);
            Tensor<xpu, 2, DType> grad_out =
                inputs[0].get_with_shape<xpu, 2, DType>(
                    Shape2(oshape.ProdShape(0, oshape.ndim() - 1),
                           oshape[oshape.ndim() - 1]),
                    s);
            ...
            if (req[embedding::kWeight] == kWriteTo ||
                req[embedding::kWeight] == kAddTo) {
                if (req[embedding::kWeight] == kWriteTo) {
                    grad_in = scalar<DType>(0.0f);
                }
                // shape_out_prod ~= the number of elements loaded in
                // AddTakeGrad
                // shape_in_prod  ~= the number of elements stored in
                // AddTakeGrad
                // When the number of elements processed is low, use
                // AddTakeGrad.
                // The approximate cut-off value 16384 was found experimentally
                // on Titan X Pascal
                ...
                if (shape_out_prod < (uint64_t)16384 &&
    ...
    DMLC_DECLARE_PARAMETER(TakeParam) {
        ...
        DMLC_DECLARE_FIELD(mode)
            ...
            .describe(
                ...
                " \"clip\" means clip to the range. So, if all indices "
                "mentioned are too large,"
...
inline bool TakeOpType(const nnvm::NodeAttrs& attrs, std::vector<int>* in_attrs,
...
template <typename xpu>
void TakeOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                   const std::vector<TBlob>& inputs,
                   const std::vector<OpReqType>& req,
                   const std::vector<TBlob>& outputs) {
    using namespace mxnet_op;
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {     // output data type
        MSHADOW_TYPE_SWITCH(inputs[1].type_flag_, IType, {  // index data type
            Kernel<Take, xpu>::Launch(
                s, oshape.Size(), outputs[take_::kOut].dptr<DType>(),
...
template <typename xpu>
void TakeOpBackward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                    const std::vector<TBlob>& inputs,
                    const std::vector<OpReqType>& req,
                    const std::vector<TBlob>& outputs) {
    using namespace mshadow;
            ...
            Tensor<xpu, 2, DType> grad_out =
                inputs[0].get_with_shape<xpu, 2, DType>(
                    Shape2(oshape.ProdShape(0, idxndim),
                           oshape.ProdShape(idxndim, oshape.ndim())),
                    s);
            Tensor<xpu, 2, DType> grad_in =
                outputs[0].get_with_shape<xpu, 2, DType>(
                    Shape2(arrshape[0], arrshape.ProdShape(1, arrshape.ndim())),
                    s);
...
inline bool BatchTakeOpShape(const nnvm::NodeAttrs& attrs,
    ...
    CHECK_GE((*in_attrs)[0].ndim(), 2U)
        << "Data array must have at least 2 dimensional";
    if ((*out_attrs)[0].ndim() == 0) return false;
    CHECK_EQ((*in_attrs)[0].Size() / (*in_attrs)[0][(*in_attrs)[0].ndim() - 1],
             (*out_attrs)[0].Size())
        << "Index array's size must be the same as data array's size excluding "
           "the first dimension";
    return true;
}
...
template <typename xpu>
void BatchTakeOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                        const std::vector<TBlob>& inputs,
                        const std::vector<OpReqType>& req,
                        const std::vector<TBlob>& outputs) {
    CHECK_EQ(inputs.size(), 2U);
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {
        MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {
            Kernel<batch_take<req_type>, xpu>::Launch(
...
inline void GetOneHotParams(const OneHotParam& param, int* depth,
                            double* on_value, double* off_value, int* dtype) {
    *depth = param.depth;
    CHECK_GE(*depth, 0)
        << "Dimension size, depth, must be a non-negative integer";
...
inline bool OneHotOpType(const nnvm::NodeAttrs& attrs,
    ...
    CHECK_NE((*in_attrs)[0], -1)
        << "Index type must be set for one_hot operator";
    int depth = 0;
    double on_value = 1.0;
    double off_value = 0.0;
    int dtype = -1;
...
template <typename xpu>
void OneHotOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                     const std::vector<TBlob>& inputs,
                     const std::vector<OpReqType>& req,
                     const std::vector<TBlob>& outputs) {
    ...
    mshadow::Stream<xpu>* s = ctx.get_stream<xpu>();
    MSHADOW_TYPE_SWITCH(
        outputs[0].type_flag_, DType, {  // output data type switch
            mshadow::Tensor<xpu, 1, DType> out =
                outputs[0].FlatTo1D<xpu, DType>(s);
            ASSIGN_DISPATCH(out, req[0], static_cast<DType>(off_value));
            MXNET_ASSIGN_REQ_SWITCH(req[0], req_type, {  // request type switch
                MSHADOW_TYPE_SWITCH(
                    inputs[0].type_flag_, IType, {  // indices data type switch
                        Kernel<one_hot<req_type>, xpu>::Launch(
