    ...
    DMLC_DECLARE_PARAMETER(BroadcastToParam) {
        DMLC_DECLARE_FIELD(shape)
            ...
            .describe(
                ...
                "has the same meaning as `A = broadcast_axis(B, axis=0, "
                "size=10)`.");
    }
};
...
inline TShape ReduceAxisShapeImpl(const ReduceAxisParam& param,
                                  const TShape& ishape) {
...
inline bool BroadcastAxesShape(const nnvm::NodeAttrs& attrs,
    ...
    for (index_t i = 0; i < param.axis.ndim(); ++i) {
        CHECK_EQ(oshape[param.axis[i]], 1U)
...
inline bool BroadcastToShape(const nnvm::NodeAttrs& attrs,
    ...
    CHECK_EQ(ishape.ndim(), param.shape.ndim()) << "Operand of shape " << ishape
    ...
    for (index_t i = 0; i < ishape.ndim(); ++i) {
        if (oshape[i] != 0) {
            CHECK(ishape[i] == oshape[i] || ishape[i] == 1)
                << "Array cannot be broadcasted from " << ishape << " to "
                << param.shape;
        } else {
            oshape[i] = ishape[i];
        }
...
inline void BroadcastReduceShapeCompact(const TShape& big, const TShape& small,
    ...
    *new_small = TShape(idim);
    ...
    if (j <= 2) {
        ...
        new_small->assign(&(*new_small)[0],
                          &(*new_small)[MXNET_SPECIAL_MAX_NDIM]);
...
void SearchAxisCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
    ...
    MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
        ...
        Tensor<xpu, 3, DType> in =
            inputs[0].get_with_shape<xpu, 3, DType>(shape.get<3>(), s);
...
void ReduceAxesComputeImpl(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                           const std::vector<TBlob>& inputs,
                           const std::vector<OpReqType>& req,
                           const std::vector<TBlob>& outputs,
                           const TShape& small) {
    ...
    BroadcastReduceShapeCompact(inputs[0].shape_, small, &src_shape,
                                &dst_shape);
    Stream<xpu>* s = ctx.get_stream<xpu>();
...
template <typename xpu, typename reducer, bool normalize = false>
void ReduceAxesCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                       const std::vector<TBlob>& inputs,
                       const std::vector<OpReqType>& req,
                       const std::vector<TBlob>& outputs) {
    const ReduceAxesParam& param = nnvm::get<ReduceAxesParam>(attrs.parsed);
        ...
        if (dst_shape.ndim() == 2) {
            ...
            ASSIGN_DISPATCH(igrad, req[0],
                            broadcast_to(ograd, src_shape) *
                                F<OP>(data, broadcast_to(out, src_shape)));
            if (normalize)
                igrad /= scalar<DType>(src_shape.Size() / dst_shape.Size());
        } else {
            const int ndim = MXNET_SPECIAL_MAX_NDIM;
            Tensor<xpu, ndim, DType> igrad =
                outputs[0].get_with_shape<xpu, ndim, DType>(
                    src_shape.get<ndim>(), s);
            Tensor<xpu, ndim, DType> ograd =
                inputs[0].get_with_shape<xpu, ndim, DType>(
                    dst_shape.get<ndim>(), s);
            Tensor<xpu, ndim, DType> data =
                inputs[1].get_with_shape<xpu, ndim, DType>(
                    src_shape.get<ndim>(), s);
            Tensor<xpu, ndim, DType> out =
                inputs[2].get_with_shape<xpu, ndim, DType>(
                    dst_shape.get<ndim>(), s);
            ASSIGN_DISPATCH(igrad, req[0],
                            broadcast_to(ograd, src_shape) *
                                F<OP>(data, broadcast_to(out, src_shape)));
...
template <typename xpu>
inline void BroadcastCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                             const std::vector<TBlob>& inputs,
                             const std::vector<OpReqType>& req,
                             const std::vector<TBlob>& outputs) {
    BroadcastComputeImpl<xpu>(attrs, ctx, inputs, req, outputs,
                              inputs[0].shape_);
}
...
struct ReduceGrad {
    ...
    std::vector<nnvm::NodeEntry> operator()(
        ...
        return MakeNonlossGradNode(op_name, n, ograds,
                                   {n->inputs[0], nnvm::NodeEntry{n, 0, 0}},
                                   n->attrs.dict);
    }
};

template <typename xpu>
void L2NormCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                   const std::vector<TBlob>& inputs,
                   const std::vector<OpReqType>& req,
                   const std::vector<TBlob>& outputs) {
    ...
    MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
        ...
        mshadow::Tensor<xpu, 1, DType> in =
            inputs[0].get_with_shape<xpu, 1, DType>(
                mshadow::Shape1(inputs[0].shape_.Size()), s);
        mshadow::VectorDot(out, in, in);
        ASSIGN_DISPATCH(
            out, req[0],
            mshadow::expr::F<mxnet::op::mshadow_op::square_root>(out));
    });
}
...
inline bool PickOpType(const nnvm::NodeAttrs& attrs, std::vector<int>* in_attrs,
...
template <typename xpu>
void PickOpForward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                   const std::vector<TBlob>& inputs,
                   const std::vector<OpReqType>& req,
                   const std::vector<TBlob>& outputs) {
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {     // output type
        MSHADOW_TYPE_SWITCH(inputs[1].type_flag_, IType, {  // index type
            if (trailing == 1) {
                Kernel<pick<2>, xpu>::Launch(
                    s, outputs[0].Size(), outputs[0].dptr<DType>(),
                    inputs[0].dptr<DType>(), inputs[1].dptr<IType>(), M, 1,
                    Shape2(leading, M), Shape2(leading, 1));
            } else {
                Kernel<pick<3>, xpu>::Launch(
                    s, outputs[0].Size(), outputs[0].dptr<DType>(),
                    inputs[0].dptr<DType>(), inputs[1].dptr<IType>(), M,
...
template <typename xpu>
void PickOpBackward(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                    const std::vector<TBlob>& inputs,
                    const std::vector<OpReqType>& req,
                    const std::vector<TBlob>& outputs) {
    using namespace mxnet_op;
    ...
    MSHADOW_TYPE_SWITCH(outputs[0].type_flag_, DType, {     // output type
        MSHADOW_TYPE_SWITCH(inputs[1].type_flag_, IType, {  // index type
            ...
            if (trailing == 1) {
                Kernel<pick_grad<2>, xpu>::Launch(
                    s, inputs[0].Size(), outputs[0].dptr<DType>(),
                    inputs[0].dptr<DType>(), inputs[1].dptr<IType>(), M, 1,
                    Shape2(leading, M), Shape2(leading, 1));
            } else {
                Kernel<pick_grad<3>, xpu>::Launch(
                    s, inputs[0].Size(), outputs[0].dptr<DType>(),
                    inputs[0].dptr<DType>(), inputs[1].dptr<IType>(), M,
                    trailing, Shape3(leading, M, trailing),
                    Shape3(leading, 1, trailing));
