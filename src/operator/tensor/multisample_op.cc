...
struct NormalSampler {
    ...
    struct Sampler {
        ...
        template <typename PType>
        Sampler(PType mu, PType sigma, int seed)
            : rnd_(seed), gen_(mu, sigma) {}
        MSHADOW_XINLINE DType operator()() { return gen_(rnd_); }
    };
};
...
struct GammaSampler {
    ...
    struct Sampler {
        std::mt19937 rnd_;
        // Avoid problems with static check during compilation for integral
        // types.
        typedef typename std::conditional<std::is_floating_point<DType>::value,
                                          DType, double>::type GType;
        std::gamma_distribution<GType> gen_;
        template <typename PType>
        Sampler(PType alpha, PType beta, int seed)
            : rnd_(seed), gen_(alpha, beta) {}
        MSHADOW_XINLINE DType operator()() { return gen_(rnd_); }
    };
};
...
struct PoissonSampler {
    ...
    struct Sampler {
        std::mt19937 rnd_;
        // Allow sampling of a Poisson distribution also to output floating
        // point types.
        ...
        template <typename PType>
        Sampler(PType lambda, PType, int seed) : rnd_(seed), gen_(lambda) {}
        MSHADOW_XINLINE DType operator()() {
            return static_cast<DType>(gen_(rnd_));
        }
    };
};
...
struct NegativeBinomialSampler {
    ...
    struct Sampler {
        std::mt19937 rnd_;
        // Allow sampling of a negative binomial distribution also to output
...
struct GeneralizedNegativeBinomialSampler {
    template <typename DType>
    struct Sampler {
        // We allow the boundary case where the negative binomial equals the
        // Poisson distribution
        bool poisson_;
        double mu_;
        std::mt19937 rnd_;
        // Realize the negative binomial by a Poisson distribution over a gamma
        // distributed mean.
        std::gamma_distribution<> gen_;
        template <typename PType>
        Sampler(PType mu, PType alpha, int seed)
            : poisson_(alpha == 0.0),
              mu_(mu),
              rnd_(seed),
              gen_((alpha == PType(0) ? PType(1) : PType(1) / alpha),
                   mu * alpha) {}
        // Allow sampling of a Poisson distribution also to output floating
        // point types.
        typedef typename std::conditional<std::is_integral<DType>::value, DType,
                                          int>::type GType;
        MSHADOW_XINLINE DType operator()() {
            return static_cast<DType>(std::poisson_distribution<GType>(
                poisson_ ? mu_ : gen_(rnd_))(rnd_));
        }
    };
};
        ...
        .set_attr<FResourceRequest>("FResourceRequest",                      \
                                    [](const NodeAttrs& attrs) {             \
                                        return std::vector<ResourceRequest>( \
                                            1, ResourceRequest::kRandom);    \
                                    })                                       \
        .set_attr<FCompute>("FCompute<cpu>",                                 \
                            MultiSampleOpForward<cpu, sampler>)              \
        .set_attr<nnvm::FGradient>("FGradient", MakeZeroGradNodes)           \
        .add_arguments(MultiSampleParam::__FIELDS__())                       \
        .add_argument(input_name_1, "NDArray-or-Symbol", input_desc_1)

#define MXNET_OPERATOR_REGISTER_SAMPLING1(distr, sampler, input_name,    \
                                          input_desc, description)       \
    MXNET_OPERATOR_REGISTER_SAMPLING(distr, sampler, 1, input_name,      \
                                     input_name, input_desc, input_desc, \
                                     description);

#define MXNET_OPERATOR_REGISTER_SAMPLING2(distr, sampler, input_name_1,        \
                                          input_name_2, input_desc_1,          \
                                          input_desc_2, description)           \
    MXNET_OPERATOR_REGISTER_SAMPLING(distr, sampler, 2, input_name_1,          \
                                     input_name_2, input_desc_1, input_desc_2, \
                                     description)                              \
...
MXNET_OPERATOR_REGISTER_SAMPLING2(normal, NormalSampler, "mu", "sigma",
                                  ...
                                  normal_desc)
MXNET_OPERATOR_REGISTER_SAMPLING2(
    gamma, GammaSampler, "alpha", "beta",
    "Alpha (shape) parameters of the distributions.",
    "Beta (scale) parameters of the distributions.", gamma_desc)
MXNET_OPERATOR_REGISTER_SAMPLING1(
    exponential, ExponentialSampler, "lam",
    "Lambda (rate) parameters of the distributions.", exponential_desc)
MXNET_OPERATOR_REGISTER_SAMPLING1(
    poisson, PoissonSampler, "lam",
    "Lambda (rate) parameters of the distributions.", poisson_desc)
MXNET_OPERATOR_REGISTER_SAMPLING2(negative_binomial, NegativeBinomialSampler,
                                  "k", "p",
                                  "Limits of unsuccessful experiments.",
                                  "Failure probabilities in each experiment.",
                                  negative_binomial_desc)
MXNET_OPERATOR_REGISTER_SAMPLING2(
    generalized_negative_binomial, GeneralizedNegativeBinomialSampler, "mu",
    "alpha", "Means of the distributions.",
