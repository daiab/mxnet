...
inline Tensor<xpu, dst_dim, DType> inplace_reshape(
    Tensor<xpu, src_dim, DType> src, Shape<dst_dim> target_shape) {
    ...
    DMLC_DECLARE_PARAMETER(TopKParam) {
        ...
        DMLC_DECLARE_FIELD(k)
            ...
            .describe(
                ...
                " should be always smaller than or equal to the element number "
                "in the given axis."
        ...
        DMLC_DECLARE_FIELD(ret_typ)
            ...
            .describe(
                ...
                " \"mask\" means to return a mask array containing 0 and 1. 1 "
                "means the top k values."
                " \"both\" means to return a list of both values and indices "
                "of top k elements.");
        DMLC_DECLARE_FIELD(is_ascend)
            .set_default(false)
            .describe(
...
inline void ParseTopKParam(const TShape& src_shape, const TopKParam& param,
                           TShape* target_shape, int* batch_size,
                           int* element_num, int* axis, int* k,
...
void TopKImpl(RunContext ctx, Resource resource, const TBlob& src,
              const std::vector<TBlob>& ret, const TopKParam& param) {
    ...
    int k = 0;
    TShape target_shape;
    ParseTopKParam(src.shape_, param, &target_shape, &batch_size, &element_num,
                   &axis, &k, &do_transpose, &is_ascend);
    Tensor<xpu, 3, real_t> dat = src.FlatTo3D<xpu, real_t>(axis, axis, s);
    if (param.ret_typ == topk_enum::kReturnMask) {
        workspace = resource.get_space_typed<xpu, 1, real_t>(
            Shape1(src.Size() * 3 + 2 * batch_size * k), s);
    } else {
        workspace = resource.get_space_typed<xpu, 1, real_t>(
            mshadow::Shape1(src.Size() * 3), s);
    }
    sorted_dat = Tensor<xpu, 1, real_t>(workspace.dptr_, Shape1(src.Size()),
                                        s);  // contain sorted dat
    indices =
        Tensor<xpu, 1, real_t>(workspace.dptr_ + src.Size(), Shape1(src.Size()),
                               s);  // indices in the original matrix
    batch_id = Tensor<xpu, 1, real_t>(workspace.dptr_ + 2 * src.Size(),
                                      Shape1(src.Size()),
                                      s);  // batch id in the original matrix
    if (do_transpose) {
        sorted_dat =
            reshape(transpose(dat, Shape3(0, 2, 1)), Shape1(src.Size()));
    } else {
    ...
    if (param.ret_typ == topk_enum::kReturnMask) {
        ...
        mask_val = Tensor<xpu, 2, real_t>(
            workspace.dptr_ + 3 * src.Size() + batch_size * k,
    ...
    if (param.ret_typ == topk_enum::kReturnMask) {
        ...
        if (do_transpose) {
            ...
            sel_indices = transpose_indices(
                sel_indices, Shape3(src_shape[0], src_shape[2], src_shape[1]),
                Shape3(0, 2, 1));
        }
        ...
        if (do_transpose) {
            Tensor<xpu, 3, real_t> ret_indices =
                ret[0].FlatTo3D<xpu, real_t>(axis, axis, s);
            ret_indices = transpose(
                slice<2>(inplace_reshape(indices, Shape3(ret_indices.shape_[0],
            ...
            ret_indices = slice<1>(
                inplace_reshape(indices, Shape2(batch_size, element_num)), 0,
                k);
        }
    } else {
        indices -= batch_id * static_cast<real_t>(element_num);
        if (do_transpose) {
            Tensor<xpu, 3, real_t> ret_value =
                ret[0].FlatTo3D<xpu, real_t>(axis, axis, s);
            Tensor<xpu, 3, real_t> ret_indices =
                ret[1].FlatTo3D<xpu, real_t>(axis, axis, s);
            ret_value = transpose(
                slice<2>(inplace_reshape(sorted_dat, Shape3(ret_value.shape_[0],
                                                            ret_value.shape_[2],
                                                            element_num)),
                         0, k),
                Shape3(0, 2, 1));
            ...
            ret_value = slice<1>(
                inplace_reshape(sorted_dat, Shape2(batch_size, element_num)), 0,
                k);
            ret_indices = slice<1>(
                inplace_reshape(indices, Shape2(batch_size, element_num)), 0,
                k);
...
template <typename xpu>
void TopK(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
...
template <typename xpu>
void Sort(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
          const std::vector<TBlob>& inputs, const std::vector<OpReqType>& req,
          const std::vector<TBlob>& outputs) {
    ...
    TopKImpl<xpu>(ctx.run_ctx, ctx.requested[0], inputs[0], outputs,
                  topk_param);
}

template <typename xpu>
void ArgSort(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
...
template <typename xpu>
void TopKBackward_(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                   const std::vector<TBlob>& inputs,
                   const std::vector<OpReqType>& req,
                   const std::vector<TBlob>& outputs) {
    ...
    const TopKParam& param = nnvm::get<TopKParam>(attrs.parsed);
    CHECK(param.ret_typ == topk_enum::kReturnValue ||
    ...
    int k = 0;
    TShape target_shape;
    ParseTopKParam(outputs[0].shape_, param, &target_shape, &batch_size,
                   &element_num, &axis, &k, &do_transpose, &is_ascend);
    Tensor<xpu, 1, real_t> workspace =
        ctx.requested[0].get_space_typed<xpu, 1, real_t>(
            Shape1(batch_size * k * 2 + batch_size), s);
    Tensor<xpu, 1, real_t> sel_indices =
        Tensor<xpu, 1, real_t>(workspace.dptr_, Shape1(batch_size * k), s);
    Tensor<xpu, 1, real_t> batch_shift = Tensor<xpu, 1, real_t>(
        workspace.dptr_ + batch_size * k, Shape1(batch_size), s);
    Tensor<xpu, 1, real_t> dummy_index =
        Tensor<xpu, 1, real_t>(workspace.dptr_ + batch_size * k + batch_size,
                               Shape1(batch_size * k), s);
    Tensor<xpu, 2, real_t> out_grad = inputs[0].get_with_shape<xpu, 2, real_t>(
        Shape2(inputs[0].shape_.Size(), 1), s);
    Tensor<xpu, 2, real_t> in_grad = outputs[0].get_with_shape<xpu, 2, real_t>(
        Shape2(outputs[0].shape_.Size(), 1), s);
    ...
    if (do_transpose) {
        ...
        sel_indices = reshape(
            transpose(broadcast_to(
                          inplace_reshape(batch_shift, Shape3(src_shape[0],
                                                              src_shape[2], 1)),
        ...
        sel_indices = reshape(
            indices + broadcast_to(
                          inplace_reshape(batch_shift, Shape2(batch_size, 1)),
    ...
    if (kWriteTo == req[0]) {
        ...
        // However, the current implementation of AddTakeGrad is not so
        // efficient.
        dummy_index = range<real_t>(0, sel_indices.shape_.Size());
        mxnet::op::AddTakeGradLargeBatch(in_grad, sel_indices, dummy_index,
                                         out_grad);
...
inline bool TopKType(const nnvm::NodeAttrs& attrs, std::vector<int>* in_attrs,
                     std::vector<int>* out_attrs) {
    return ElemwiseAttr<int, type_is_none, type_assign, true, type_string>(
        attrs, in_attrs, out_attrs, -1);
}

inline bool TopKShapeImpl(const TopKParam& param, std::vector<TShape>* in_attrs,
    ...
    int k = 0;
    TShape target_shape;
    ParseTopKParam(in_shape, param, &target_shape, &batch_size, &element_num,
                   &axis, &k, &do_transpose, &is_ascend);
    if (param.ret_typ == topk_enum::kReturnIndices ||
        param.ret_typ == topk_enum::kReturnMask) {
