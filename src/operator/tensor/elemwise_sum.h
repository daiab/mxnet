...
struct Sum {
    ...
    template <typename DType, typename... DTypes>
    MSHADOW_XINLINE static void Map(int i, DType* out, const OpReqType req,
                                    const DType* in0, const DTypes... ins) {
        KERNEL_ASSIGN(out[i], req, sum(i, in0, ins...));
    }
};

template <typename xpu, typename DType>
void ElementWiseSumCompute_(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
    ...
    switch (size) {
        ...
        default: {
            ...
            for (size_t i = 1; i < size; ++i) {
                ...
                Kernel<Sum, xpu>::Launch(s, out_size, out_dptr, req[0],
                                         out_dptr, in_dptr);
            }
            break;
        }
...
void ElementWiseSumCompute(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
