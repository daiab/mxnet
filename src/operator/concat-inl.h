...
class ConcatOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        for (int i = dimension_ + 1; i < out_data[concat_enum::kOut].ndim();
             ++i) {
        ...
        out = out_data[concat_enum::kOut].get_with_shape<xpu, 3, DType>(oshape,
                                                                        s);

        for (int i = 0; i < size_; ++i) {
            Shape<3> dshape =
                Shape3(leading, in_data[i].shape_[dimension_], trailing);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        for (int i = dimension_ + 1; i < out_grad[concat_enum::kOut].ndim();
             ++i) {
            trailing *= out_grad[concat_enum::kOut].shape_[i];
        }
        size_t mid = out_grad[concat_enum::kOut].shape_[dimension_];
        Shape<3> oshape = Shape3(leading, mid, trailing);
        grad = out_grad[concat_enum::kOut].get_with_shape<xpu, 3, DType>(oshape,
                                                                         s);

        for (int i = 0; i < size_; ++i) {
            Shape<3> dshape =
                Shape3(leading, in_grad[i].shape_[dimension_], trailing);
...
class ConcatProp : public OperatorProperty {
   public:
        ...
        for (int i = 0; i < param_.num_args; ++i) {
            ...
            if (tmp.ndim()) {
                CHECK_LT(static_cast<index_t>(param_.dim), tmp.ndim())
                    << "concat dim " << param_.dim
                    << " out of range of input shape " << tmp;
    ...
    std::string TypeString() const override { return "Concat"; }
