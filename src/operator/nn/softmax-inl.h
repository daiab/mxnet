...
inline void Softmax(Stream<cpu> *s, DType *in, DType *out, Shape<ndim> shape,
                    int axis) {
...
inline void SoftmaxGrad(Stream<cpu> *s, DType *out, DType *ograd, DType *igrad,
...
__global__ void softmax_compute_kernel(DType *in, DType *out, index_t M,
                                       int axis, Shape<ndim> sshape,
                                       Shape<ndim> stride) {
    ...
    for (index_t i = x; i < M; i += x_size) {
        red::sum::Reduce(smem[x],
                         static_cast<DType>(expf(in[base + i * sa] - smax)));
    }
    __syncthreads();
...
template <typename OP, typename DType, int ndim>
inline void Softmax(Stream<gpu> *s, DType *in, DType *out, Shape<ndim> shape,
                    int axis) {
    const int x_bits = 7;
    ...
    softmax_compute_kernel<
        x_bits, OP, DType,
        ndim><<<N, x_size, 0, mshadow::Stream<gpu>::GetStream(s)>>>(
...
__global__ void softmax_gradient_kernel(DType *out, DType *ograd, DType *igrad,
    ...
    for (index_t i = x; i < M; i += x_size) {
        red::sum::Reduce(smem[x],
                         OP1::Map(ograd[base + i * sa], out[base + i * sa]));
    }
    __syncthreads();
    cuda::Reduce1D<red::sum, x_bits>(smem);
    __syncthreads();
...
template <typename OP1, typename OP2, typename DType, int ndim>
inline void SoftmaxGrad(Stream<gpu> *s, DType *out, DType *ograd, DType *igrad,
                        Shape<ndim> shape, int axis) {
    const int x_bits = 7;
    const int x_size = 1 << x_bits;
    index_t M = shape[axis];
...
void SoftmaxCompute(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
...
void SoftmaxGradCompute(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
                        const std::vector<TBlob> &inputs,
                        const std::vector<OpReqType> &req,
                        const std::vector<TBlob> &outputs) {
    using namespace mxnet_op;
