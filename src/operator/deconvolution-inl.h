...
struct DeconvolutionParam : public dmlc::Parameter<DeconvolutionParam> {
    ...
    DMLC_DECLARE_PARAMETER(DeconvolutionParam) {
        DMLC_DECLARE_FIELD(kernel)
            .describe(
                "Deconvolution kernel size: (h, w) or (d, h, w). "
                "This is same as the kernel size used for the corresponding "
                "convolution");
        DMLC_DECLARE_FIELD(stride)
            .set_default(TShape())
            .describe(
                "The stride used for the corresponding convolution: (h, w) or "
                "(d, h, w).");
        DMLC_DECLARE_FIELD(dilate)
            .set_default(TShape())
            .describe(
                "Dilation factor for each dimension of the input: (h, w) or "
                "(d, h, w).");
        DMLC_DECLARE_FIELD(pad)
            .set_default(TShape())
            .describe(
                "The amount of implicit zero padding added during convolution "
                "for each "
                "dimension of the input: "
                "(h, w) or (d, h, w). "
                "``(kernel-1)/2`` is usually a good choice. "
                "If `target_shape` is set, "
                "`pad` will be ignored and a padding that will generate the "
                "target shape "
                "will be used.");
        DMLC_DECLARE_FIELD(adj)
            .set_default(TShape())
            .describe(
                "Adjustment for output shape: (h, w) or (d, h, w). "
                "If `target_shape` is set, "
        ...
        DMLC_DECLARE_FIELD(cudnn_tune)
            ...
            .describe(
                "Whether to pick convolution algorithm by running performance "
                "test.");
    ...
    void InferPad(TShape input, index_t (&o_pad)[ndim],
                  index_t (&o_adj)[ndim]) const {
        if (target_shape.ndim() != 0) {
            size_t input_ndim = input.ndim();

            for (unsigned int i = 0; i < ndim; i++) {
                // input.ndim() can be larger than ndim, in case that the
                // complete input
                // shape was passed and not only the ndim last ones
                o_pad[i] = stride[i] * (input[(input_ndim - ndim) + i] - 1) +
                           DilatedKernelSize(i);
...
class DeconvolutionOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (param_.kernel.ndim() != 2) {
            LOG(FATAL)
                << "If not using CUDNN only 2D-Deconvolution is supported";
        }
        ...
        Tensor<xpu, 4, DType> data =
            in_data[deconv::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> out =
            out_data[deconv::kOut].get<xpu, 4, DType>(s);
        ...
        Shape<3> wmat_shape =
            Shape3(param_.num_group, data.shape_[1] / param_.num_group,
                   param_.num_filter / param_.num_group * param_.kernel[0] *
                       param_.kernel[1]);
        Tensor<xpu, 3, DType> wmat =
            in_data[deconv::kWeight].get_with_shape<xpu, 3, DType>(wmat_shape,
                                                                   s);
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            Tensor<xpu, 3, DType> temp_dst = Tensor<xpu, 3, DType>(
                ...
                Shape3(shape_dstunit_[0], shape_dstunit_[1],
            ...
            if (o_pad[0] == 0 && o_pad[1] == 0) {
                temp_col = unpack_patch2col(out.Slice(i, i + step),
                                            ...
                                            param_.dilate[0], param_.dilate[1]);
            } else {
                temp_col = unpack_patch2col(
                    pad(out.Slice(i, i + step), o_pad[0], o_pad[1]),
                    param_.kernel[0], param_.kernel[1], param_.stride[0],
                    param_.stride[1], param_.dilate[0], param_.dilate[1]);
            }
            const index_t gstride = temp_col.size(0) / param_.num_group;
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                mshadow::Tensor<xpu, 2, DType> tmpc =
                    temp_col.Slice(gstride * gid, gstride * (gid + 1));
                tmpc = dot(wmat[gid].T(), temp_dst[gid]);
            }
            if (o_pad[0] == 0 && o_pad[1] == 0) {
                out.Slice(i, i + step) = pack_col2patch(
                    temp_col, out.Slice(i, i + step).shape_, param_.kernel[0],
                    param_.kernel[1], param_.stride[0], param_.stride[1],
                    param_.dilate[0], param_.dilate[1]);
            } else {
                ...
                pshape[3] += 2 * o_pad[1];
                out.Slice(i, i + step) =
                    crop(pack_col2patch(temp_col, pshape, param_.kernel[0],
                                        param_.kernel[1], param_.stride[0],
                                        param_.stride[1], param_.dilate[0],
                                        param_.dilate[1]),
                         out[i][0].shape_);
            }
        }
        if (!param_.no_bias) {
            ...
            Tensor<xpu, 1, DType> bias =
                in_data[deconv::kBias].get<xpu, 1, DType>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad =
            out_grad[deconv::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> gdata =
            in_grad[deconv::kData].get<xpu, 4, DType>(s);
        Shape<3> wmat_shape =
            Shape3(param_.num_group, data.shape_[1] / param_.num_group,
                   param_.num_filter / param_.num_group * param_.kernel[0] *
                       param_.kernel[1]);
        ...
        Tensor<xpu, 3, DType> gwmat =
            in_grad[deconv::kWeight].get_with_shape<xpu, 3, DType>(wmat_shape,
                                                                   s);
#if defined(__CUDACC__)
        CHECK_EQ(s->blas_handle_ownership_, Stream<xpu>::OwnHandle)
            << "Must init CuBLAS handle in stream";
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            if (o_pad[0] == 0 && o_pad[1] == 0) {
                ...
                temp_col = unpack_patch2col(
                    pad(grad.Slice(i, i + step), o_pad[0], o_pad[1]),
                    param_.kernel[0], param_.kernel[1], param_.stride[0],
                    param_.stride[1], param_.dilate[0], param_.dilate[1]);
            }
            const index_t gstride = temp_col.size(0) / param_.num_group;
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                Tensor<xpu, 2, DType> tmpc =
                    temp_col.Slice(gstride * gid, gstride * (gid + 1));
                if (i == 0) {
                    Tensor<xpu, 2, DType> tmp_gwmat = gwmat[gid];
                    Assign(tmp_gwmat, req[deconv::kWeight],
                           dot(temp_dst[gid], tmpc.T()));
                } else {
            ...
            if (req[deconv::kData] == kWriteTo ||
                ...
                Assign(gdata.Slice(i, i + step), req[deconv::kData],
                       (swapaxis<1, 0>(reshape(
                           temp_dst,
                           mshadow::Shape4(gdata.shape_[1], step, gdata.size(2),
                                           gdata.size(3))))));
            }
        }
        if (!param_.no_bias) {
            Tensor<xpu, 1, DType> gbias =
                in_grad[deconv::kBias].get<xpu, 1, DType>(s);
   ...
   private:
    inline index_t InitTemp(const mshadow::Shape<4> &ishape,
        ...
        nstep_ =
            std::max(std::min(static_cast<index_t>(param_.workspace /
                                                   (shape_colunit_.Size() +
                                                    shape_dstunit_.Size())),
                              ishape[0]),
                     1U);

        mshadow::Shape<2> scol =
            mshadow::Shape2(shape_colunit_[0], shape_colunit_[1] * nstep_);
        mshadow::Shape<3> sdst = mshadow::Shape3(
   ...
   public:
    ...
    void Init(const std::vector<std::pair<std::string, std::string> > &kwargs)
        ...
        if (param_.kernel.ndim() == 1) {
            param_.layout =
                param_.layout ? param_.layout.value() : mshadow::kNCW;
            ...
            param_.layout =
                param_.layout ? param_.layout.value() : mshadow::kNCHW;
            if (param_.stride.ndim() == 0) param_.stride = Shape2(1, 1);
            if (param_.dilate.ndim() == 0) param_.dilate = Shape2(1, 1);
            if (param_.pad.ndim() == 0) param_.pad = Shape2(0, 0);
            if (param_.adj.ndim() == 0) param_.adj = Shape2(0, 0);
        } else {
            CHECK_EQ(param_.kernel.ndim(), 3U)
                << param_.kernel.ndim() << "D deconvolution not supported";
            param_.layout =
                param_.layout ? param_.layout.value() : mshadow::kNCDHW;
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.kernel.ndim() == 1) {
            ...
            Shape<3> dshape_ncw =
                ConvertLayout(dshape.get<3>(), param_.layout.value(), kNCW);
            Shape<3> wshape =
                Shape3(dshape_ncw[1], param_.num_filter / param_.num_group,
                       param_.kernel[0]);
            wshape = ConvertLayout(wshape, kNCW, param_.layout.value());
            SHAPE_ASSIGN_CHECK(*in_shape, deconv::kWeight, wshape);
            if (!param_.no_bias) {
                SHAPE_ASSIGN_CHECK(*in_shape, deconv::kBias,
                                   Shape1(param_.num_filter));
            }
            ...
            CHECK_GT(param_.kernel.Size(), 0U) << "incorrect kernel size: "
                                               << param_.kernel;
            CHECK_GT(param_.stride.Size(), 0U) << "incorrect stride size: "
                                               << param_.stride;
            CHECK_GT(param_.dilate.Size(), 0U) << "incorrect dilate size: "
                                               << param_.dilate;

            CHECK_GE(param_.stride[0] - 1, o_adj[0])
                << "adj(x) must be samller than stride[0]";
            ...
            if (param_.target_shape[0] > 0) {
                CHECK_EQ(param_.target_shape[0], oshape[2])
                    << "param_.target_shape[0] was not reasonable, please set "
                       "it carefully";
            }
            ...
            Shape<4> wshape =
                Shape4(dshape_nchw[1], param_.num_filter / param_.num_group,
                       param_.kernel[0], param_.kernel[1]);
            ...
            CHECK_GE(param_.stride[0] - 1, o_adj[0])
                << "adj(y) must be samller than stride[0]";
            CHECK_GE(param_.stride[1] - 1, o_adj[1])
                << "adj(x) must be samller than stride[1]";
            ...
            if (param_.target_shape[1] > 0) {
                CHECK_EQ(param_.target_shape[1], oshape[3])
                    << "param_.target_shape[1] was not reasonable, please set "
                       "it carefully";
            }
            ...
            Shape<5> dshape_ncdhw =
                ConvertLayout(dshape.get<5>(), param_.layout.value(), kNCDHW);
            Shape<5> wshape =
                Shape5(dshape_ncdhw[1], param_.num_filter / param_.num_group,
            ...
            CHECK_GE(param_.stride[0] - 1, o_adj[0])
                << "adj(d) must be samller than stride[0]";
            CHECK_GE(param_.stride[1] - 1, o_adj[1])
                << "adj(y) must be samller than stride[1]";
            CHECK_GE(param_.stride[2] - 1, o_adj[2])
                << "adj(x) must be samller than stride[2]";
            ...
            if (param_.target_shape[0] > 0) {
                CHECK_EQ(param_.target_shape[0], oshape[2])
                    << "param_.target_shape[0] was not reasonable, please it "
                       "carefully";
            }
            if (param_.target_shape[1] > 0) {
                CHECK_EQ(param_.target_shape[1], oshape[3])
            ...
            if (param_.target_shape[2] > 0) {
                CHECK_EQ(param_.target_shape[2], oshape[4])
                    << "param_.target_shape[2] was not reasonable, please set "
                       "it carefully";
            }

            SHAPE_ASSIGN_CHECK(
                *out_shape, 0,
                ConvertLayout(oshape, kNCDHW, param_.layout.value()));
        ...
        for (index_t i = 0; i < in_type->size(); ++i) {
            if ((*in_type)[i] == -1) {
                ...
                CHECK_EQ((*in_type)[i], dtype)
                    ...
                    << "Expected " << dtype << " v.s. given " << (*in_type)[i]
                    << " at " << ListArguments()[i];
            }
        }
        out_type->clear();
    ...
    std::string TypeString() const override { return "Deconvolution"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[deconv::kOut], in_data[deconv::kData],
