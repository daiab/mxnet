...
class L2NormalizationOp : public Operator {
   public:
    explicit L2NormalizationOp(L2NormalizationParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (param_.mode == l2_normalization::kInstance) {
            Shape<2> dshape = Shape2(
                orig_shape[0], orig_shape.ProdShape(1, orig_shape.ndim()));
            Tensor<xpu, 2> data =
                in_data[l2_normalization::kData].get_with_shape<xpu, 2, real_t>(
                    dshape, s);
            Tensor<xpu, 2> out =
                out_data[l2_normalization::kOut].get_with_shape<xpu, 2, real_t>(
                    dshape, s);
            Tensor<xpu, 1> norm =
                out_data[l2_normalization::kNorm].get<xpu, 1, real_t>(s);
            ...
            Shape<3> dshape =
                Shape3(orig_shape[0], orig_shape[1],
                       orig_shape.ProdShape(2, orig_shape.ndim()));
            Tensor<xpu, 3> data =
                in_data[l2_normalization::kData].get_with_shape<xpu, 3, real_t>(
                    dshape, s);
            Tensor<xpu, 3> out =
                out_data[l2_normalization::kOut].get_with_shape<xpu, 3, real_t>(
                    dshape, s);
            Shape<2> norm_shape = Shape2(dshape[0], dshape[2]);
            Tensor<xpu, 2> norm =
                out_data[l2_normalization::kNorm]
                    .get_with_shape<xpu, 2, real_t>(norm_shape, s);
            norm = reduce_with_axis<red::sum, false>(
                F<mxnet::op::mshadow_op::square>(data), 1);
            norm = F<mxnet::op::mshadow_op::square_root>(norm + param_.eps);
            ...
            Tensor<xpu, 2> norm =
                out_data[l2_normalization::kNorm]
                    .get_with_shape<xpu, 2, real_t>(norm_shape, s);
            norm = reduce_with_axis<red::sum, false>(
                F<mxnet::op::mshadow_op::square>(data), 2);
            norm = F<mxnet::op::mshadow_op::square_root>(norm + param_.eps);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (param_.mode == l2_normalization::kInstance) {
            ...
            Tensor<xpu, 2> data =
                out_data[l2_normalization::kOut].get_with_shape<xpu, 2, real_t>(
                    dshape, s);
            Tensor<xpu, 2> grad_in =
                in_grad[l2_normalization::kData].get_with_shape<xpu, 2, real_t>(
                    dshape, s);
            Tensor<xpu, 2> grad_out =
                out_grad[l2_normalization::kOut].get_with_shape<xpu, 2, real_t>(
                    dshape, s);
            Tensor<xpu, 1> norm =
                out_data[l2_normalization::kNorm].get<xpu, 1, real_t>(s);
            Tensor<xpu, 1> temp =
                ctx.requested[l2_normalization::kTempSpace].get_space<xpu>(
            ...
            Tensor<xpu, 3> data =
                out_data[l2_normalization::kOut].get_with_shape<xpu, 3, real_t>(
                    dshape, s);
            Tensor<xpu, 3> grad_in =
                in_grad[l2_normalization::kData].get_with_shape<xpu, 3, real_t>(
                    dshape, s);
            Tensor<xpu, 3> grad_out =
                out_grad[l2_normalization::kOut].get_with_shape<xpu, 3, real_t>(
                    dshape, s);
            Shape<2> norm_shape = Shape2(dshape[0], dshape[2]);
            Tensor<xpu, 2> norm =
                out_data[l2_normalization::kNorm]
            ...
            Assign(grad_in, req[l2_normalization::kData],
                   (grad_out -
                    data * broadcast_with_axis(temp, 0, orig_shape[1])) /
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(in_shape->size(), 1U)
            << "L2Normalization layer only accepts data as input";
        ...
        if (param_.mode == l2_normalization::kInstance) {
            ...
            CHECK_GE(dshape.ndim(), 3U)
                << "At lease 3 dimensions required in channel mode";
            TShape norm_shape = dshape;
            norm_shape[1] = 1;
            out_shape->push_back(norm_shape);
        } else if (param_.mode == l2_normalization::kSpatial) {
            CHECK_GE(dshape.ndim(), 3U)
                << "At lease 3 dimensions required in spatial mode";
    ...
    std::string TypeString() const override { return "L2Normalization"; }
