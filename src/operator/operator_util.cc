...
class SimpleOpRegEntryImpl : public SimpleOpRegEntry {
   public:
    TSelf &set_symbol_op_name(char const *symbol_name_str) override {
        ...
        CHECK(op_reg_ == nullptr || symbol_name == symbol_name_)
            << " operator " << this->name << " need to call set_symbol_op_name "
            << symbol_name << "before all other calls";
        symbol_name_ = symbol_name;
        return *this;
    }

    TSelf &set_enable_scalar(bool enable_scalar,
    ...
    TSelf &set_function(int dev_mask, SourceFunction fsource,
    ...
    TSelf &set_function(int dev_mask, UnaryFunction funary,
    ...
    TSelf &set_function(int dev_mask, BinaryFunction fbinary,
    ...
    TSelf &set_gradient(int dev_mask, UnaryGradFunctionT0 fgrad,
    ...
    TSelf &set_gradient(int dev_mask, UnaryGradFunctionT1 fgrad,
                        SimpleOpInplaceOption inplace_out_in_grad) override {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    TSelf &set_gradient(int dev_mask, UnaryGradFunctionT2 fgrad,
                        SimpleOpInplaceOption inplace_out_in_grad) override {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    TSelf &set_gradient(int dev_mask, BinaryGradFunctionT0 fgrad,
                        SimpleOpInplaceOption inplace_out_lhs_grad) override {
        std::lock_guard<std::mutex> lock(mutex_);
        SetFunction(&fbinary_grad_t0_, dev_mask, fgrad, "BinaryGradFunctionT0");
        binary_backward_inplace_out_lhs_ =
            (inplace_out_lhs_grad == kInplaceLhsOut);
        return *this;
    }

    TSelf &set_gradient(int dev_mask, BinaryGradFunctionT1 fgrad,
                        SimpleOpInplaceOption inplace_out_lhs_grad) override {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    TSelf &add_arguments(
        const std::vector<dmlc::ParamFieldInfo> &args) override {
        std::lock_guard<std::mutex> lock(mutex_);
        if (reg_counter_ != 1) return *this;
   ...
   protected:
    ...
    inline void SetFunction(std::vector<TFunction> *vfunc, int dev_mask,
                            TFunction func, const char *type) {
    ...
    inline NDArrayFunctionReg &NDArrayReg() {
        if (ndarray_reg_ == nullptr) {
            NDArrayFunctionReg &reg =
                ::dmlc::Registry<NDArrayFunctionReg>::Get()->__REGISTER__(
                    this->name);
            ndarray_reg_ = &reg;
        }
        return *ndarray_reg_;
    ...
    inline OperatorPropertyReg &OpReg() {
        if (op_reg_ == nullptr) {
            ...
            OperatorPropertyReg &reg =
                ::dmlc::Registry<OperatorPropertyReg>::Get()->__REGISTER__(
                    symbol_name_);
            op_reg_ = &reg;
        }
        return *op_reg_;
...
class SimpleOpPropBase : public OperatorProperty {
   public:
    ...
    std::map<std::string, std::string> GetParams() const override {
        if (source->enable_kwargs_) {
            return std::map<std::string, std::string>(env.kwargs.begin(),
                                                      env.kwargs.end());
        } else if (source->enable_scalar_) {
            SimpleOpScalarParam param;
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        for (unsigned i = 0; i < in_type->size(); ++i) {
            if (dtype == -1) {
                ...
                CHECK(in_type->at(i) == -1 || in_type->at(i) == dtype)
                    << "Non-uniform input data type. Expected " << dtype
                    << "got " << in_type->at(i);
            }
        }
...
void SimpleOpRegEntryImpl::RegisterSourceImperative() {
    ...
    auto body = [this](NDArray **used_vars, real_t *s, NDArray **mutate_vars,
        ...
        if (enable_kwargs_) {
            ...
            CHECK_EQ(num_params, 0) << "operator " << this->name
                                    << " do not take keyword arguments";
        }
        // shape inference.
        ...
        for (ResourceRequest req : resource_requests_) {
            env.resource.push_back(
                ResourceManager::Get()->Request(ret.ctx(), req));
        ...
        Engine::Get()->PushSync(
            ...
            ret.ctx(), {}, write_vars, FnProperty::kNormal, 0,
            PROFILER_MESSAGE("RegisterSourceImperative"));
    };
    // register the function.
    NDArrayReg().set_body(body).set_num_use_vars(0).set_num_mutate_vars(1);
    if (enable_scalar_) {
        NDArrayReg().set_num_scalars(1).add_argument(
            "scalar", "float", "scalar input to the function");
    }
}
...
class SimpleSourceOpProp : public SimpleOpPropBase {
   public:
    ...
    Operator *CreateOperator(Context ctx) const override {
        ...
        CHECK(dev_mask < source->fsource_.size() &&
              source->fsource_[dev_mask] != nullptr);
        op->forward = source->fsource_[dev_mask];
        op->env = this->env;
        return op;
    }

    std::vector<std::string> ListArguments() const override { return {}; }
...
void SimpleOpRegEntryImpl::RegisterUnaryImperative() {
    ...
    auto body = [this](NDArray **used_vars, real_t *s, NDArray **mutate_vars,
        ...
        for (ResourceRequest req : resource_requests_) {
            env.resource.push_back(
                ResourceManager::Get()->Request(src.ctx(), req));
            write_vars.push_back(env.resource.back().var);
        }
        ...
        Engine::Get()->PushSync(
            ...
            src.ctx(), const_vars, write_vars, FnProperty::kNormal, 0,
            PROFILER_MESSAGE("RegisterUnaryImperative"));
    };
    // register the function.
    ...
    if (enable_scalar_) {
        if (scalar_type_mask_ == kArrayBeforeScalar) {
            NDArrayReg()
                ...
                .add_argument("src", "NDArray-or-Symbol",
                              "Source input to the function")
                .add_argument("scalar", "float",
                              "scalar input to the function");
        } else {
            NDArrayReg()
                .set_num_scalars(1)
                .set_type_mask(kScalarArgBeforeNDArray |
                               kAcceptEmptyMutateTarget)
...
class SimpleUnaryOpProp : public SimpleOpPropBase {
   public:
    ...
    Operator *CreateOperator(Context ctx) const override {
        ...
        CHECK(dev_mask < source->funary_.size() &&
              source->funary_[dev_mask] != nullptr);
    ...
    auto body = [this](NDArray **used_vars, real_t *s, NDArray **mutate_vars,
        ...
        if (lhs.ctx().dev_mask() != cpu::kDevMask ||
            ...
            CHECK(lhs.ctx() == rhs.ctx())
                << "operands context mismatch " << lhs.ctx().dev_type << " "
                << lhs.ctx().dev_id << " vs. " << rhs.ctx().dev_type << " "
                << rhs.ctx().dev_id;
        }
        ...
        for (ResourceRequest req : resource_requests_) {
            env.resource.push_back(
                ResourceManager::Get()->Request(lhs.ctx(), req));
            write_vars.push_back(env.resource.back().var);
        }
        ...
        if (rhs.var() == ret.var()) {
            LOG(ERROR) << " operation " << this->name
                       << " warning, perform inplace operation with right "
                          "operand, may not be supported";
        }
        ...
        Engine::Get()->PushSync(
            ...
            lhs.ctx(), const_vars, write_vars, FnProperty::kNormal, 0,
            PROFILER_MESSAGE("RegisterBinaryImperative"));
    };
    // register the function.
    ...
    if (enable_scalar_) {
        if (scalar_type_mask_ == kArrayBeforeScalar) {
            NDArrayReg()
                ...
                .set_type_mask(kNDArrayArgBeforeScalar |
                               kAcceptEmptyMutateTarget)
                .add_argument("lhs", "NDArray-or-Symbol",
                              "Left operand  to the function")
                .add_argument("rhs", "NDArray-or-Symbol",
...
struct SimpleBinaryOperator : public Operator {
    ...
    void Backward(const OpContext &ctx, const std::vector<TBlob> &out_grad,
        ...
        if (backward0 != nullptr) {
            (*backward0)(ograd, env, &lgrad, &rgrad, req[0], req[1],
                         ctx.run_ctx);
        } else if (backward1 != nullptr) {
            Input0 in0;
            in0.data = in_data[0];
            Input1 in1;
            in1.data = in_data[1];
            (*backward1)(ograd, in0, in1, env, &lgrad, &rgrad, req[0], req[1],
                         ctx.run_ctx);
        } else {
            LOG(FATAL) << "Backward is not supported";
        }
   ...
   public:
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (source->binary_shape_ == nullptr) {
            ...
            out_shape->push_back(
                (*(source->binary_shape_))(lshape, rshape, env));
        }
        return true;
    }
    ...
    Operator *CreateOperator(Context ctx) const override {
        ...
        CHECK(dev_mask < source->fbinary_.size() &&
              source->fbinary_[dev_mask] != nullptr);
