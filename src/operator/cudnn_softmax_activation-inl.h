   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (param_.mode == softmax_activation::kInstance) {
            ...
            Shape<4> dshape =
                Shape4(in_data[softmax_activation::kData].shape_[0],
                       in_data[softmax_activation::kData].shape_[1], 1, 1);
            data = in_data[softmax_activation::kData]
                       .get_with_shape<gpu, 4, real_t>(dshape, s);
            out = out_data[softmax_activation::kOut]
                      .get_with_shape<gpu, 4, real_t>(dshape, s);
        ...
        CUDNN_CALL(cudnnSoftmaxForward(
            s->dnn_handle_, CUDNN_SOFTMAX_ACCURATE, softmax_mode, &alpha,
            shape_desc_, data.dptr_, &beta, shape_desc_, out.dptr_));
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (param_.mode == softmax_activation::kInstance) {
            CHECK_EQ(in_grad[softmax_activation::kData].ndim(), 2)
                << "Input need to have 2 dimensions when mode=instance.";
            Shape<4> dshape =
                Shape4(in_grad[softmax_activation::kData].shape_[0],
                       in_grad[softmax_activation::kData].shape_[1], 1, 1);
            grad = out_grad[softmax_activation::kOut]
                       .get_with_shape<gpu, 4, real_t>(dshape, s);
            output_data = out_data[softmax_activation::kOut]
                              .get_with_shape<gpu, 4, real_t>(dshape, s);
            input_grad = in_grad[softmax_activation::kData]
                             .get_with_shape<gpu, 4, real_t>(dshape, s);
            softmax_mode = CUDNN_SOFTMAX_MODE_INSTANCE;
        } else {
        ...
        CUDNN_CALL(cudnnSoftmaxBackward(
            s->dnn_handle_, CUDNN_SOFTMAX_ACCURATE, softmax_mode, &alpha,
            shape_desc_, output_data.dptr_, shape_desc_, grad.dptr_, &beta,
