 ...
 * \file lsoftmax-inl.h
 * \brief LSoftmax from <Large-Margin Softmax Loss for Convolutional Neural
 * Networks>
 * \author luoyetx
 */
...
class LSoftmaxOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> x =
            in_data[lsoftmax_enum::kData].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> w =
            in_data[lsoftmax_enum::kWeight].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 1, DType> label =
            in_data[lsoftmax_enum::kLabel].get_with_shape<xpu, 1, DType>(
                Shape1(n), s);
        Tensor<xpu, 2, DType> out =
            out_data[lsoftmax_enum::kOut].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 1, DType> x_norm =
            out_data[lsoftmax_enum::kDataNorm].get_with_shape<xpu, 1, DType>(
                Shape1(n), s);
        Tensor<xpu, 1, DType> w_norm =
            out_data[lsoftmax_enum::kWeightNorm].get_with_shape<xpu, 1, DType>(
                Shape1(m), s);
        ...
        if (ctx.is_train) {
            ...
            Tensor<cpu, 1, DType> k_table_cpu(k_table_.data(),
                                              Shape1(k_table_.size()));
            Tensor<cpu, 1, DType> c_table_cpu(c_table_.data(),
                                              Shape1(c_table_.size()));
            ...
            LSoftmaxForward(x, w, label, out, x_norm, w_norm, k_table_xpu,
                            c_table_xpu, margin, beta);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> o_grad =
            out_grad[lsoftmax_enum::kOut].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> x_grad =
            in_grad[lsoftmax_enum::kData].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> w_grad =
            in_grad[lsoftmax_enum::kWeight].FlatTo2D<xpu, DType>(s);
        // workspace is used for cos_t, cos_mt, k, sin2_t, fo and cos_t_m for
        // every data point
        Tensor<xpu, 2, DType> workspace =
            ctx.requested[lsoftmax_enum::kTempSpace]
                .get_space_typed<xpu, 2, DType>(Shape2(6, n), s);
#if defined(__CUDACC__)
        CHECK_EQ(s->blas_handle_ownership_, Stream<xpu>::OwnHandle)
            << "Must init CuBLAS handle in stream";
#endif
        ...
        Copy(k_table_xpu, k_table_cpu, s);
        Copy(c_table_xpu, c_table_cpu, s);
        LSoftmaxBackward(x, w, label, x_norm, w_norm, o_grad, x_grad, w_grad,
                         workspace, k_table_xpu, c_table_xpu, margin, beta);
        FreeSpace(&k_table_xpu);
        FreeSpace(&c_table_xpu);
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(dshape.ndim(), 2)
            << "data shape should be (batch_size, feature_dim)";
        ...
        SHAPE_ASSIGN_CHECK(*in_shape, lsoftmax_enum::kWeight,
                           Shape2(m, feature_dim));
        out_shape->clear();
    ...
    std::vector<int> DeclareBackwardDependency(
        ...
        return {out_grad[lsoftmax_enum::kOut],
                out_data[lsoftmax_enum::kDataNorm],
                out_data[lsoftmax_enum::kWeightNorm],
                in_data[lsoftmax_enum::kData],
                in_data[lsoftmax_enum::kWeight],
                in_data[lsoftmax_enum::kLabel]};
    }

    std::string TypeString() const override { return "LSoftmax"; }
