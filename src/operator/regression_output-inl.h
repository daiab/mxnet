   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        CHECK_EQ(in_data.size(), 2U)
            << "RegressionOutputOp Input: [data, label]";
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Stream<xpu> *s = ctx.get_stream<xpu>();
        real_t num_output = in_data[reg_enum::kLabel].Size() /
                            in_data[reg_enum::kLabel].shape_[0];
        Tensor<xpu, 2> out = out_data[reg_enum::kOut].FlatTo2D<xpu, real_t>(s);
        Tensor<xpu, 2> grad = in_grad[reg_enum::kData].FlatTo2D<xpu, real_t>(s);
        Tensor<xpu, 2> label =
            in_data[reg_enum::kLabel].get_with_shape<xpu, 2, real_t>(out.shape_,
                                                                     s);
        Assign(grad, req[reg_enum::kData],
               param_.grad_scale / num_output *
