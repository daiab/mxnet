...
class MultiBoxPriorOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        // TODO(zhreshold): this implementation is to be compliant to original
        // ssd in caffe
        // The prior boxes could be implemented in more versatile ways
        // since input sizes are same in each batch, we could share
        // MultiBoxPrior
        const int num_sizes = static_cast<int>(sizes_.size());
        const int num_ratios = static_cast<int>(ratios_.size());
        const int num_anchors =
            num_sizes - 1 + num_ratios;  // anchors per location
        ...
        out = out_data[mboxprior_enum::kOut].get_with_shape<xpu, 2, DType>(
            oshape, s);
        CHECK_GE(steps_[0] * steps_[1], 0)
            << "Must specify both step_y and step_x";
        ...
        if (clip_) {
            Assign(out, req[mboxprior_enum::kOut],
                   F<mshadow_op::clip_zero_one>(out));
        }
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> grad =
            in_grad[mboxprior_enum::kData].FlatTo2D<xpu, DType>(s);
        grad = 0.f;
    }
...
class MultiBoxPriorProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_GE(dshape.ndim(), 4)
            << "Input data should be 4D: batch-channel-y-x";
        ...
        CHECK_EQ(param_.steps.ndim(), 2)
            << "Step ndim must be 2: (step_y, step_x)";
        return true;
    }
    ...
    std::string TypeString() const override { return "_contrib_MultiBoxPrior"; }
