...
enum fftOpOutputs {
    kOutComplex
};  // seperate the image and real parts at the moment
enum fftOpResource {
    kTempSpace
};  // might be requiered as we need to pad the real matrices
}

struct FFTParam : public dmlc::Parameter<FFTParam> {
    int compute_size;  // the maximum size of sub-batch to be forwarded through
                       // FFT in one time
...
class FFTOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (!init_cufft_) {
            n_ffts = in_data[fft::kData].shape_.ProdShape(
                0, in_data[fft::kData].ndim() - 1);
        ...
        Tensor<xpu, 2, DType> data =
            in_data[fft::kData].get_with_shape<xpu, 2, DType>(
                Shape2(n_ffts, dim_), s);
        Tensor<xpu, 2, DType> out =
            out_data[fft::kOutComplex].get_with_shape<xpu, 2, DType>(
                Shape2(n_ffts, dim_ * 2), s);

        // need temp space to pad the data into complex numbers due to cufft
        // interface
        Tensor<xpu, 1, DType> workspace =
            ctx.requested[fft::kTempSpace].get_space_typed<xpu, 1, DType>(
                Shape1(param_.compute_size * dim_ * 2), s);
        Tensor<xpu, 2, DType> complex_data = Tensor<xpu, 2, DType>(
            workspace.dptr_, Shape2(param_.compute_size, dim_ * 2), s);
#if MSHADOW_USE_CUDNN
        // start fft
        cufftHandle plan;
        cufftPlanMany(&plan, 1, &dim_, nullptr, 0, 0, nullptr, 0, 0, CUFFT_C2C,
                      param_.compute_size);
        for (size_t idx = 0; idx < num_compute; ++idx) {
            complex_data = complex_pad_imag(
                data.Slice(idx * param_.compute_size,
            ...
            cufftComplex *out_tmp =
                reinterpret_cast<cufftComplex *>(out.dptr_ + 2 * idx * stride_);
            CHECK_EQ(cufftExecC2C(plan, in_tmp, out_tmp, CUFFT_FORWARD),
                     CUFFT_SUCCESS);
        }
        cufftDestroy(plan);
        ...
        if (remain_num > 0) {
            ...
            complex_data = Tensor<xpu, 2, DType>(
                workspace.dptr_, Shape2(remain_num, dim_ * 2), s);
            complex_data = complex_pad_imag(
                data.Slice(num_compute * param_.compute_size,
                           num_compute * param_.compute_size + remain_num));

            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(complex_data.dptr_));
            cufftComplex *out_tmp = reinterpret_cast<cufftComplex *>(
                out.dptr_ + 2 * num_compute * stride_);
            CHECK_EQ(cufftExecC2C(plan_remain, in_tmp, out_tmp, CUFFT_FORWARD),
                     CUFFT_SUCCESS);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> gdata =
            in_grad[fft::kData].get_with_shape<xpu, 2, DType>(
                Shape2(n_ffts, dim_), s);
        Tensor<xpu, 2, DType> grad =
            out_grad[fft::kOutComplex].get_with_shape<xpu, 2, DType>(
                Shape2(n_ffts, dim_ * 2), s);
        // need temp space to pad the data into complex numbers due to cufft
        // interface
        Tensor<xpu, 1, DType> workspace =
...
// but if it is not, remember that we have implemented complex_take_real, and
// use this
#if MSHADOW_USE_CUDNN
        cufftHandle plan;
        cufftPlanMany(&plan, 1, &dim_, nullptr, 0, 0, nullptr, 0, 0, CUFFT_C2C,
        ...
        for (size_t idx = 0; idx < num_compute; ++idx) {
            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(grad.dptr_ +
                                                       2 * idx * stride_));
            cufftComplex *out_tmp =
                reinterpret_cast<cufftComplex *>(complex_data.dptr_);
            CHECK_EQ(cufftExecC2C(plan, in_tmp, out_tmp, CUFFT_INVERSE),
                     CUFFT_SUCCESS);

            Assign(gdata.Slice(idx * param_.compute_size,
                               (idx + 1) * param_.compute_size),
        ...
        if (remain_num > 0) {
            ...
            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(
                    grad.dptr_ + 2 * num_compute * stride_));
            cufftComplex *out_tmp =
                reinterpret_cast<cufftComplex *>(complex_data.dptr_);
            CHECK_EQ(cufftExecC2C(plan_remain, in_tmp, out_tmp, CUFFT_INVERSE),
                     CUFFT_SUCCESS);
...
class FFTProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (dshape.ndim() == 4) {
            out_shape->push_back(
                Shape4(dshape[0], dshape[1], dshape[2], dshape[3] * 2));
    ...
    std::string TypeString() const override { return "_contrib_fft"; }
