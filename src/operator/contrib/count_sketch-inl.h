...
class CountSketchOp : public Operator {
   public:
    explicit CountSketchOp(CountSketchParam param) { this->param_ = param; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> data =
            in_data[CountSketch::kData].FlatTo2D<xpu, DType>(s);

        const TShape &hshape = in_data[CountSketch::kH].shape_;
        const TShape &sshape = in_data[CountSketch::kS].shape_;
        Tensor<xpu, 1, DType> h =
            in_data[CountSketch::kH].get_with_shape<xpu, 1, DType>(
                Shape1(hshape.ProdShape(0, hshape.ndim())), s);
        Tensor<xpu, 1, DType> ss =
            in_data[CountSketch::kS].get_with_shape<xpu, 1, DType>(
                Shape1(sshape.ProdShape(0, sshape.ndim())), s);
        Tensor<xpu, 2, DType> out =
            out_data[CountSketch::kOut].FlatTo2D<xpu, DType>(s);
        ...
        CountSketchForward(out, data, h, ss, n_samples,
                           this->param_.processing_batch_size, in_dim,
                           this->param_.out_dim);
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> ograd =
            out_grad[CountSketch::kOut].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> dgrad =
            in_grad[CountSketch::kData].FlatTo2D<xpu, DType>(s);
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (dshape.ndim() == 4) {
            ...
            out_shape->push_back(
                Shape4(dshape[0], dshape[1], dshape[2], param_.out_dim));
    ...
    std::string TypeString() const override { return "_contrib_count_sketch"; }
