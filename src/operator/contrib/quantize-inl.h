...
struct quantize {
    ...
    MSHADOW_XINLINE static void Map(int i, DstDType *out, float *omin_range,
                                    ...
                                    const float *imax_range, double min_limit,
                                    double max_limit) {
...
void QuantizeCompute(const nnvm::NodeAttrs &attrs, const OpContext &ctx,
    ...
    Kernel<quantize, xpu>::Launch(
        s, outputs[0].Size(), outputs[0].dptr<DstDType>(),
        outputs[1].dptr<float>(), outputs[2].dptr<float>(),
        inputs[0].dptr<SrcDType>(), inputs[1].dptr<float>(),
        inputs[2].dptr<float>(), std::numeric_limits<DstDType>::min(),
        std::numeric_limits<DstDType>::max());
}
...
inline bool QuantizeType(const nnvm::NodeAttrs &attrs,
    ...
    CHECK_EQ((*in_attrs)[1], mshadow::kFloat32) << "the second input of "
                                                   "`quantize` should be a "
                                                   "tensor with type of float";
    CHECK_EQ((*in_attrs)[2], mshadow::kFloat32) << "the third input of "
                                                   "`quantize` should be a "
                                                   "tensor with type of float";
