...
inline void MultiBoxDetectionForward(
    const Tensor<cpu, 3, DType> &out, const Tensor<cpu, 3, DType> &cls_prob,
    const Tensor<cpu, 2, DType> &loc_pred, const Tensor<cpu, 2, DType> &anchors,
    const Tensor<cpu, 3, DType> &temp_space, const float threshold,
    const bool clip, const nnvm::Tuple<float> &variances,
    const float nms_threshold, const bool force_suppress, const int nms_topk) {
    ...
    for (int nbatch = 0; nbatch < num_batches; ++nbatch) {
        const DType *p_cls_prob =
            cls_prob.dptr_ + nbatch * num_classes * num_anchors;
        ...
        for (int i = 0; i < num_anchors; ++i) {
            ...
            if (id > 0) {
                ...
                p_out[valid_count * 6] =
                    id - 1;  // remove background, restore original id
                p_out[valid_count * 6 + 1] = (id == 0 ? DType(-1) : score);
                int offset = i * 4;
                TransformLocations(p_out + valid_count * 6 + 2,
                                   p_anchor + offset, p_loc_pred + offset, clip,
        ...
        for (int i = 0; i < valid_count; ++i) {
            ...
            for (int j = i + 1; j < valid_count; ++j) {
                ...
                if (force_suppress || (p_out[offset_i] == p_out[offset_j])) {
                    ...
                    DType iou = CalculateOverlap(p_out + offset_i + 2,
                                                 p_out + offset_j + 2);
...
Operator *CreateOp<cpu>(MultiBoxDetectionParam param, int dtype) {
    ...
    MSHADOW_REAL_TYPE_SWITCH(
        dtype, DType, { op = new MultiBoxDetectionOp<cpu, DType>(param); });
    return op;
}

Operator *MultiBoxDetectionProp::CreateOperatorEx(
...
MXNET_REGISTER_OP_PROPERTY(_contrib_MultiBoxDetection, MultiBoxDetectionProp)
    ...
    .add_argument("loc_pred", "NDArray-or-Symbol",
