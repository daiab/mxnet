...
inline void AssignLocTargets(const DType *anchor, const DType *l, DType *dst,
                             const float vx, const float vy, const float vw,
                             const float vh) {
    float al = *(anchor);
    float at = *(anchor + 1);
    float ar = *(anchor + 2);
...
inline void MultiBoxTargetForward(
    const Tensor<cpu, 2, DType> &loc_target,
    const Tensor<cpu, 2, DType> &loc_mask,
    const Tensor<cpu, 2, DType> &cls_target,
    const Tensor<cpu, 2, DType> &anchors, const Tensor<cpu, 3, DType> &labels,
    const Tensor<cpu, 3, DType> &cls_preds,
    const Tensor<cpu, 4, DType> &temp_space, const float overlap_threshold,
    const float background_label, const float negative_mining_ratio,
    const float negative_mining_thresh, const int minimum_negative_samples,
    ...
    for (int nbatch = 0; nbatch < num_batches; ++nbatch) {
        ...
        const DType *p_overlaps =
            temp_space.dptr_ + nbatch * num_anchors * num_labels;
        int num_valid_gt = 0;
        for (int i = 0; i < num_labels; ++i) {
            if (static_cast<float>(*(p_label + i * label_width)) == -1.0f) {
                CHECK_EQ(static_cast<float>(*(p_label + i * label_width + 1)),
                         -1.0f);
                CHECK_EQ(static_cast<float>(*(p_label + i * label_width + 2)),
                         -1.0f);
                CHECK_EQ(static_cast<float>(*(p_label + i * label_width + 3)),
                         -1.0f);
                CHECK_EQ(static_cast<float>(*(p_label + i * label_width + 4)),
                         -1.0f);
                break;
            }
            ++num_valid_gt;
        ...
        if (num_valid_gt > 0) {
            ...
            std::vector<std::pair<float, int>> max_matches(
                num_anchors, std::pair<float, int>(-1.0f, -1));
            std::vector<char> anchor_flags(num_anchors,
                                           -1);  // -1 means don't care
            int num_positive = 0;
            while (std::find(gt_flags.begin(), gt_flags.end(), false) !=
                   gt_flags.end()) {
                // ground-truths not fully matched
                int best_anchor = -1;
                int best_gt = -1;
                float max_overlap =
                    1e-6;  // start with a very small positive overlap
            ...
            if (negative_mining_ratio > 0) {
                ...
                DType *p_cls_preds =
                    cls_preds.dptr_ + nbatch * num_classes * num_anchors;
                ...
                if (num_negative > 0) {
                    ...
                    for (int j = 0; j < num_anchors; ++j) {
                        ...
                        if (max_matches[j].first < 0) {
                            // not yet calculated
                            const DType *pp_overlaps =
                                p_overlaps + j * num_labels;
                            int best_gt = -1;
                            float max_iou = -1.0f;
                            for (int k = 0; k < num_valid_gt; ++k) {
                                float iou =
                                    static_cast<float>(*(pp_overlaps + k));
                                if (iou > max_iou) {
                                    best_gt = k;
                                    max_iou = iou;
                                }
                        ...
                        if (max_matches[j].first < negative_mining_thresh &&
                            ...
                            DType prob =
                                std::exp(p_cls_preds[j] - max_val) / sum;
                            // loss should be -log(x), but value does not
                            // matter, skip log
                    ...
                    for (int i = 0; i < num_negative; ++i) {
                        anchor_flags[temp[i].index] =
                            0;  // mark as negative sample
                    }
                }
            } else {
                ...
                if (anchor_flags[i] == 1) {
                    ...
                    *(p_loc_mask + offset + 3) = 1;
                    AssignLocTargets(
                        p_anchor + i * 4,
                        p_label + label_width * max_matches[i].second + 1,
                        p_loc_target + offset, variances[0], variances[1],
...
Operator *MultiBoxTargetProp::CreateOperatorEx(
