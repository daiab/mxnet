...
struct NumericalParam {
    ...
    inline size_t ndim() const { return info.size(); }
    std::vector<VType> info;
};

template <typename VType>
inline std::istream &operator>>(std::istream &is,
                                NumericalParam<VType> &param) {
    while (true) {
        char ch = is.get();
        if (ch == '(') break;
        if (!isspace(ch)) {
...
inline std::ostream &operator<<(std::ostream &os,
                                const NumericalParam<VType> &param) {
    os << '(';
...
struct ProposalParam : public dmlc::Parameter<ProposalParam> {
    ...
    DMLC_DECLARE_PARAMETER(ProposalParam) {
        ...
        DMLC_DECLARE_FIELD(rpn_pre_nms_top_n)
            .set_default(6000)
            .describe(
                "Number of top scoring boxes to keep after applying NMS to RPN "
                "proposals");
        DMLC_DECLARE_FIELD(rpn_post_nms_top_n)
            .set_default(300)
            .describe(
                "Overlap threshold used for non-maximum"
...
class ProposalProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "_contrib_Proposal"; }
...
inline void _MakeAnchor(float w, float h, float x_ctr, float y_ctr,
...
inline void _Transform(float scale, float ratio,
