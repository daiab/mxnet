...
class CpuCTC {
   public:
    // Noncopyable
    CpuCTC(int alphabet_size, int minibatch, void* workspace, int blank_label)
    ...
    ctcStatus_t cost_and_grad(const ProbT* const activations, ProbT* grads,
                              ProbT* costs, const int* const flat_labels,
                              const int* const label_lengths,
                              const int* const input_lengths);

    ctcStatus_t score_forward(const ProbT* const activations, ProbT* costs,
   ...
   private:
    class CpuCTC_metadata {
       private:
        int setup_labels(const int* const labels, int blank_label, int L,
                         int S);
    ...
    std::tuple<ProbT, bool> cost_and_grad_kernel(ProbT* grad,
                                                 ...
                                                 const int* const labels, int T,
                                                 int L, int mb,
                                                 size_t bytes_used);

    ProbT compute_alphas(const ProbT* probs, int repeats, int S, int T,
                         const int* const e_inc, const int* const s_inc,
                         const int* const labels, ProbT* alphas);

    ProbT compute_betas_and_grad(ProbT* grad, const ProbT* const probs,
                                 ProbT log_partition, int repeats, int S, int T,
                                 const int* const e_inc, const int* const s_inc,
                                 const int* const labels, ProbT* alphas,
                                 ProbT* betas, ProbT* output);
};

template <typename ProbT>
CpuCTC<ProbT>::CpuCTC_metadata::CpuCTC_metadata(
    int L, int S, int T, int mb, int alphabet_size, void* workspace,
    size_t bytes_used, int blank_label, const int* const labels) {
    alphas =
...
template <typename ProbT>
void CpuCTC<ProbT>::softmax(const ProbT* const activations, ProbT* probs,
                            const int* const input_lengths) {
    ...
    for (int mb = 0; mb < minibatch_; ++mb) {
        for (int c = 0; c < input_lengths[mb]; ++c) {
            ...
            for (int r = 0; r < alphabet_size_; ++r)
                max_activation =
                    std::max(max_activation, activations[r + col_offset]);
...
std::tuple<ProbT, bool> CpuCTC<ProbT>::cost_and_grad_kernel(
    ProbT* grad, const ProbT* const probs, const int* const labels, int T,
    int L, int mb, size_t bytes_used) {
    const int S = 2 * L + 1;  // Number of labels with blanks

    CpuCTC_metadata ctcm(L, S, T, mb, alphabet_size_, workspace_, bytes_used,
                         blank_label_, labels);
    ...
    ProbT llForward =
        compute_alphas(probs, ctcm.repeats, S, T, ctcm.e_inc, ctcm.s_inc,
                       ctcm.labels_w_blanks, ctcm.alphas);

    ProbT llBackward = compute_betas_and_grad(
        grad, probs, llForward, ctcm.repeats, S, T, ctcm.e_inc, ctcm.s_inc,
        ctcm.labels_w_blanks, ctcm.alphas, ctcm.betas, ctcm.output);
...
template <typename ProbT>
ProbT CpuCTC<ProbT>::compute_alphas(const ProbT* probs, int repeats, int S,
                                    int T, const int* const e_inc,
    ...
    for (int t = 1; t < T; ++t) {
        ...
        for (int i = startloop; i < end; ++i) {
            ProbT prev_sum = ctc_helper::log_plus<ProbT>()(
                alphas[i + idx2], alphas[(i - 1) + idx2]);

            // Skip two if not on blank and not on repeat.
            if (labels[i] != blank_label_ && i != 1 &&
                labels[i] != labels[i - 2])
                prev_sum = ctc_helper::log_plus<ProbT>()(
                    prev_sum, alphas[(i - 2) + idx2]);
    ...
    for (int i = start; i < end; ++i) {
        loglike =
            ctc_helper::log_plus<ProbT>()(loglike, alphas[i + (T - 1) * S]);
    }
...
ProbT CpuCTC<ProbT>::compute_betas_and_grad(
    ProbT* grad, const ProbT* const probs, ProbT log_partition, int repeats,
    int S, int T, const int* const e_inc, const int* const s_inc,
    const int* const labels, ProbT* alphas, ProbT* betas, ProbT* output) {
    ...
    for (int i = start; i < end; ++i) {
        betas[i] = std::log(
            probs[labels[i] + (T - 1) * (alphabet_size_ * minibatch_)]);
        ...
        output[labels[i]] = ctc_helper::log_plus<ProbT>()(
            alphas[i + (T - 1) * S], output[labels[i]]);
    }
    ...
    for (int t = T - 2; t >= 0; --t) {
        ...
        std::fill(output, output + alphabet_size_,
                  ctc_helper::neg_inf<ProbT>());

        for (int i = start; i < endloop; ++i) {
            ProbT next_sum =
                ctc_helper::log_plus<ProbT>()(betas[i], betas[(i + 1)]);
            // Skip two if not on blank and not on repeat.
            if (labels[i] != blank_label_ && i != (S - 2) &&
                labels[i] != labels[i + 2]) {
                next_sum =
                    ctc_helper::log_plus<ProbT>()(next_sum, betas[(i + 2)]);
            }
            ...
            // update the gradient associated with this label
            output[labels[i]] = ctc_helper::log_plus<ProbT>()(
                alphas[i + idx1], output[labels[i]]);
        }

        if (end == S) {
            betas[(S - 1)] =
                betas[(S - 1)] + std::log(probs[blank_label_ + idx3]);
...
#pragma omp parallel for
    for (int mb = 0; mb < minibatch_; ++mb) {
        ...
        std::tie(costs[mb], mb_status) = cost_and_grad_kernel(
            grads + mb * alphabet_size_, probs + mb * alphabet_size_,
            flat_labels + std::accumulate(label_lengths, label_lengths + mb, 0),
            T, L, mb, bytes_used + mb * per_minibatch_bytes);
    }
...
#pragma omp parallel for
    for (int mb = 0; mb < minibatch_; ++mb) {
        ...
        CpuCTC_metadata ctcm(
            L, S, T, mb, alphabet_size_, workspace_,
            bytes_used + mb * per_minibatch_bytes, blank_label_,
            flat_labels +
                std::accumulate(label_lengths, label_lengths + mb, 0));

        if (L + ctcm.repeats > T)
            costs[mb] = ProbT(0);
        else {
            costs[mb] = -compute_alphas(
                probs + mb * alphabet_size_, ctcm.repeats, S, T, ctcm.e_inc,
                ctcm.s_inc, ctcm.labels_w_blanks, ctcm.alphas);
        }
    }
