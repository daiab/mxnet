...
struct CTASegReduce {
    ...
    __device__ static void preprocessKeys(KeyT *keys, int count,
                                          int *numUniqueLabels,
                                          int seg_start[VT], int seg_end[VT],
                                          int *scanout) {
...
__global__ void compute_alpha_kernel(
    const ProbT *probs, const int *label_sizes, const int *utt_length,
    const int *repeats_in_labels, const int *labels_without_blanks,
    const int *label_offsets, int *labels_with_blanks, ProbT *alphas,
    ProbT *nll_forward, int stride, int out_dim, int S_memoffset,
    int T_memoffset, int blank_label) {
    ...
    // Generate labels with blanks from labels without blanks
    {
        ...
        if (tid == 0) {
            labels_with_blanks[(blockIdx.x * S_memoffset) + 2 * L] =
                blank_label;
        }
    }
    __syncthreads();
...
#pragma unroll
    ...
    for (int t = 1; t < T; ++t) {
        ...
        // The prob is a 2D column major array, with probabilites for each t
        // strided
...
#pragma unroll
        for (int idx = (tid + 1); idx < S; idx += blockDim.x) {
            ...
            if ((label[idx] != blank_label) && (idx != 1) &&
                ...
                prev_sum =
                    log_plus_f(prev_sum, alpha[(idx - 2) + start_prev_row]);

            alpha[idx + start_cur_row] =
                prev_sum +
                log(probs[prob_offset + start_prob_col + label[idx]]);
        }
...
template <typename ProbT, int NT, int VT>
__global__ void compute_betas_and_grad_kernel(
    const ProbT *probs, const int *label_sizes, const int *utt_length,
    const int *repeats_in_labels, const int *labels_with_blanks, ProbT *alphas,
    const ProbT *nll_forward, ProbT *nll_backward, ProbT *grads, int stride,
    int out_dim, int S_memoffset, int T_memoffset, int blank_label) {
    ctc_helper::log_plus<ProbT> log_plus_f;
...
#pragma unroll
        ...
        CTAMergesort<NT, VT, true, true, int, int, mgpu::less<int>>(
            key, gather_val, keys_shared, gather_indices, S, tid,
            mgpu::less<int>());
        ...
        SegReduce::preprocessKeys(keys_shared, S, &uniquelabels, seg_start,
                                  seg_end, temp_buffer.result);
        __syncthreads();
    }
...
#pragma unroll
    ...
    for (int i = tid; i < (end - start); i += blockDim.x)
        temp_buffer.beta[i + start] =
            log(probs[prob_offset + (T - 1) * (out_dim * stride) +
                      label[i + start]]);
...
#pragma unroll
            for (int idx = tid, i = 0; idx < (S - 1); idx += NT, i++) {
                ProbT next_sum = log_plus_f(temp_buffer.beta[idx],
                                            temp_buffer.beta[idx + 1]);
            ...
            // Initialize values for the rightmost column since there is nothing
            // to the right
...
#pragma unroll
            for (int idx = tid; idx < S; idx += NT) {
                output[idx] =
                    alpha[idx + start_cur_row] + temp_buffer.beta[idx];
            }
        ...
        // Compute segmented reduction of output by using label as key
        {
            ...
            for (int idx = tid, j = 0; idx < uniquelabels;
                 idx += blockDim.x, ++j) {
