...
class GpuCTC {
   public:
    GpuCTC(int alphabet_size, int minibatch, void* workspace, CUstream stream,
           int blank_label)
   ...
   private:
    ...
    ctcStatus_t launch_alpha_beta_kernels(const ProbT* const probs,
                                          ProbT* grads, bool compute_alpha,
                                          bool compute_beta);

    ctcStatus_t launch_gpu_kernels(const ProbT* const probs, ProbT* grads,
                                   size_t config, bool launch_alpha,
                                   bool launch_beta);
    ...
    ctcStatus_t create_metadata_and_choose_config(
        const int* const label_lengths, const int* const flat_labels,
        const int* const input_lengths, size_t& best_config);
...
ctcStatus_t GpuCTC<ProbT>::setup_gpu_metadata(const int* const flat_labels,
    ...
    nll_backward_ = reinterpret_cast<ProbT*>(
    ...
    for (int pass = 0; pass < num_passes; ++pass) {
        ...
        cuda_status =
            cudaMemcpyAsync(&(label_offsets_[start_idx]), label_offsets,
                            (end_idx - start_idx) * sizeof(int),
                            cudaMemcpyHostToDevice, stream_);
        if (cuda_status != cudaSuccess) return CTC_STATUS_MEMOPS_FAILED;
    }
    ...
    cuda_status =
        cudaMemcpyAsync(utt_length_, input_lengths, minibatch_ * sizeof(int),
                        cudaMemcpyHostToDevice, stream_);
    if (cuda_status != cudaSuccess) return CTC_STATUS_MEMOPS_FAILED;
    ...
    gpu_bytes_used += minibatch_ * sizeof(int);
    cuda_status =
        cudaMemcpyAsync(label_sizes_, label_lengths, minibatch_ * sizeof(int),
                        cudaMemcpyHostToDevice, stream_);
    if (cuda_status != cudaSuccess) return CTC_STATUS_MEMOPS_FAILED;
...
ctcStatus_t GpuCTC<ProbT>::launch_alpha_beta_kernels(const ProbT* const probs,
    ...
    if (compute_alpha)
        compute_alpha_kernel<ProbT, NT, VT><<<grid_size, NT, 0, stream_>>>(
            probs, label_sizes_, utt_length_, repeats_, labels_without_blanks_,
            label_offsets_, labels_with_blanks_, alphas_, nll_forward_, stride,
            out_dim_, S_, T_, blank_label_);

    if (compute_beta) {
        compute_betas_and_grad_kernel<ProbT, NT,
                                      VT><<<grid_size, NT, 0, stream_>>>(
            probs, label_sizes_, utt_length_, repeats_, labels_with_blanks_,
            alphas_, nll_forward_, nll_backward_, grads, stride, out_dim_, S_,
            T_, blank_label_);
...
ctcStatus_t GpuCTC<ProbT>::create_metadata_and_choose_config(
    const int* const flat_labels, const int* const label_lengths,
    const int* const input_lengths, size_t& best_config) {
    ...
    int config_NT[num_configs] = {32, 64,  128, 64,  128, 32,
...
template <typename ProbT>
ctcStatus_t GpuCTC<ProbT>::launch_gpu_kernels(const ProbT* const probs,
...
ctcStatus_t GpuCTC<ProbT>::compute_probs(const ProbT* const activations) {
    cudaError_t cuda_status;
    cuda_status = cudaMemcpyAsync(probs_, activations,
    ...
    Tensor<mxnet::gpu, 2, ProbT> probs_handle(
        probs_, mshadow::Shape2(activation_cols_, out_dim_), &mxstream);
    Tensor<mxnet::gpu, 1, ProbT> denoms_handle(
        denoms_, mshadow::Shape1(activation_cols_), &mxstream);
    ...
    compute_probs_kernel<ProbT, VT><<<grid_size, NT, 0, stream_>>>(
        ctc_helper::exponential<ProbT>(), probs_, denoms_, out_dim_,
        num_elements);
...
ctcStatus_t GpuCTC<ProbT>::compute_cost_and_score(
    const ProbT* const activations, ProbT* grads, ProbT* costs,
    const int* const flat_labels, const int* const label_lengths,
    const int* const input_lengths, bool compute_alpha,
    bool compute_betas_and_grad) {
    size_t best_config;
    ...
    launch_gpu_kernels(probs_, grads, best_config, compute_alpha,
                       compute_betas_and_grad);

    cudaError_t cuda_status_mem, cuda_status_sync;
    cuda_status_mem =
        cudaMemcpyAsync(costs, nll_forward_, sizeof(ProbT) * minibatch_,
