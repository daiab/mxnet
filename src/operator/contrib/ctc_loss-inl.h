...
class CTCLossOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        get_workspace_size<real_t>(&label_lengths, &input_lengths,
...
class CTCLossProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(dshape[1], lshape[0]) << "The batch size for the labels and "
                                          "data arrays must be the same.";

        CHECK_GE(dshape[0], lshape[1])
            << "The max number of labels cannot exceed "
