...
class IFFTOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (!init_cufft_) {
            n_iffts = in_data[ifft::kData].shape_.ProdShape(
                0, in_data[ifft::kData].ndim() - 1);
            // remember that input is complex
            dim_ =
                in_data[ifft::kData].shape_[in_data[ifft::kData].ndim() - 1] /
                2;
        ...
        Tensor<xpu, 2, DType> data =
            in_data[ifft::kData].get_with_shape<xpu, 2, DType>(
                Shape2(n_iffts, dim_ * 2), s);
        Tensor<xpu, 2, DType> out =
            out_data[ifft::kOut].get_with_shape<xpu, 2, DType>(
        ...
        for (size_t idx = 0; idx < num_compute; ++idx) {
            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(data.dptr_ +
                                                       2 * idx * stride_));
            ...
            Assign(out.Slice(idx * param_.compute_size,
        ...
        if (remain_num > 0) {
            ...
            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(
                    data.dptr_ + 2 * num_compute * stride_));
            cufftComplex *out_tmp =
                reinterpret_cast<cufftComplex *>(complex_data.dptr_);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> gdata =
            in_grad[ifft::kData].get_with_shape<xpu, 2, DType>(
                Shape2(n_iffts, dim_ * 2), s);
        Tensor<xpu, 2, DType> grad =
            out_grad[ifft::kOut].get_with_shape<xpu, 2, DType>(
                Shape2(n_iffts, dim_), s);
        ...
        for (size_t idx = 0; idx < num_compute; ++idx) {
            complex_data = complex_pad_imag(
                grad.Slice(idx * param_.compute_size,
        ...
        if (remain_num > 0) {
            ...
            complex_data = Tensor<xpu, 2, DType>(
                workspace.dptr_, Shape2(remain_num, dim_ * 2), s);
            complex_data = complex_pad_imag(
                grad.Slice(num_compute * param_.compute_size,
                           num_compute * param_.compute_size + remain_num));

            cufftComplex *in_tmp = const_cast<cufftComplex *>(
                reinterpret_cast<const cufftComplex *>(complex_data.dptr_));
            cufftComplex *out_tmp = reinterpret_cast<cufftComplex *>(
                gdata.dptr_ + 2 * num_compute * stride_);
...
class IFFTProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "_contrib_ifft"; }
