    ...
    DMLC_DECLARE_PARAMETER(SliceChannelParam) {
        ...
        DMLC_DECLARE_FIELD(squeeze_axis)
            ...
            .describe(
                ...
                "the output arrays."
                " **Note** that setting `squeeze_axis` to ``true`` removes "
                "axis with length 1"
...
class SliceChannelOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        for (int i = real_axis + 1; i < in_data[slice_enum::kData].ndim();
             ++i) {
        ...
        for (int i = 0; i < size_; ++i) {
            outputs[i] =
                out_data[i].get_with_shape<xpu, 3, DType>(slice_shape, s);
        }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        for (int i = real_axis + 1; i < in_grad[slice_enum::kData].ndim();
             ++i) {
        ...
        for (int i = 0; i < size_; ++i) {
            grad_out[i] =
                out_grad[i].get_with_shape<xpu, 3, DType>(slice_shape, s);
        }
...
class SliceChannelProp : public OperatorProperty {
   public:
    ...
    int NumOutputs() const override { return param_.num_outputs; }
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.squeeze_axis && ishape[real_axis] != 0) {
            CHECK_EQ(ishape[real_axis], static_cast<size_t>(param_.num_outputs))
                << "If squeeze axis is True, the size of the sliced axis must "
                   "be the same as num_outputs."
                << " Input shape=" << ishape << ", axis=" << real_axis
                << ", num_outputs=" << param_.num_outputs << ".";
        }
        dshape[real_axis] /= param_.num_outputs;
        if (param_.squeeze_axis &&
            (dshape[real_axis] == 1 || ishape[real_axis] == 0)) {
            for (int d = real_axis; d < static_cast<int>(dshape.ndim()) - 1;
                 ++d) {
                dshape[d] = dshape[d + 1];
            }
        ...
        for (int i = 0; i < param_.num_outputs; ++i) {
            ...
            if (param_.squeeze_axis && (dshape.ndim() == ishape.ndim() - 1)) {
                ...
                for (int d = real_axis + 1; d < static_cast<int>(ishape.ndim());
                     ++d) {
            ...
            SHAPE_ASSIGN_CHECK(*in_shape, slice_enum::kData,
                               back_calculate_dshape);
        }
        return true;
    }
    ...
    std::string TypeString() const override { return "SliceChannel"; }
