...
struct GridGeneratorParam : public dmlc::Parameter<GridGeneratorParam> {
    ...
    DMLC_DECLARE_PARAMETER(GridGeneratorParam) {
        ...
        DMLC_DECLARE_FIELD(transform_type)
            ...
            .describe(
                "The type of transformation. For `affine`, input data should "
                "be an affine matrix "
                "of size (batch, 6). For `warp`, input data should be an "
                "optical flow of size "
                "(batch, 2, h, w).");
        DMLC_DECLARE_FIELD(target_shape)
            ...
            .describe(
                ...
                "`affine`. If transformation type is `warp`, this parameter is "
                "ignored.");
    }
};
...
class GridGeneratorOp : public Operator {
   public:
    explicit GridGeneratorOp(GridGeneratorParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        switch (param_.transform_type) {
            case grid::kAffine: {
                // if transform_type is affine, data is affine matrix, input
                // shape : (batch, 2, 3)
                Tensor<xpu, 2, DType> out =
                    out_data[grid::kOut].get_with_shape<xpu, 2, DType>(
                        Shape2(out_data[grid::kOut].shape_[0] * 2,
                               out_data[grid::kOut].shape_[2] *
                                   out_data[grid::kOut].shape_[3]),
                        s);
                Tensor<xpu, 2, DType> grid_dst =
                    out_data[grid::kGridDst].get<xpu, 2, DType>(s);
                Shape<2> data_shape =
                    Shape2(out_data[grid::kOut].shape_[0] * 2, 3);
                Tensor<xpu, 2, DType> data =
                    in_data[grid::kData].get_with_shape<xpu, 2, DType>(
                        data_shape, s);
                // x, y, 1
                grid_dst[0] = range<DType>(0, grid_dst.shape_[1]);
                grid_dst[0] =
                    grid_dst[0] -
                    tcast<DType>(tcast<int>(
                        grid_dst[0] / scalar<DType>(param_.target_shape[1]))) *
                ...
                grid_dst[1] =
                    scalar<DType>(-1.0) +
                    tcast<DType>(tcast<int>(
                        grid_dst[1] / scalar<DType>(param_.target_shape[1]))) *
            ...
            case grid::kWarp: {
                // if transform_type is warp, data is optical flow, input shape
                // : (batch, 2, height, width)
                // grid_src = grid_dst + optical flow
                Tensor<xpu, 4, DType> data =
                    in_data[grid::kData].get<xpu, 4, DType>(s);
                Tensor<xpu, 4, DType> out =
                    out_data[grid::kOut].get<xpu, 4, DType>(s);
                // grid_dst : (2, H, W)
                Tensor<xpu, 3, DType> grid_dst =
                    out_data[grid::kGridDst].get<xpu, 3, DType>(s);
                Tensor<xpu, 2, DType> workspace =
                    ctx.requested[grid::kTempSpace]
                        .get_space_typed<xpu, 2, DType>(Shape2(2, 1), s);
                grid_dst[0] =
                    repmat(range<DType>(0, data.size(3)), data.size(2));
                grid_dst[1] =
                    reshape(range<DType>(0, data.size(2), 1, data.size(3)),
        ...
        switch (param_.transform_type) {
            case grid::kAffine: {
                Tensor<xpu, 2, DType> grid_dst =
                    out_data[grid::kGridDst].get<xpu, 2, DType>(s);
                Shape<2> data_shape =
                    Shape2(in_grad[grid::kData].shape_[0] * 2, 3);
                Tensor<xpu, 2, DType> gdata =
                    in_grad[grid::kData].get_with_shape<xpu, 2, DType>(
                        data_shape, s);
                Shape<2> grad_shape =
                    Shape2(out_grad[grid::kOut].shape_[0] * 2,
                           param_.target_shape[0] * param_.target_shape[1]);
                Tensor<xpu, 2, DType> grad =
                    out_grad[grid::kOut].get_with_shape<xpu, 2, DType>(
                        grad_shape, s);
            ...
            case grid::kWarp: {
                Tensor<xpu, 4, DType> grad =
                    out_grad[grid::kOut].get<xpu, 4, DType>(s);
                Tensor<xpu, 4, DType> gdata =
                    in_grad[grid::kData].get<xpu, 4, DType>(s);
                Tensor<xpu, 2, DType> workspace =
                    ctx.requested[grid::kTempSpace]
                        .get_space_typed<xpu, 2, DType>(Shape2(2, 1), s);
                workspace[0] =
                    scalar<DType>((DType(gdata.size(3)) - 1.0) / 2.0);
                workspace[1] =
                    scalar<DType>((DType(gdata.size(2)) - 1.0) / 2.0);
                Assign(
                    gdata, req[grid::kData],
...
class GridGeneratorProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        switch (param_.transform_type) {
            case grid::kAffine: {
                ...
                CHECK_EQ(lshape[1], 6U)
                    << "incorrect data shape[1], should be 6";
                ...
                out_shape->push_back(Shape4(lshape[0], 2,
                                            param_.target_shape[0],
                                            param_.target_shape[1]));
                out_shape->push_back(
                    Shape2(3, param_.target_shape[0] * param_.target_shape[1]));
                break;
            }
            case grid::kWarp: {
                ...
                CHECK_EQ(lshape[1], 2U)
                    << "incorrect data shape[1], should be 2";
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        if (dtype == -1) {
            LOG(FATAL)
                << "Not enough information to infer type in GridGenerator.";
            return false;
        }
    ...
    std::string TypeString() const override { return "GridGenerator"; }
