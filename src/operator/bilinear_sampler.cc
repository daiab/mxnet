    ...
    for (index_t n = 0; n < o_n; ++n) {
        for (index_t h = 0; h < o_h; ++h) {
            for (index_t w = 0; w < o_w; ++w) {
                ...
                // calc grad of grid
                *(grad_grid + grid_src_index + o_h * o_w) +=
...
Operator *BilinearSamplerProp::CreateOperatorEx(
...
MXNET_REGISTER_OP_PROPERTY(BilinearSampler, BilinearSamplerProp)
    ...
    .add_argument("grid", "NDArray-or-Symbol",
    ...
    .describe(
        ...
        "output[batch, channel, y_dst, x_dst] = G(data[batch, channel, y_src, "
        "x_src)\n    "
        ...
        "The out-boundary points will be padded as zeros. (The boundary is "
        "defined to be [-1, 1])\n"
        "The shape of output will be (data.shape[0], data.shape[1], "
