...
class MKLPoolingOp : public Operator {
   public:
    static std::string getName() { return "MKLPoolingOp"; }
   ...
   private:
    void LayerSetUp(const mshadow::Tensor<xpu, 4, DType> &data,
        ...
        if (global_pooling_) {
            CHECK(pad_h_ == 0 && pad_w_ == 0 && stride_h_ == 1 &&
                  stride_w_ == 1)
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        bottom_data = reinterpret_cast<void *>(
            mkl_prv_data<DType>(in_data[pool_enum::kData]));
#endif
        ...
        if (NULL == bottom_data) {
            ...
            if (NULL == poolingFwd) {
                status = dnnPoolingCreateForward<DType>(
                    &poolingFwd, NULL, algorithm, fwd_bottom_data->layout_usr,
                    kernel_size, kernel_stride, src_offset, border_type);
                CHECK_EQ(status, E_SUCCESS);
                // Now create poolingBwd
                status = dnnPoolingCreateBackward<DType>(
                    &poolingBwd, NULL, algorithm, fwd_bottom_data->layout_usr,
                    kernel_size, kernel_stride, src_offset, border_type);
                CHECK_EQ(status, E_SUCCESS);
            }
        }
        ...
        if (NULL != bottom_data) {
            if (NULL == poolingFwd) {
                std::shared_ptr<MKLMemHolder> bottom_data_mem =
                    in_data[pool_enum::kData].Mkl_mem_;
                ...
                status = dnnPoolingCreateForward<DType>(
                    &poolingFwd, NULL, algorithm, fwd_bottom_data->layout_int,
                    kernel_size, kernel_stride, src_offset, border_type);
                CHECK_EQ(status, E_SUCCESS);
                fwd_top_data->create_internal_layout(poolingFwd,
                                                     dnnResourceDst);

                // Now create poolingBwd
                status = dnnPoolingCreateBackward<DType>(
                    &poolingBwd, NULL, algorithm, fwd_bottom_data->layout_int,
                    kernel_size, kernel_stride, src_offset, border_type);
                CHECK_EQ(status, E_SUCCESS);
                bwd_top_diff->create_internal_layout(poolingFwd,
                                                     dnnResourceDst);
                bwd_bottom_diff->create_internal_layout(poolingFwd,
                                                        dnnResourceSrc);
            }
        }
#endif
        ...
        if (first_pass) {
            ...
            status = dnnAllocateBuffer<DType>(
                reinterpret_cast<void **>(&max_idx_data), max_idx_datal);
            CHECK_EQ(status, E_SUCCESS);
            ...
            bwd_top_diff->create_internal_layout(poolingBwd,
                                                 dnnResourceDiffDst);
            bwd_bottom_diff->create_internal_layout(poolingBwd,
                                                    dnnResourceDiffSrc);
#endif
   ...
   private:
    size_t kernel_size[2], kernel_stride[4];
