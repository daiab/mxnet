...
class MKLFullyConnectedOp : public Operator {
   public:
    ...
    static std::string getName() { return "MKLFullyConnectedOp"; }
   ...
   private:
    void LayerSetUp(const std::vector<TShape> &in_shapes,
        ...
        const size_t dim = 4;
        const size_t src_sizes[4] = {1, 1, ishape.ProdShape(1, ishape.ndim()),
                                     ishape[0]};
        ...
        if (!param_.no_bias) {
            MKLDNN_CALL(dnnInnerProductCreateForwardBias<DType>(
                &fullyConnectedFwd, attributes, dim, src_sizes,
                output_channels));
        } else {
        ...
        MKLDNN_CALL(dnnInnerProductCreateBackwardData<DType>(
            &fullyConnectedBwdData, attributes, dim, src_sizes,
            output_channels));
        MKLDNN_CALL(dnnInnerProductCreateBackwardFilter<DType>(
            &fullyConnectedBwdFilter, attributes, dim, src_sizes,
            output_channels));
        if (!param_.no_bias) {
            MKLDNN_CALL(dnnInnerProductCreateBackwardBias<DType>(
                &fullyConnectedBwdBias, attributes, 2, dst_sizes));
        }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (!param_.no_bias) {
            res_fullyConnected[dnnResourceBias] =
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        MKLDNN_CALL(
            dnnExecute<DType>(fullyConnectedBwdFilter, res_fullyConnected));
        if (!param_.no_bias) {
            MKLDNN_CALL(
                dnnExecute<DType>(fullyConnectedBwdBias, res_fullyConnected));
        }
        MKLDNN_CALL(
            dnnExecute<DType>(fullyConnectedBwdData, res_fullyConnected));
    }
