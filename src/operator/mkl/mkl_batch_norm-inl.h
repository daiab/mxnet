...
class MKLBatchNormOp : public Operator {
   public:
    ...
    virtual ~MKLBatchNormOp() {
        ...
        if (batchNormFwdTraining != NULL)
            dnnDelete<DType>(batchNormFwdTraining);
        if (batchNormBwdScaleShift != NULL)
            dnnDelete<DType>(batchNormBwdScaleShift);
        dnnLayoutDelete<DType>(layout_usr_);
        if (scaleShift_space.dptr) Storage::Get()->Free(scaleShift_space);
        if (scaleShiftDiff_space.dptr)
            Storage::Get()->Free(scaleShiftDiff_space);
    }
    static std::string getName() { return "MKLBatchNormOp"; }
   ...
   private:
    void LayerSetUp(const mshadow::Tensor<xpu, 4, DType> &data,
        ...
        scaleShift_space =
            Storage::Get()->Alloc(scaleShift_size, Context::CPU());
        scaleShiftDiff_space =
            Storage::Get()->Alloc(scaleShift_size, Context::CPU());
        DType *scaleShift_buf =
            reinterpret_cast<DType *>(scaleShift_space.dptr);
        /*!use_weight_bias_*/
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        if (in_data[batchnorm::kData].ndim() == 2) {
            ...
            data = mkl_experimental_direct_get<xpu, 4, DType>(
                in_data[batchnorm::kData], s);
            out = mkl_experimental_direct_get<xpu, 4, DType>(
                out_data[batchnorm::kOut], s);
        }
        ...
        Tensor<xpu, 1, DType> slope =
            in_data[batchnorm::kGamma].get<xpu, 1, DType>(s);
        Tensor<xpu, 1, DType> bias =
            in_data[batchnorm::kBeta].get<xpu, 1, DType>(s);
        ...
        bottom_data = reinterpret_cast<void *>(
            mkl_prv_data<DType>(in_data[batchnorm::kData]));
#endif
        ...
        if (NULL != bottom_data) {
            ...
            if (batchNormFwdInference == NULL) {
                std::shared_ptr<MKLMemHolder> bottom_data_mem =
                    in_data[batchnorm::kData].Mkl_mem_;
                ...
                fwd_top_data->create_internal_layout(batchNormFwdInference,
                                                     dnnResourceDst);
                bwd_top_diff->create_internal_layout(batchNormFwdInference,
                                                     dnnResourceDst);
                bwd_bottom_diff->create_internal_layout(batchNormFwdInference,
                                                        dnnResourceSrc);
        ...
        if (NULL == bottom_data) {
            if (batchNormFwdInference == NULL) {
                ...
                e = dnnBatchNormalizationCreateForward_v2<DType>(
                    &batchNormFwdTraining, NULL, layout_usr_, eps_,
                    dnnUseScaleShift);
                CHECK_EQ(e, E_SUCCESS);
        ...
        BatchNorm_res[dnnResourceDst] = fwd_top_data->get_output_ptr(
            out.dptr_, fwd_top_data, out_data[batchnorm::kOut]);
        if (ctx.is_train && !param_.use_global_stats) {
            Tensor<xpu, 1, DType> mean =
                out_data[batchnorm::kMean].get<xpu, 1, DType>(s);
            Tensor<xpu, 1, DType> var =
                out_data[batchnorm::kVar].get<xpu, 1, DType>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (in_data[batchnorm::kData].ndim() == 2) {
            ...
            data = mkl_experimental_direct_get<xpu, 4, DType>(
                in_data[batchnorm::kData], s);
            grad = mkl_experimental_direct_get<xpu, 4, DType>(
                out_grad[batchnorm::kOut], s);
            grad_in = mkl_experimental_direct_get<xpu, 4, DType>(
                in_grad[batchnorm::kData], s);
        }

        Tensor<xpu, 1, DType> slope =
            in_data[batchnorm::kGamma].get<xpu, 1, DType>(s);
        Tensor<xpu, 1, DType> gslope =
            in_grad[batchnorm::kGamma].get<xpu, 1, DType>(s);
        Tensor<xpu, 1, DType> gbias =
            in_grad[batchnorm::kBeta].get<xpu, 1, DType>(s);
        Tensor<xpu, 1, DType> mean =
            out_data[batchnorm::kMean].get<xpu, 1, DType>(s);
        Tensor<xpu, 1, DType> var =
            out_data[batchnorm::kVar].get<xpu, 1, DType>(s);
        ...
        if (ctx.is_train && !param_.use_global_stats) {
            ...
            float *moving_mean_ptr =
                reinterpret_cast<float *>(moving_mean.dptr_);
        ...
        BatchNorm_res[dnnResourceDiffSrc] = bwd_bottom_diff->get_output_ptr(
            grad_in.dptr_, bwd_bottom_diff, in_grad[batchnorm::kData]);
        BatchNorm_res[dnnResourceDiffDst] = bwd_top_diff->get_converted_prv(
            grad.dptr_, true, out_grad[batchnorm::kOut]);
...
#endif
        DType *scaleShiftDiff_buf =
            reinterpret_cast<DType *>(scaleShiftDiff_space.dptr);
        if (!param_.fix_gamma) {
