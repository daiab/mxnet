...
void MKLElementWiseSumCompute_(const nnvm::NodeAttrs &attrs,
    ...
    eltwise_res[dnnResourceMultipleSrc] = reinterpret_cast<void *>(
        reinterpret_cast<void *>(in_data[in_place_idx].dptr_));
    for (size_t i = 1; i < size; ++i) {
        if (i == in_place_idx) continue;
        eltwise_res[dnnResourceMultipleSrc + i] = reinterpret_cast<void *>(
            reinterpret_cast<void *>(in_data[i].dptr_));
    }
