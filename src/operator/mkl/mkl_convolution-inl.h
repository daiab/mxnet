...
class MKLConvolutionOp : public Operator {
   public:
    static std::string getName() { return "MKLConvolutionOp"; }
    void SetupBuffer() {
    ...
    virtual ~MKLConvolutionOp() { ReleaseBuffer(); }
   ...
   private:
    void LayerSetUp(const mshadow::Tensor<xpu, 4, DType> &data,
        ...
        * becomes 5D, i.e. groups become a separate dimension */
        ...
        size_t fdata_strides[5] = {1, kw, kw * kh, kw * kh * ic / g,
                                   kw * kh * ic / g * oc / g};
        ...
        if (!param_.no_bias) {
            status = dnnGroupsConvolutionCreateForwardBias<DType>(
                &convolutionFwd, NULL, dnnAlgorithmConvolutionDirect, g,
                dimension, bdata_sizes, tdata_sizes, fdata_sizes,
                convolutionStrides, inputOffset, dnnBorderZeros);
        } else {
        ...
        */
        status = dnnGroupsConvolutionCreateBackwardData<DType>(
            &convolutionBwdData, NULL, dnnAlgorithmConvolutionDirect, g,
            dimension, bdata_sizes, tdata_sizes, fdata_sizes,
            convolutionStrides, inputOffset, dnnBorderZeros);
        CHECK_EQ(status, 0)
        ...
        */
        status = dnnGroupsConvolutionCreateBackwardFilter<DType>(
            &convolutionBwdFilter, NULL, dnnAlgorithmConvolutionDirect, g,
            dimension, bdata_sizes, tdata_sizes, fdata_sizes,
            convolutionStrides, inputOffset, dnnBorderZeros);
        CHECK_EQ(status, 0)
        ...
        bwdf_top_diff->create_layouts(convolutionBwdFilter, dnnResourceDiffDst,
                                      dimension, tdata_sizes, tdata_strides);
        bwdf_filter_diff->create_layouts(convolutionBwdFilter,
        ...
        */
        if (!param_.no_bias) {
            status = dnnGroupsConvolutionCreateBackwardBias<DType>(
                &convolutionBwdBias, NULL, dnnAlgorithmConvolutionDirect, g,
                dimension, tdata_sizes);
            CHECK_EQ(status, 0)
                << "Failed dnnConvolutionCreateBackwardBias with status "
                << status << "\n";
            bwdb_top_diff->create_layouts(convolutionBwdBias,
                                          dnnResourceDiffDst, dimension,
                                          tdata_sizes, tdata_strides);
            bwdb_bias_diff->create_layouts(convolutionBwdBias,
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> wmat = mkl_experimental_direct_get<xpu, 4, DType>(
            in_data[conv::kWeight], s);
        if (!init_mkldnn_) {
            LayerSetUp(data, out);
            init_mkldnn_ = true;
        }
        ...
        res_convolutionFwd[dnnResourceSrc] = fwd_bottom_data->get_converted_prv(
            data_ptr, false, in_data[conv::kData]);
        res_convolutionFwd[dnnResourceFilter] =
            fwd_filter_data->get_converted_prv(wmat_ptr, true,
                                               in_data[conv::kWeight]);
        if (!param_.no_bias) {
            Tensor<xpu, 1, DType> bias =
                mkl_experimental_direct_get<xpu, 1, DType>(in_data[conv::kBias],
                                                           s);
            res_convolutionFwd[dnnResourceBias] =
                fwd_bias_data->get_converted_prv(bias.dptr_, true,
                                                 in_data[conv::kBias]);
        }

        res_convolutionFwd[dnnResourceDst] = fwd_top_data->get_output_ptr(
            out_ptr, fwd_top_data, out_data[conv::kOut]);
        status = dnnExecute<DType>(convolutionFwd, res_convolutionFwd);
        CHECK_EQ(status, 0) << "Forward convolution failed with status "
                            << status;
    ...
    void AddToModeAllocAndStoreBuffer(void *src, int blob_size,
                                      Storage::Handle *pws) {
    ...
    void AddToModeAddAndReleaseBuffer(Storage::Handle *pws, void *dst_,
                                      int blob_size) {
...
#pragma omp parallel for
        ...
        if (pws->dptr) Storage::Get()->Free(*pws);
        pws->dptr = NULL;
    }
    virtual void Backward(const OpContext &ctx,
        ...
        if (req[0]) {
            ...
            res_convolutionBwdData[dnnResourceDiffDst] =
                bwdd_top_diff->get_converted_prv(grad.dptr_, true,
                                                 out_grad[conv::kOut]);
            ...
            if (req[0] == kAddTo) {
                ...
                AddToModeAllocAndStoreBuffer(
                    gdata.dptr_, in_grad[conv::kData].Size(), &addtoWorkspace);
            }

            res_convolutionBwdData[dnnResourceDiffSrc] =
                bwdd_bottom_diff->get_output_ptr(gdata.dptr_, bwdd_bottom_diff,
                                                 in_grad[conv::kData]);
            status =
                dnnExecute<DType>(convolutionBwdData, res_convolutionBwdData);
            CHECK_EQ(status, 0) << "Backward Data conv failed with status "
                                << status;
#if MKL_EXPERIMENTAL == 0
            ...
            if (req[0] == kAddTo) {
                ...
                AddToModeAddAndReleaseBuffer(&addtoWorkspace, gdata.dptr_,
        ...
        if (req[1]) {
            ...
            res_convolutionBwdFilter[dnnResourceDiffDst] =
                bwdf_top_diff->get_converted_prv(grad.dptr_, true,
                                                 out_grad[conv::kOut]);
            ...
            if (req[1] == kAddTo) {
                // wait mkl support addto mode
                AddToModeAllocAndStoreBuffer(gwmat.dptr_,
                                             in_grad[conv::kWeight].Size(),
                                             &addtoWorkspace);
            }

            res_convolutionBwdFilter[dnnResourceDiffFilter] =
                bwdf_filter_diff->get_output_ptr(gwmat.dptr_, bwdf_filter_diff,
                                                 in_grad[conv::kWeight]);
            status = dnnExecute<DType>(convolutionBwdFilter,
                                       res_convolutionBwdFilter);
            CHECK_EQ(status, 0) << "Backward Filter conv failed with status "
                                << status;
#if MKL_EXPERIMENTAL == 0
            ...
            if (req[1] == kAddTo) {
                ...
                AddToModeAddAndReleaseBuffer(&addtoWorkspace, gwmat.dptr_,
                                             in_grad[conv::kWeight].Size());
            }
        }
        if (!param_.no_bias) {
            Tensor<xpu, 1, DType> gbias =
                mkl_experimental_direct_get<xpu, 1, DType>(in_grad[conv::kBias],
                                                           s);
            void *res_convolutionBwdBias[dnnResourceNumber];
            res_convolutionBwdBias[dnnResourceDiffDst] =
                bwdb_top_diff->get_converted_prv(grad.dptr_, true,
                                                 out_grad[conv::kOut]);

            res_convolutionBwdBias[dnnResourceDiffBias] =
                bwdb_bias_diff->get_output_ptr(gbias.dptr_, bwdb_bias_diff,
                                               in_grad[conv::kBias]);
            status =
                dnnExecute<DType>(convolutionBwdBias, res_convolutionBwdBias);
            CHECK_EQ(status, 0) << "Backward Bias failed with status "
                                << status;
#if MKL_EXPERIMENTAL == 0
   ...
   private:
    ...
    size_t width_, height_, width_out_, height_out_, kernel_w_, kernel_h_,
        stride_w_, stride_h_;
    int group_, num_, num_output_;
    size_t channels_;
    int pad_w_, pad_h_;
    bool init_mkldnn_;
    ...
    /* Fwd step */
    std::shared_ptr<MKLData<DType> > fwd_bottom_data, fwd_top_data,
    ...
    std::shared_ptr<MKLData<DType> > bwdf_filter_diff_iter,
        bwdf2fwd_filter_diff, bwdb_bias_diff_iter;
    /* Bwd bias step */
