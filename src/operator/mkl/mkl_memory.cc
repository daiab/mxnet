...
void MKLMemoryDescriptorBase<Dtype>::create_conversions() {
    ...
    if (layout_int && !dnnLayoutCompare<Dtype>(layout_usr, layout_int)) {
        CHECK(layout_usr);
        status =
            dnnConversionCreate<Dtype>(&convert_to_int, layout_usr, layout_int);
        CHECK_EQ(status, E_SUCCESS)
            << "Failed creation convert_to_int with status " << status
            << " for buffer: " << this->name << "\n";
        status = dnnConversionCreate<Dtype>(&convert_from_int, layout_int,
                                            layout_usr);
        CHECK_EQ(status, E_SUCCESS)
            << "Failed creation convert_from_int with status " << status
            << " for buffer: " << this->name << "\n";
    }
}
...
void MKLMemoryDescriptorBase<Dtype>::create_layouts(
    const dnnPrimitive_t primitive, dnnResourceType_t type, size_t dimension,
...
bool MKLMemoryDescriptorBase<Dtype>::layout_compare(
    ...
    CHECK_EQ(other->get_descr_type(), PrvMemDescr::PRV_DESCR_MKL2017);
    std::shared_ptr<MKLMemoryDescriptorBase<Dtype> > other_descr =
        std::static_pointer_cast<MKLMemoryDescriptorBase<Dtype> >(other);

    if (dnnLayoutCompare<Dtype>(other_descr->layout_int, this->layout_int))
        return true;
    else
        return false;
}
...
void MKLMemoryDescriptorBase<Dtype>::convert_from_other(
    ...
    status = dnnConversionCreate<Dtype>(&convert, other_descr->layout_int,
                                        this->layout_int);
...
template <typename Dtype>
Dtype *MKLMemoryDescriptor<Dtype>::get_converted_prv(Dtype *cpu_ptr,
                                                     bool set_prv_ptr,
                                                     const TBlob &blob) {
    Dtype *prv_ptr = NULL;
        ...
        if (prv_ptr != NULL) {
            ...
            if (!dnnLayoutCompare<Dtype>(current_descr->layout_int,
                ...
                if (this->convert_prv2prv) {
                    ...
                    status = dnnConversionCreate<Dtype>(
                        &this->convert_prv2prv, current_descr->layout_int,
                        this->layout_int);
                    if (status == 0)
                ...
                if (status != 0) {
                    ...
                    status = dnnExecute<Dtype>(this->convert_to_int,
                                               convert_resources);
                    CHECK_EQ(status, 0) << "Conversion failed with status "
                                        << status;
                } else {
                    this->allocate();
                    convert_resources[dnnResourceFrom] =
                        reinterpret_cast<void *>(prv_ptr);
                    convert_resources[dnnResourceTo] =
                        reinterpret_cast<void *>(this->internal_ptr);
                    status = dnnExecute<Dtype>(this->convert_prv2prv,
                                               convert_resources);
                    CHECK_EQ(status, 0) << "Conversion failed with status "
                                        << status;
                }
...
template <typename Dtype>
void *MKLMemoryDescriptor<Dtype>::get_output_ptr(
    Dtype *data_ptr, std::shared_ptr<MKLMemoryDescriptor<Dtype> > self_ptr,
    const TBlob &blob, bool in_place) {
#if MKL_EXPERIMENTAL == 1
    ...
    if (this->conversion_needed()) {
        ...
        if (!in_place) {
            ...
            if (blob_prv != NULL) return blob_prv;
        }
#endif
        return prv_ptr;
    } else {
