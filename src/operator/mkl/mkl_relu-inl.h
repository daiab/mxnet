...
class MKLReluOp : public Operator {
   public:
    static std::string getName() { return "MKLReluOp"; }
    MKLReluOp() : reluFwd_(NULL), reluBwd_(NULL) {
        init_mkldnn_ = false;
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        if (in_data[activation::kData].ndim() == 2) {
            Shape<4> dshape =
                Shape4(in_data[activation::kData].shape_[0],
            ...
            data = mkl_experimental_direct_get<xpu, 4, DType>(
                in_data[activation::kData], s);
            out = mkl_experimental_direct_get<xpu, 4, DType>(
                out_data[activation::kOut], s);
        }
        if (!init_mkldnn_) {
            LayerSetUp(data, out);
            init_mkldnn_ = true;
        ...
        bottom_data = reinterpret_cast<void *>(
            mkl_prv_data<DType>(in_data[activation::kData]));
#endif
#if MKL_EXPERIMENTAL == 1
        if (bottom_data != NULL) {
            if (reluFwd_ == NULL) {
                std::shared_ptr<MKLData<DType> > mem_descr =
                    mkl_get_mem_desc<DType>(
                        in_data[activation::kData].Mkl_mem_);
                DType negative_slope = 0;
                dnnError_t e;
                e = dnnReLUCreateForward<DType>(
                    &reluFwd_, NULL, mem_descr->layout_int, negative_slope);
                CHECK_EQ(e, E_SUCCESS);
                e = dnnReLUCreateBackward<DType>(
                    &reluBwd_, NULL, mem_descr->layout_int,
                ...
                bwd_bottom_diff_->create_internal_layout(reluFwd_,
                                                         dnnResourceSrc);
            }
        }
#endif
        if (bottom_data == NULL) {
            ...
            if (reluFwd_ == NULL) {
                ...
                e = dnnReLUCreateForward<DType>(&reluFwd_, NULL,
                                                fwd_bottom_data_->layout_usr,
                                                negative_slope);
                CHECK_EQ(e, E_SUCCESS);
                e = dnnReLUCreateBackward<DType>(
                    &reluBwd_, NULL, fwd_bottom_data_->layout_usr,
        ...
        relu_res[dnnResourceDst] = fwd_top_data_->get_output_ptr(
            out.dptr_, fwd_top_data_, out_data[activation::kOut],
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (out_grad[activation::kOut].ndim() == 2) {
            Shape<4> dshape =
                Shape4(out_grad[activation::kOut].shape_[0],
            ...
            m_out_grad = mkl_experimental_direct_get<xpu, 4, DType>(
                out_grad[activation::kOut], s);
            m_out_data = mkl_experimental_direct_get<xpu, 4, DType>(
                out_data[activation::kOut], s);
            m_in_grad = mkl_experimental_direct_get<xpu, 4, DType>(
                in_grad[activation::kData], s);
        }
        dnnError_t e;
        void *relu_res[dnnResourceNumber];
...
#if MKL_EXPERIMENTAL == 1
        bottom_data = reinterpret_cast<void *>(
            mkl_prv_data<DType>(out_data[activation::kOut]));
#endif
        if (NULL == bottom_data) {
            bottom_data =
        ...
        relu_res[dnnResourceSrc] = bottom_data;
        relu_res[dnnResourceDiffDst] = bwd_top_diff_->get_converted_prv(
            m_out_grad.dptr_, true, out_grad[activation::kOut]);
