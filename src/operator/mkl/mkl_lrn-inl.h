...
class MKLLRNOp : public Operator {
   public:
    static std::string getName() { return "MKLLRNOp"; }
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        bottom_data = reinterpret_cast<void *>(
            mkl_prv_data<DType>(in_data[lrn_enum::kData]));
#endif
#if MKL_EXPERIMENTAL == 1
        if (NULL != bottom_data) {
            if (lrnFwd == NULL) {
                ...
                e = dnnLRNCreateForward<DType>(&lrnFwd, NULL,
                                               fwd_bottom_data_->layout_int,
                                               size_, alpha_, beta_, k_);
                CHECK_EQ(e, E_SUCCESS);

                fwd_top_data_->create_internal_layout(lrnFwd, dnnResourceDst);

                e = dnnLRNCreateBackward<DType>(
                    &lrnBwd, NULL, fwd_bottom_data_->layout_int,
                    fwd_bottom_data_->layout_int, size_, alpha_, beta_, k_);
                CHECK_EQ(e, E_SUCCESS);

                e = dnnLayoutCreateFromPrimitive<DType>(&lrn_buffer_l, lrnFwd,
                ...
                bwd_top_diff_->create_internal_layout(lrnBwd,
                                                      dnnResourceDiffDst);
                bwd_bottom_diff_->create_internal_layout(lrnBwd,
                                                         dnnResourceDiffSrc);
            }
        }
#endif
        if (bottom_data == NULL) {
            if (lrnFwd == NULL) {
                ...
                e = dnnLRNCreateBackward<DType>(
                    &lrnBwd, NULL, fwd_bottom_data_->layout_usr,
                    fwd_bottom_data_->layout_usr, size_, alpha_, beta_, k_);
                CHECK_EQ(e, E_SUCCESS);
            }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad_in =
            mkl_experimental_direct_get<xpu, 4, DType>(in_grad[lrn_enum::kData],
                                                       s);
        dnnError_t e;
        void *lrn_res[dnnResourceNumber];
        lrn_res[dnnResourceDiffDst] = bwd_top_diff_->get_converted_prv(
            grad.dptr_, true, out_grad[lrn_enum::kOut]);
        lrn_res[dnnResourceWorkspace] = lrn_buffer_;
        lrn_res[dnnResourceSrc] = fwd_bottom_data_->get_converted_prv(
            data.dptr_, false, in_data[lrn_enum::kData]);
