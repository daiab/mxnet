...
class MKLConcatOp : public Operator {
   public:
    static std::string getName() { return "MKLConcatOp"; }
    ...
    void LayerSetUp(const std::vector<mshadow::Tensor<xpu, 4, DType> > &data,
        ...
        for (size_t i = 0; i < num_concats_; ++i) {
            ...
            for (size_t d = 0; d < dim_src; ++d) {
                ...
                strides_src[d] =
                    (d == 0) ? 1 : strides_src[d - 1] * sizes_src[d - 1];
            }

            split_channels_[i] = data[i].shape_[1];
            channels_ += split_channels_[i];
            fwd_bottom_data_[i]->create_user_layout(dim_src, sizes_src,
                                                    strides_src);
            bwd_bottom_diff_[i]->create_user_layout(dim_src, sizes_src,
                                                    strides_src);
            delete[] sizes_src;
            delete[] strides_src;
        }
        ...
        for (size_t d = 0; d < dim_dst; ++d) {
            ...
            strides_dst[d] =
                (d == 0) ? 1 : strides_dst[d - 1] * sizes_dst[d - 1];
        }
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        if (in_data[0].ndim() == 2) {
            for (int i = 0; i < size_; ++i) {
                Shape<4> dshape =
                    Shape4(in_data[i].shape_[0], in_data[i].shape_[1], 1, 1);
                data[i] = mkl_experimental_direct_get_with_shape<xpu, 4, DType>(
                    in_data[i], dshape, s);
            }
            Shape<4> dshape =
                Shape4(out_data[concat_enum::kOut].shape_[0],
            ...
            for (int i = 0; i < size_; ++i) {
                Shape<4> dshape =
                    Shape4(in_data[i].shape_[0], in_data[i].shape_[1],
            ...
            for (int i = 0; i < size_; ++i) {
                data[i] =
                    mkl_experimental_direct_get<xpu, 4, DType>(in_data[i], s);
            }
            out = mkl_experimental_direct_get<xpu, 4, DType>(
                out_data[concat_enum::kOut], s);
        }
        ...
        if (isFirstPass) {
            e = dnnConcatCreate<DType>(&concatFwd_, NULL, num_concats_,
                                       layouts);
            ...
            e = dnnSplitCreate<DType>(&concatBwd_, NULL, num_concats_,
                                      bwd_top_diff_->layout_int,
                                      split_channels_);
            CHECK_EQ(e, E_SUCCESS);

            for (size_t n = 0; n < num_concats_; ++n) {
                fwd_bottom_data_[n]->create_internal_layout(
                    concatFwd_,
                    (dnnResourceType_t)(dnnResourceMultipleSrc + n));
                bwd_bottom_diff_[n]->create_internal_layout(
                    concatBwd_,
        ...
        concat_res[dnnResourceDst] = fwd_top_data_->get_output_ptr(
            out.dptr_, fwd_top_data_, out_data[concat_enum::kOut]);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (in_grad[0].ndim() == 2) {
            Shape<4> dshape =
                Shape4(out_grad[concat_enum::kOut].shape_[0],
            ...
            for (int i = 0; i < size_; ++i) {
                dshape =
                    Shape4(in_grad[i].shape_[0], in_grad[i].shape_[1], 1, 1);
                grad_in[i] =
                    mkl_experimental_direct_get_with_shape<xpu, 4, DType>(
                        in_grad[i], dshape, s);
            }
            ...
            grad = mkl_experimental_direct_get_with_shape<xpu, 4, DType>(
                out_grad[concat_enum::kOut], dshape, s);
            for (int i = 0; i < size_; ++i) {
                dshape = Shape4(in_grad[i].shape_[0], in_grad[i].shape_[1],
            ...
            grad = mkl_experimental_direct_get<xpu, 4, DType>(
                out_grad[concat_enum::kOut], s);
            for (int i = 0; i < size_; ++i) {
                grad_in[i] =
                    mkl_experimental_direct_get<xpu, 4, DType>(in_grad[i], s);
            }
        }
        ...
        dnnError_t e;
        void *concat_res[dnnResourceNumber];
        concat_res[dnnResourceSrc] = bwd_top_diff_->get_converted_prv(
            grad.dptr_, true, out_grad[concat_enum::kOut]);
        for (size_t i = 0; i < num_concats_; ++i) {
            concat_res[dnnResourceMultipleDst + i] =
                bwd_bottom_diff_[i]->get_output_ptr(
