...
class CuDNNBatchNormOp : public Operator {
   public:
    explicit CuDNNBatchNormOp(BatchNormParam param) {
        ...
        dtype_param_ =
            (dtype_ == CUDNN_DATA_HALF) ? kFloat32 : DataType<DType>::kFlag;
    }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (!init_cudnn_) {
            ...
            CUDNN_CALL(cudnnSetTensor4dDescriptor(io_desc_, CUDNN_TENSOR_NCHW,
                                                  dtype_, shape_[0], shape_[1],
                                                  shape_[2], shape_[3]));
            CUDNN_CALL(cudnnDeriveBNTensorDescriptor(mean_desc_, io_desc_,
        ...
        Tensor<gpu, 4, DType> x =
            in_data[cudnnbatchnorm::kData].get_with_shape<gpu, 4, DType>(shape_,
                                                                         s);

        Tensor<gpu, 4, DType> y =
            out_data[cudnnbatchnorm::kOut].get_with_shape<gpu, 4, DType>(shape_,
                                                                         s);
            ...
            if (ctx.is_train) {
                Tensor<gpu, 1, DTypeParam> save_mean =
                    out_data[cudnnbatchnorm::kMean]
                        .get_with_shape<gpu, 1, DTypeParam>(Shape1(shape_[1]),
                                                            s);
                ...
                CUDNN_CALL(cudnnBatchNormalizationForwardTraining(
                    s->dnn_handle_, CUDNN_BATCHNORM_SPATIAL, &a, &b, io_desc_,
                    x.dptr_, io_desc_, y.dptr_, mean_desc_, gamma.dptr_,
                    beta.dptr_, 1 - param_.momentum, moving_mean.dptr_,
                    moving_inv_var.dptr_, param_.eps, save_mean.dptr_,
                    save_inv_var.dptr_));
            } else {
                CUDNN_CALL(cudnnBatchNormalizationForwardInference(
                    s->dnn_handle_, CUDNN_BATCHNORM_SPATIAL, &a, &b, io_desc_,
                    x.dptr_, io_desc_, y.dptr_, mean_desc_, gamma.dptr_,
                    beta.dptr_, moving_mean.dptr_, moving_inv_var.dptr_,
                    param_.eps));
            }
        })
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<gpu, 4, DType> dx =
            in_grad[cudnnbatchnorm::kData].get_with_shape<gpu, 4, DType>(shape_,
                                                                         s);
        Tensor<gpu, 4, DType> dy =
            out_grad[cudnnbatchnorm::kOut].get_with_shape<gpu, 4, DType>(shape_,
                                                                         s);
        ...
        MSHADOW_REAL_TYPE_SWITCH(dtype_param_, DTypeParam, {
            ...
            Tensor<gpu, 1, DTypeParam> dbeta =
                in_grad[cudnnbatchnorm::kBeta]
                    .get_with_shape<gpu, 1, DTypeParam>(Shape1(shape_[1]), s);
            Tensor<gpu, 1, DTypeParam> dgamma =
                in_grad[cudnnbatchnorm::kGamma]
                    .get_with_shape<gpu, 1, DTypeParam>(Shape1(shape_[1]), s);
            Tensor<gpu, 1, DTypeParam> save_mean =
                out_data[cudnnbatchnorm::kMean]
                    .get_with_shape<gpu, 1, DTypeParam>(Shape1(shape_[1]), s);
            ...
            CUDNN_CALL(cudnnBatchNormalizationBackward(
                s->dnn_handle_, CUDNN_BATCHNORM_SPATIAL, &a, &b, &a,
                req[cudnnbatchnorm::kGamma] == kWriteTo ? &b : &b_add, io_desc_,
                x.dptr_, io_desc_, dy.dptr_, io_desc_, dx.dptr_, mean_desc_,
                gamma.dptr_, dgamma.dptr_, dbeta.dptr_, param_.eps,
                save_mean.dptr_, save_inv_var.dptr_));
...
class CuDNNBatchNormProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "CuDNNBatchNorm"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[cudnnbatchnorm::kOut], out_data[cudnnbatchnorm::kMean],
                out_data[cudnnbatchnorm::kInvVar],
