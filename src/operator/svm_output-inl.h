    ...
    DMLC_DECLARE_PARAMETER(SVMOutputParam) {
        ...
        DMLC_DECLARE_FIELD(regularization_coefficient)
            ...
            .describe(
                ...
                "This balances the tradeoff between coefficient size and "
                "error.");
   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> data =
            in_data[svm_enum::kData].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> out =
            out_data[svm_enum::kOut].FlatTo2D<xpu, DType>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 1, DType> label =
            in_data[svm_enum::kLabel].get_with_shape<xpu, 1, DType>(
                Shape1(label_shape.ProdShape(0, label_shape.ndim())), s);
        Tensor<xpu, 2, DType> out =
            out_data[svm_enum::kOut].FlatTo2D<xpu, DType>(s);
        Tensor<xpu, 2, DType> grad =
            in_grad[svm_enum::kData].FlatTo2D<xpu, DType>(s);
        CHECK_EQ(grad.shape_, out.shape_) << "SVMOutputs: shape mismatch";

        if (param_.use_linear) {
            L1_SVM(DType(param_.margin),
                   DType(param_.regularization_coefficient), grad, label, out);
        } else {
            L2_SVM(DType(param_.margin),
                   DType(param_.regularization_coefficient), grad, label, out);
        }
    }
...
class SVMOutputProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "SVMOutput"; }
