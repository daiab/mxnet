...
inline int rnn_single_param_size(int inputSize, int hiddenSize, int mode) {
...
inline int rnn_param_size(int layerNum, int inputSize, int hiddenSize,
                          bool bidirectional, int mode) {
    // get size of first layer
    int size = rnn_single_param_size(inputSize, hiddenSize, mode);
    // get size of remaining layers
    if (bidirectional) {
        size += (layerNum - 1) *
                rnn_single_param_size(2 * hiddenSize, hiddenSize, mode);
        size *= 2;
    } else {
        size += (layerNum - 1) *
                rnn_single_param_size(hiddenSize, hiddenSize, mode);
    }
    return size;
}
...
struct RNNParam : public dmlc::Parameter<RNNParam> {
    ...
    DMLC_DECLARE_PARAMETER(RNNParam) {
        ...
        DMLC_DECLARE_FIELD(p)
            ...
            .set_range(0, 1)
            .describe(
                "Dropout probability, fraction of the input that gets dropped "
...
class RNNProp : public OperatorProperty {
   public:
    ...
    std::vector<std::string> ListOutputs() const override {
        ...
        if (param_.mode == rnn_enum::kLstm) outputs.push_back("state_cell");
        return outputs;
    }
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(dshape.ndim(), 3U) << "Input data should be rank-3 tensor of "
                                       "dim [sequence length, batch size, "
                                       "input size]";
        ...
        int total_layers =
            numDirections * param_.num_layers;  // double for bidirectional
        SHAPE_ASSIGN_CHECK(*in_shape, rnn_enum::kState,
                           Shape3(total_layers, batch_size, param_.state_size));
        if (param_.mode == rnn_enum::kLstm)
            SHAPE_ASSIGN_CHECK(
                *in_shape, rnn_enum::kStateCell,
                Shape3(total_layers, batch_size, param_.state_size));

        // calculate parameter vector length
        int param_size =
            rnn_param_size(param_.num_layers, input_size, param_.state_size,
                           param_.bidirectional, param_.mode);
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        if (!param_.state_outputs) {
            ...
            out_type->push_back(dtype);
            // Deal with lstm cell state
            if (param_.mode == rnn_enum::kLstm) out_type->push_back(dtype);
            return true;
        }
    }
    ...
    std::string TypeString() const override { return "RNN"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        std::vector<int> dep = {
            in_data[rnn_enum::kData], in_data[rnn_enum::kParams],
