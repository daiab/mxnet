...
struct ConvolutionParam : public dmlc::Parameter<ConvolutionParam> {
    ...
    DMLC_DECLARE_PARAMETER(ConvolutionParam) {
        DMLC_DECLARE_FIELD(kernel)
            .describe("convolution kernel size: (h, w) or (d, h, w)");
...
class ConvolutionOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        for (index_t n = 0; n < num_; ++n) {
            ...
            for (index_t g = 0; g < group_; ++g) {
                ASSIGN_DISPATCH(output_3d[g], req[conv::kOut],
                                dot(weight_3d[g], col_buffer_3d[g]));
            }
        }
        if (bias_term_) {
            Tensor<xpu, 1, DType> bias =
                in_data[conv::kBias].get<xpu, 1, DType>(s);
            ...
            // has bias term, broadcast it to the same shape of output_3d in
            // channel dim
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> out_grad_4d =
            out_grad[conv::kOut].get_with_shape<xpu, 4, DType>(
        ...
        Tensor<xpu, 3, DType> dweight_3d =
            in_grad[conv::kWeight].get_with_shape<xpu, 3, DType>(
                Shape3(group_, K, M), s);
        ...
        for (index_t n = 0; n < num_; ++n) {
            ...
            col2im(s, col_buffer.dptr<DType>(), in_grad[conv::kData].shape_,
                   col_buffer.shape_, param_.kernel, param_.pad, param_.stride,
                   param_.dilate,
                   in_grad[conv::kData].dptr<DType>() + n * input_dim_,
                   req[conv::kData]);

            // gradient w.r.t. weight, dWeight should accumulate across the
            // batch and group
            im2col(s, in_data[conv::kData].dptr<DType>() + n * input_dim_,
                   in_data[conv::kData].shape_, col_buffer.shape_,
                   param_.kernel, param_.pad, param_.stride, param_.dilate,
        ...
        // gradient w.r.t bias
        if (bias_term_) {
            Tensor<xpu, 1, DType> dbias =
                in_grad[conv::kBias].get<xpu, 1, DType>(s);
            Tensor<xpu, 3, DType> dout =
                out_grad[conv::kOut].get_with_shape<xpu, 3, DType>(
                    Shape3(num_, conv_out_channels_, conv_out_spatial_dim_), s);
            ASSIGN_DISPATCH(dbias, req[conv::kBias],
                            sumall_except_dim<1>(dout));
        }
    }
   ...
   private:
    void LayerSetUp(const TShape &ishape, const TShape &oshape) {
        ...
        for (index_t i = 0; i < param_.kernel.ndim(); ++i) {
            is_1x1_ &= param_.kernel[i] == 1 && param_.stride[i] == 1 &&
                       param_.pad[i] == 0;
            if (!is_1x1_) break;
        }
   ...
   private:
    ...
    index_t
        weight_offset_;  // number of output channels per group * kernel_dim_
    index_t col_offset_;
...
class ConvolutionProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.kernel.ndim() == 1) {
            ...
            oshape[2] =
                dshape[2]
                    ? (AddPad(dshape[2], param_.pad[0]) - dilated_ksize_x) /
                              param_.stride[0] +
                          1
                    : 0;
            SHAPE_ASSIGN_CHECK(
            ...
            // Perform incomplete shape inference. Fill in the missing values in
            // data shape.
            // 1) We can always fill in the batch_size.
            // 2) We can back-calculate the input height/width if the
            // corresponding stride is 1.
            oshape = ConvertLayout((*out_shape)[0].get<3>(),
            ...
            if (dshape[2] != 0) {
                CHECK_LE(dilated_ksize_x, AddPad(dshape[2], param_.pad[0]))
            ...
            Shape<4> dshape =
                ConvertLayout(dshp.get<4>(), param_.layout.value(), kNCHW);
            ...
            oshape[2] =
                dshape[2]
                    ? (AddPad(dshape[2], param_.pad[0]) - dilated_ksize_y) /
                              param_.stride[0] +
                          1
                    : 0;
            oshape[3] =
                dshape[3]
                    ? (AddPad(dshape[3], param_.pad[1]) - dilated_ksize_x) /
                              param_.stride[1] +
                          1
                    : 0;
            SHAPE_ASSIGN_CHECK(
            ...
            // 2) We can back-calculate the input height/width if the
            // corresponding stride is 1.
            oshape = ConvertLayout((*out_shape)[0].get<4>(),
            ...
            // Check whether the kernel sizes are valid
            if (dshape[2] != 0) {
                CHECK_LE(dilated_ksize_y, AddPad(dshape[2], param_.pad[0]))
                    << "kernel size exceed input";
            }
            if (dshape[3] != 0) {
                CHECK_LE(dilated_ksize_x, AddPad(dshape[3], param_.pad[1]))
                    << "kernel size exceed input";
            }
            return true;
            ...
            Shape<5> dshape =
                ConvertLayout(dshp.get<5>(), param_.layout.value(), kNCDHW);
            Shape<5> wshape =
            ...
            oshape[2] =
                dshape[2]
                    ? (AddPad(dshape[2], param_.pad[0]) - dilated_ksize_d) /
                              param_.stride[0] +
                          1
                    : 0;
            oshape[3] =
                dshape[3]
                    ? (AddPad(dshape[3], param_.pad[1]) - dilated_ksize_y) /
                              param_.stride[1] +
                          1
                    : 0;
            oshape[4] =
                dshape[4]
                    ? (AddPad(dshape[4], param_.pad[2]) - dilated_ksize_x) /
                              param_.stride[2] +
                          1
                    : 0;
            SHAPE_ASSIGN_CHECK(
            ...
            // corresponding stride is 1.
            oshape = ConvertLayout((*out_shape)[0].get<5>(),
            ...
            // Check whether the kernel sizes are valid
            if (dshape[2] != 0) {
                CHECK_LE(dilated_ksize_d, AddPad(dshape[2], param_.pad[0]))
                    << "kernel size exceed input";
            }
            if (dshape[3] != 0) {
                CHECK_LE(dilated_ksize_y, AddPad(dshape[3], param_.pad[1]))
                    << "kernel size exceed input";
            }
            if (dshape[4] != 0) {
                CHECK_LE(dilated_ksize_x, AddPad(dshape[4], param_.pad[2]))
                    << "kernel size exceed input";
            }
            return true;
        } else {
    ...
    std::string TypeString() const override { return "Convolution"; }
   ...
   private:
    ...
    index_t AddPad(index_t dsize, index_t pad) const { return dsize + 2 * pad; }
