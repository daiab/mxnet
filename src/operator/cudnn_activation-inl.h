...
class CuDNNActivationOp : public Operator {
   public:
    explicit CuDNNActivationOp(ActivationParam param) {
        ...
        CUDNN_CALL(
            cudnnSetActivationDescriptor(desc_, mode_, nan_prop_, relu_ceil_));
#endif
    }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (in_data[activation::kData].ndim() == 2) {
            ...
            data = in_data[activation::kData].get_with_shape<gpu, 4, DType>(
                dshape, s);
            out = out_data[activation::kOut].get_with_shape<gpu, 4, DType>(
                dshape, s);
        } else {
            Shape<4> dshape;
        ...
        if (!init_cudnn_) {
            ...
            CUDNN_CALL(cudnnSetTensor4dDescriptor(
                shape_desc_, CUDNN_TENSOR_NCHW, dtype_, data.shape_[0],
                data.shape_[1], data.shape_[2], data.shape_[3]));
        }
#if CUDNN_MAJOR <= 4
        CUDNN_CALL(cudnnActivationForward(s->dnn_handle_, mode_, &alpha,
                                          shape_desc_, data.dptr_, &beta,
                                          shape_desc_, out.dptr_));
#elif CUDNN_MAJOR >= 5
        CUDNN_CALL(cudnnActivationForward(s->dnn_handle_, desc_, &alpha,
                                          shape_desc_, data.dptr_, &beta,
                                          shape_desc_, out.dptr_));
#endif
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (in_grad[activation::kData].ndim() == 2) {
            Shape<4> dshape =
                Shape4(in_grad[activation::kData].shape_[0],
                       in_grad[activation::kData].shape_[1], 1, 1);
            data = in_data[activation::kData].get_with_shape<gpu, 4, DType>(
                dshape, s);
            grad = out_grad[activation::kOut].get_with_shape<gpu, 4, DType>(
                dshape, s);
            output_data =
                out_data[activation::kOut].get_with_shape<gpu, 4, DType>(dshape,
                                                                         s);
            input_grad =
                in_grad[activation::kData].get_with_shape<gpu, 4, DType>(dshape,
                                                                         s);
        } else {
            Shape<4> dshape;
        ...
        CUDNN_CALL(cudnnActivationBackward(
            s->dnn_handle_, mode_, &alpha, shape_desc_, output_data.dptr_,
            shape_desc_, grad.dptr_, shape_desc_, data.dptr_, &beta,
            shape_desc_, input_grad.dptr_));
#elif CUDNN_MAJOR >= 5
        CUDNN_CALL(cudnnActivationBackward(
            s->dnn_handle_, desc_, &alpha, shape_desc_, output_data.dptr_,
            shape_desc_, grad.dptr_, shape_desc_, data.dptr_, &beta,
            shape_desc_, input_grad.dptr_));
#endif
    }
