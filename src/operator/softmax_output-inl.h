   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        if (param_.multi_output) {
            ...
            Shape<3> s3 = Shape3(
                n, k, static_cast<int>(in_data[softmaxout_enum::kData].Size() /
                                       n / k));
            Tensor<xpu, 3, DType> data =
                in_data[softmaxout_enum::kData].get_with_shape<xpu, 3, DType>(
                    s3, s);
            ...
            if (param_.preserve_shape) {
                Tensor<xpu, 2, DType> data =
                    in_data[softmaxout_enum::kData].FlatTo2D<xpu, DType>(s);
                Tensor<xpu, 2, DType> out =
                    out_data[softmaxout_enum::kOut].FlatTo2D<xpu, DType>(s);
                Softmax(out, data);
            } else {
                int n = in_data[softmaxout_enum::kData].size(0);
                ...
                Tensor<xpu, 2, DType> data =
                    in_data[softmaxout_enum::kData]
                        .get_with_shape<xpu, 2, DType>(s2, s);
                Tensor<xpu, 2, DType> out =
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (out_data[softmaxout_enum::kOut].shape_ ==
            ...
            Tensor<xpu, 2, DType> label =
                in_data[softmaxout_enum::kLabel].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> out =
                out_data[softmaxout_enum::kOut].FlatTo2D<xpu, DType>(s);
            Tensor<xpu, 2, DType> grad =
                in_grad[softmaxout_enum::kData].FlatTo2D<xpu, DType>(s);
            if (param_.out_grad) {
                Tensor<xpu, 2, DType> ograd =
                    out_grad[softmaxout_enum::kOut].FlatTo2D<xpu, DType>(s);
            ...
            Shape<3> s3 = Shape3(
                n, k, static_cast<int>(out_data[softmaxout_enum::kOut].Size() /
                                       n / k));
            Shape<2> s2 = Shape2(s3[0], s3[2]);
            Tensor<xpu, 2, DType> label =
                in_data[softmaxout_enum::kLabel].get_with_shape<xpu, 2, DType>(
                    s2, s);
            Tensor<xpu, 3, DType> out =
                out_data[softmaxout_enum::kOut].get_with_shape<xpu, 3, DType>(
                    s3, s);
            Tensor<xpu, 3, DType> grad =
                in_grad[softmaxout_enum::kData].get_with_shape<xpu, 3, DType>(
                    s3, s);

            index_t valid_cnt = label.shape_.Size();
            if (param_.use_ignore) {
                SoftmaxGrad(grad, out, label,
                            static_cast<DType>(param_.ignore_label));
            } else {
                SoftmaxGrad(grad, out, label);
            }
            if (param_.normalization == softmaxout_enum::kBatch) {
                ...
                Tensor<cpu, 2, DType> workspace =
                    ctx.requested[softmaxout_enum::kTempSpace]
                        .get_host_space_typed<2, DType>(label.shape_);
            ...
            if (param_.out_grad) {
                Tensor<xpu, 3, DType> ograd =
                    out_grad[softmaxout_enum::kOut]
                        .get_with_shape<xpu, 3, DType>(s3, s);
                grad *= ograd;
            }
        } else {
            Shape<1> label_shape =
                Shape1(in_data[softmaxout_enum::kLabel].Size());
            Shape<2> data_shape;
            if (param_.preserve_shape) {
                data_shape = out_data[softmaxout_enum::kOut].shape_.FlatTo2D();
                //        Tensor<xpu, 1, DType> label =
                //        in_data[softmaxout_enum::kLabel].FlatTo1D<xpu,
                //        DType>(s);
                //        Tensor<xpu, 2, DType> out =
                //        out_data[softmaxout_enum::kOut].FlatTo2D<xpu,
                //        DType>(s);
                //        Tensor<xpu, 2, DType> grad =
                //        in_grad[softmaxout_enum::kData].FlatTo2D<xpu,
                //        DType>(s);
            } else {
                int n = out_data[softmaxout_enum::kOut].size(0);
                data_shape =
                    Shape2(n, out_data[softmaxout_enum::kOut].Size() / n);
            }
            Tensor<xpu, 1, DType> label =
                in_data[softmaxout_enum::kLabel].get_with_shape<xpu, 1, DType>(
                    label_shape, s);
            Tensor<xpu, 2, DType> out =
                out_data[softmaxout_enum::kOut].get_with_shape<xpu, 2, DType>(
                    data_shape, s);
            Tensor<xpu, 2, DType> grad =
                in_grad[softmaxout_enum::kData].get_with_shape<xpu, 2, DType>(
                    data_shape, s);
            index_t valid_cnt = label.shape_.Size();
            if (param_.use_ignore) {
                SoftmaxGrad(grad, out, label,
            ...
            if (param_.normalization == softmaxout_enum::kBatch) {
                ...
                Tensor<cpu, 1, DType> workspace =
                    ctx.requested[softmaxout_enum::kTempSpace]
                        .get_host_space_typed<1, DType>(label.shape_);
                Copy(workspace, label, label.stream_);
...
class SoftmaxOutputProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "SoftmaxOutput"; }
    ...
    std::vector<std::pair<int, void *> > BackwardInplaceOption(
        ...
        return {
            {out_data[softmaxout_enum::kOut], in_grad[softmaxout_enum::kData]}};
    }
    ...
    std::vector<std::pair<int, void *> > ForwardInplaceOption(
        ...
        return {
            {in_data[softmaxout_enum::kData], out_data[softmaxout_enum::kOut]}};
    }
...
class DeprecatedSoftmaxProp : public SoftmaxOutputProp {
   public:
    ...
    std::string TypeString() const override { return "Softmax"; }
};
