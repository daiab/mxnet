 ...
 * \brief
 *  Reproducing paper: aderberg M, Simonyan K, Zisserman A. "Spatial transformer
 * networks"
 * \author Wei Wu
*/
...
struct SpatialTransformerParam
    : public dmlc::Parameter<SpatialTransformerParam> {
    TShape target_shape;
    int transform_type;
    int sampler_type;
    DMLC_DECLARE_PARAMETER(SpatialTransformerParam) {
        ...
        DMLC_DECLARE_FIELD(sampler_type)
            .add_enum("bilinear", st::kBilinear)
   ...
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> grid_dst =
            out_data[st::kGridDst].get<xpu, 2, DType>(s);
        Tensor<xpu, 3, DType> grid_src =
            out_data[st::kGridSrc].get<xpu, 3, DType>(s);
        Shape<3> loc_shape = Shape3(data.size(0), 2, 3);
        Tensor<xpu, 3, DType> loc =
            in_data[st::kLoc].get_with_shape<xpu, 3, DType>(loc_shape, s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 3, DType> grid_src =
            out_data[st::kGridSrc].get<xpu, 3, DType>(s);
        Shape<3> loc_shape = Shape3(data.size(0), 2, 3);
        Tensor<xpu, 3, DType> gloc =
            in_grad[st::kLoc].get_with_shape<xpu, 3, DType>(loc_shape, s);
        gdata = 0.0;
        if (param_.sampler_type == st::kBilinear) {
...
class SpatialTransformerProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK_EQ(param_.transform_type, st::kAffine)
            << "only supports affine transform currently";
        CHECK_EQ(param_.sampler_type, st::kBilinear)
            << "only supports bilinear sampling currently";
        ...
        if (param_.transform_type == st::kAffine) {
            CHECK_EQ(lshape[1], 6U)
                << "incorrect locolisation network shape[1], should be 6";
        }
        out_shape->clear();
        out_shape->push_back(dshape);
        CHECK_GT(param_.target_shape[0], 0U) << "incorrect target_shape: "
                                             << param_.target_shape[0];
        CHECK_GT(param_.target_shape[1], 0U) << "incorrect target_shape: "
        ...
        out_shape->push_back(Shape3(
            dshape[0], 2, param_.target_shape[0] * param_.target_shape[1]));
        return true;
    }
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        if (dtype == -1) {
            LOG(FATAL) << "Not enough information to infer type in "
                          "SpatialTransformer.";
            return false;
        }
    ...
    std::string TypeString() const override { return "SpatialTransformer"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[st::kOut], out_data[st::kGridDst],
                out_data[st::kGridSrc], in_data[st::kData]};
    }
