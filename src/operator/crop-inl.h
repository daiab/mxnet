    ...
    DMLC_DECLARE_PARAMETER(CropParam) {
        DMLC_DECLARE_FIELD(num_args)
            ...
            .describe(
                ...
                "the h_w"
                "for crop height and width, else if equals two, then we will "
                "use the height"
...
class CropOp : public Operator {
   public:
    explicit CropOp(CropParam param) { this->param_ = param; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        out = crop(data, Shape2(out.size(2), out.size(3)), offset_hw_[0],
                   offset_hw_[1]);
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (param_.num_args > 1) {
            ...
            // however, this should only be done when num_args > 1, i.e.,
            // crop_like exists
            Tensor<xpu, 4> gcrop_like =
                in_grad[crop_enum::kCropLike].get<xpu, 4, real_t>(s);
   ...
   private:
    ...
    std::vector<int> InferCropOfferset(const mshadow::Shape<4> &data_shape,
        ...
        if (param_.center_crop) {
            offset_hw.push_back(
                static_cast<int>((data_shape[2] - out_shape[2]) / 2));
            offset_hw.push_back(
                static_cast<int>((data_shape[3] - out_shape[3]) / 2));
        } else {
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.num_args == 1) {
            ...
            CHECK_LE(static_cast<int>(param_.h_w[0]),
                     static_cast<int>(data_shape[2]))
                << "the crop height(h_w[0]) should be less than the input "
                   "data's height";
            CHECK_GE(static_cast<int>(param_.h_w[1]), 1)
                << "the crop width(h_w[1]) should be larger than 1";
            CHECK_LE(static_cast<int>(param_.h_w[1]),
                     static_cast<int>(data_shape[3]))
                << "the crop width(h_w[1]) should be less than the input "
                   "data's width";
    ...
    std::string TypeString() const override { return "Crop"; }
