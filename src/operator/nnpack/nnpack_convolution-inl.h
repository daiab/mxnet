...
class NNPACKConvolutionOp : public ConvolutionOp<xpu, DType> {
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        Shape<3> wmat_shape = Shape3(
            param_.num_group, param_.num_filter / param_.num_group,
            input_c / param_.num_group * param_.kernel[0] * param_.kernel[1]);
        ...
        nnp_convolution_transform_strategy kts =
        ...
        if (nnp_status_success != status) {
            LOG(FATAL) << "nnpack convolution feedforward failed status="
                       << status;
        }
    }
