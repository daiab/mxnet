...
class NNPACKPoolingOp : public PoolingOp<xpu, DType> {
   ...
   public:
    explicit NNPACKPoolingOp(PoolingParam p) : PoolingOp<xpu, DType>(p) {
        this->param_ = p;
    }
   ...
   public:
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> data =
            in_data[pool_enum::kData].get<xpu, 4, DType>(s);
        ...
        Tensor<xpu, 4, DType> out =
            out_data[pool_enum::kOut].get<xpu, 4, DType>(s);
        nnp_size input_size = {input_w, input_h};
        nnp_padding input_padding = {param_.pad[0], param_.pad[1],
        ...
        if (nnp_status_success != status) {
            LOG(FATAL) << "nnpack max pooling feedforward failed status="
                       << status;
        }
    }
