...
class PoolingOp : public Operator {
   public:
    explicit PoolingOp(PoolingParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext& ctx,
        ...
        pool(s, in_data[pool_enum::kData].dptr<DType>(),
             in_data[pool_enum::kData].shape_, out_data[pool_enum::kOut].shape_,
             param_.global_pool
             ...
             param_.pool_type, req[pool_enum::kOut],
    ...
    virtual void Backward(const OpContext& ctx,
        ...
        unpool(
            s, out_grad[pool_enum::kOut].dptr<DType>(),
            in_data[pool_enum::kData].dptr<DType>(),
            out_data[pool_enum::kOut].dptr<DType>(),
            in_grad[pool_enum::kData].shape_, out_grad[pool_enum::kOut].shape_,
            param_.global_pool
                ? TShape(ishape.data() + ishape.ndim() - param_.kernel.ndim(),
                         ishape.data() + ishape.ndim())
                : param_.kernel,
            param_.pad,
            param_.global_pool ? TShape(param_.kernel.ndim()) : param_.stride,
            param_.pool_type, req[pool_enum::kData],
   ...
   public:
    ...
    bool InferShape(std::vector<TShape>* in_shape,
        ...
        if (param_.kernel.ndim() == 1) {
            ...
            CHECK_EQ(dshape.ndim(), 5U) << "Pooling: Input data should be 5D "
                                           "in (batch, channel, d, y, x)";
    ...
    std::string TypeString() const override { return "Pooling"; }
