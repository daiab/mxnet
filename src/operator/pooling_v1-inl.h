...
struct PoolingV1Param : public dmlc::Parameter<PoolingV1Param> {
    ...
    DMLC_DECLARE_PARAMETER(PoolingV1Param) {
        ...
        DMLC_DECLARE_FIELD(pooling_convention)
            .set_default(pool_v1_enum::kValid)
...
class PoolingV1Op : public Operator {
   public:
    explicit PoolingV1Op(PoolingV1Param p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> data =
            in_data[pool_v1_enum::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> out =
            out_data[pool_v1_enum::kOut].get<xpu, 4, DType>(s);
        ...
        if (param_.pool_type == pool_v1_enum::kMaxPooling ||
            ...
            Assign(out, req[pool_v1_enum::kOut],
                   pool<Reducer>(
                       pad(data, param_.pad[0], param_.pad[1]), out_shape,
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad =
            out_grad[pool_v1_enum::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> data =
            in_data[pool_v1_enum::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> output_data =
            out_data[pool_v1_enum::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> input_grad =
            in_grad[pool_v1_enum::kData].get<xpu, 4, DType>(s);
        ...
        if (param_.pool_type == pool_v1_enum::kMaxPooling ||
            ...
            Assign(input_grad, req[pool_v1_enum::kData],
                   crop(unpool<Reducer>(
                            pad(data, param_.pad[0], param_.pad[1]),
                            pad(output_data, 0, 0), pad(grad, 0, 0),
                            param_.global_pool ? in_shape[0] : param_.kernel[0],
                            param_.global_pool ? in_shape[1] : param_.kernel[1],
                            param_.global_pool ? 1 : param_.stride[0],
                            param_.global_pool ? 1 : param_.stride[1]),
                        in_shape, param_.pad[0], param_.pad[1]));
        } else if (param_.pool_type == pool_v1_enum::kAvgPooling) {
            Assign(
                input_grad, req[pool_v1_enum::kData],
                scalar<DType>(1.0f /
                              (param_.global_pool
                                   ? data.shape_[2] * data.shape_[3]
                                   : param_.kernel[0] * param_.kernel[1])) *
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.kernel.ndim() == 2) {
            ...
            CHECK_EQ(dshape.ndim(), 5) << "Pooling: Input data should be 5D in "
                                          "(batch, channel, d, y, x)";
    ...
    std::string TypeString() const override { return "Pooling_v1"; }
