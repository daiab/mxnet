...
class CorrelationOp : public Operator {
   public:
    explicit CorrelationOp(CorrelationParam param) { this->param_ = param; }
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4> data1 =
            in_data[Correlation::kData1].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> data2 =
            in_data[Correlation::kData2].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> out = out_data[Correlation::kOut].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> tmp1 =
            out_data[Correlation::kTemp1].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> tmp2 =
            out_data[Correlation::kTemp2].get<xpu, 4, real_t>(s);
        tmp1 = 0.0f;
        tmp2 = 0.0f;
        out = 0.0f;
        ...
        top_width_ =
            ceil(static_cast<float>(paddedbottomwidth - border_size_ * 2) /
                 static_cast<float>(stride1));
        top_height_ =
            ceil(static_cast<float>(paddedbottomheight - border_size_ * 2) /
                 static_cast<float>(stride1));
        ...
        CorrelationForward(out, data1, data2, tmp1, tmp2, top_channels_,
                           top_height_, top_width_, param_.pad_size,
                           param_.is_multiply, param_.max_displacement,
                           param_.kernel_size, neighborhood_grid_radius_,
                           neighborhood_grid_width_, kernel_radius_,
                           param_.stride1, param_.stride2);
    }
    virtual void Backward(const OpContext &ctx,
        ...
        using namespace mshadow;
        Stream<xpu> *s = ctx.get_stream<xpu>();
        Tensor<xpu, 4> grad_data1 =
            in_grad[Correlation::kData1].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> grad_data2 =
            in_grad[Correlation::kData2].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> out_g =
            out_grad[Correlation::kOut].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> tmp1 =
            out_data[Correlation::kTemp1].get<xpu, 4, real_t>(s);
        Tensor<xpu, 4> tmp2 =
            out_data[Correlation::kTemp2].get<xpu, 4, real_t>(s);
        ...
        CHECK_EQ(tmp2.CheckContiguous(), true);
        CorrelationBackward(
            out_g, grad_data1, grad_data2, tmp1, tmp2, top_channels_,
            ...
            param_.stride1, param_.stride2, num, channels, height, width);
    }
...
class CorrelationProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        neighborhood_grid_width_ = neighborhood_grid_radius_ * 2 + 1;
        top_channels_ = neighborhood_grid_width_ * neighborhood_grid_width_;
        CHECK_GE(top_width_, 1U) << "Correlation cannot be done with current "
                                    "settings.Neighborhood and kernel don't "
                                    "fit in blob";
        CHECK_GE(top_height_, 1U) << "Correlation cannot be done with current "
                                     "settings.Neighborhood and kernel don't "
                                     "fit in blob";
        out_shape->clear();
        out_shape->push_back(
            Shape4(dshape1[0], top_channels_, top_height_, top_width_));
    ...
    std::string TypeString() const override { return "Correlation"; }
    //  decalre dependency and inplace optimization options
    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[Correlation::kOut], out_data[Correlation::kTemp1],
                out_data[Correlation::kTemp2]};
    }
