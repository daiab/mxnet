...
struct ConvolutionV1Param : public dmlc::Parameter<ConvolutionV1Param> {
    ...
    DMLC_DECLARE_PARAMETER(ConvolutionV1Param) {
        ...
        DMLC_DECLARE_FIELD(num_group)
            .set_default(1)
            .describe(
                "Number of group partitions. Equivalent to slicing input into "
                "num_group\n    "
...
class ConvolutionV1Op : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> data =
            in_data[conv_v1::kData].get<xpu, 4, DType>(s);
        Shape<3> wmat_shape =
            Shape3(param_.num_group, param_.num_filter / param_.num_group,
                   data.shape_[1] / param_.num_group * param_.kernel[0] *
                       param_.kernel[1]);
        Tensor<xpu, 3, DType> wmat =
            in_data[conv_v1::kWeight].get_with_shape<xpu, 3, DType>(wmat_shape,
                                                                    s);
        Tensor<xpu, 4, DType> out =
            out_data[conv_v1::kOut].get<xpu, 4, DType>(s);
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            if (param_.pad[0] == 0 && param_.pad[1] == 0) {
                ...
                temp_col = unpack_patch2col(
                    pad(data.Slice(i, i + step), param_.pad[0], param_.pad[1]),
            ...
            out.Slice(i, i + step) = swapaxis<1, 0>(
                reshape(temp_dst, mshadow::Shape4(param_.num_filter, step,
                                                  out.size(2), out.size(3))));
        }
        if (!param_.no_bias) {
            // add bias, broadcast bias to dim 1: channel
            Tensor<xpu, 1, DType> bias =
                in_data[conv_v1::kBias].get<xpu, 1, DType>(s);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad =
            out_grad[conv_v1::kOut].get<xpu, 4, DType>(s);
        Tensor<xpu, 4, DType> gdata =
            in_grad[conv_v1::kData].get<xpu, 4, DType>(s);
        Tensor<xpu, 3, DType> gwmat =
            in_grad[conv_v1::kWeight].get_with_shape<xpu, 3, DType>(wmat_shape,
                                                                    s);
#if defined(__CUDACC__)
        CHECK_EQ(s->blas_handle_ownership_, Stream<xpu>::OwnHandle)
            << "Must init CuBLAS handle in stream";
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            Tensor<xpu, 3, DType> temp_dst = Tensor<xpu, 3, DType>(
                ...
                Shape3(shape_dstunit_[0], shape_dstunit_[1],
                       shape_dstunit_[2] * step),
                s);
            temp_dst = reshape(swapaxis<1, 0>(grad.Slice(i, i + step)),
                               temp_dst.shape_);
            if (param_.pad[0] == 0 && param_.pad[1] == 0) {
                temp_col = unpack_patch2col(data.Slice(i, i + step),
                                            param_.kernel[0], param_.kernel[1],
            ...
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                ...
                if (i == 0) {
                    ...
                    Assign(tmp_gwmat, req[conv_v1::kWeight],
            ...
            if (param_.pad[0] == 0 && param_.pad[1] == 0) {
                Assign(gdata.Slice(i, i + step), req[conv_v1::kData],
                       pack_col2patch(temp_col, data.Slice(i, i + step).shape_,
                                      param_.kernel[0], param_.kernel[1],
                                      param_.stride[0], param_.stride[1],
                                      param_.dilate[0], param_.dilate[1]));
            } else {
        ...
        if (!param_.no_bias) {
            Tensor<xpu, 1, DType> gbias =
                in_grad[conv_v1::kBias].get<xpu, 1, DType>(s);
   ...
   private:
    inline index_t InitTemp(const mshadow::Shape<4> &ishape,
        ...
        // if param_.workspace is set to zero the nstep_ equals ishape[0]
        // (batch)
        nstep_ =
   ...
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.kernel.ndim() == 2) {
            ...
            if (!param_.no_bias) {
                SHAPE_ASSIGN_CHECK(*in_shape, conv_v1::kBias,
            ...
            CHECK_GT(param_.kernel.Size(), 0) << "incorrect kernel size: "
                                              << param_.kernel;
            CHECK_GT(param_.stride.Size(), 0) << "incorrect stride size: "
                                              << param_.stride;
            CHECK_GT(param_.dilate.Size(), 0) << "incorrect dilate size: "
                                              << param_.dilate;
            ...
            oshape[2] = (dshape[2] + 2 * param_.pad[0] -
                         (param_.dilate[0] * (ksize_y - 1) + 1)) /
                            param_.stride[0] +
                        1;
            oshape[3] = (dshape[3] + 2 * param_.pad[1] -
                         (param_.dilate[1] * (ksize_x - 1) + 1)) /
                            param_.stride[1] +
                        1;
            SHAPE_ASSIGN_CHECK(
                *out_shape, 0,
            ...
            oshape[1] = param_.num_filter;
            oshape[2] =
                (dshape[2] + 2 * param_.pad[0] - (1 * (ksize_d - 1) + 1)) /
                    param_.stride[0] +
                1;
            oshape[3] =
                (dshape[3] + 2 * param_.pad[1] - (1 * (ksize_y - 1) + 1)) /
                    param_.stride[1] +
                1;
            oshape[4] =
                (dshape[4] + 2 * param_.pad[2] - (1 * (ksize_x - 1) + 1)) /
                    param_.stride[2] +
                1;
            SHAPE_ASSIGN_CHECK(
                *out_shape, 0,
    ...
    std::string TypeString() const override { return "Convolution_v1"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[conv_v1::kOut], in_data[conv_v1::kData],
                in_data[conv_v1::kWeight]};
    }
