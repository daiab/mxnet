...
class CuDNNBilinearSamplerOp : public Operator {
   public:
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<gpu, 4, DType> grid_tmp =
            out_data[bs::kTmp].get<gpu, 4, DType>(s);
        ...
        CUDNN_CALL(cudnnSpatialTfSamplerForward(
            s->dnn_handle_, st_desc_, &alpha, in_desc_, data.dptr_,
            grid_tmp.dptr_, &beta, out_desc_, out.dptr_));
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        typename DataType<DType>::ScaleType alpha =
            (req[bs::kData] == kNullOp) ? 0.0f : 1.0f;
        typename DataType<DType>::ScaleType beta =
            (req[bs::kData] == kAddTo) ? 1.0f : 0.0f;
        ...
        CUDNN_CALL(cudnnSpatialTfSamplerBackward(
            ...
            in_desc_ /*reuse in_desc_*/, gdata.dptr_ /*output*/, &alpha_dgrid,
            out_desc_ /*reuse out_desc_*/, grad.dptr_, grid_tmp.dptr_,
    ...
    inline void Init(mshadow::Stream<gpu> *s, const std::vector<TBlob> &in_data,
        ...
        if (!init_cudnn_) {
            ...
            Tensor<gpu, 4, DType> data =
                in_data[bs::kData].get<gpu, 4, DType>(s);
            Tensor<gpu, 4, DType> out =
                out_data[bs::kOut].get<gpu, 4, DType>(s);
