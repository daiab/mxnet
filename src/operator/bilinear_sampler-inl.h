...
class BilinearSamplerOp : public Operator {
   public:
    explicit BilinearSamplerOp(BilinearSamplerParam p) { this->param_ = p; }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        if (req[bs::kData] != kNullOp && req[bs::kGrid] != kNullOp) {
            ...
            LOG(FATAL)
                << "Have not implemented the data req combinations! gdata_req="
...
class BilinearSamplerProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        out_shape->clear();
        // output_shape : (data.shape[0], data.shape[1], grid.shape[2],
        // grid.shape[3])
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        if (dtype == -1) {
            LOG(FATAL)
                << "Not enough information to infer type in BilinearSampler.";
            return false;
        }
    ...
    std::string TypeString() const override { return "BilinearSampler"; }

    std::vector<int> DeclareBackwardDependency(
        const std::vector<int> &out_grad, const std::vector<int> &in_data,
        const std::vector<int> &out_data) const override {
        return {out_grad[bs::kOut], in_data[bs::kData], out_data[bs::kTmp],
                in_data[bs::kGrid]};
    }
