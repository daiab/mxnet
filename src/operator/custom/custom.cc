...
void CustomOp<xpu>::Forward(const OpContext &ctx,
    ...
    for (auto &blob : in_data) {
        ptrs.push_back(
            reinterpret_cast<void *>(new NDArray(blob, ndctx.dev_id)));
        tags.push_back(0);
    }
    ...
    auto compute = [=]() mutable {
        CHECK(reinterpret_cast<CustomOpFBFunc>(
            op_info_->callbacks[kCustomOpForward])(
            ptrs.size(), ptrs.data(), tags.data(), reqs.data(),
            static_cast<int>(ctx.is_train),
            op_info_->contexts[kCustomOpForward]));

        // NDArray* in ptrs is freed by frontend side. We keep a copy in ndcpy
        // to keep ndvar alive
        Engine::Get()->PushSync(
            [ndcpy, ctx](RunContext rctx) { ctx.async_on_complete(); }, ndctx,
            ndvar, {}, FnProperty::kNormal, 0,
            PROFILER_MESSAGE("CustomOpForward"));
    };
...
void CustomOp<xpu>::Backward(const OpContext &ctx,
    ...
    auto compute = [=]() mutable {
        CHECK(reinterpret_cast<CustomOpFBFunc>(
            op_info_->callbacks[kCustomOpBackward])(
        ...
        Engine::Get()->PushSync(
            [ndcpy, ctx](RunContext rctx) { ctx.async_on_complete(); }, ndctx,
            ndvar, {}, FnProperty::kNormal, 0,
            PROFILER_MESSAGE("CustomOpBackward"));
    };
...
Operator *CustomOpProp::CreateOperatorEx(Context ctx,
    ...
    CHECK(reinterpret_cast<CustomOpCreateFunc>(
        info_->callbacks[kCustomOpPropCreateOperator])(
        str_ctx.c_str(), shapes.size(), shapes.data(), ndims.data(),
        in_type->data(), op_info,
...
MXNET_REGISTER_OP_PROPERTY(Custom, CustomOpProp)
    .describe(
        R"code(Apply a custom operator implemented in a frontend language (like Python).
