...
struct NDArrayOpParam : public dmlc::Parameter<NDArrayOpParam> {
    ...
    DMLC_DECLARE_PARAMETER(NDArrayOpParam) { DMLC_DECLARE_FIELD(info); }
};

template <typename xpu>
class NDArrayOp : public Operator {
   public:
    explicit NDArrayOp(NDArrayOpParam p) { this->param_ = p; }
...
class NDArrayOpProp : public OperatorProperty {
   public:
    std::vector<std::string> ListArguments() const override {
        char **args = NULL;
        CHECK(param_.pinfo->list_arguments(&args,
                                           param_.pinfo->p_list_arguments));
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK(param_.pinfo->infer_shape(shapes.size(), ndims.data(),
                                        shapes.data(),
    ...
    std::string TypeString() const override { return "_NDArray"; }
    ...
    std::vector<int> DeclareBackwardDependency(
        ...
        int num_dep;
        int *rdeps;
        CHECK(param_.pinfo->declare_backward_dependency(
