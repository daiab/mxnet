...
class NativeOp : public Operator {
   public:
    explicit NativeOp(NativeOpParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        s->Wait();
        param_.pinfo->forward(ptrs.size(), ptrs.data(), ndims.data(),
                              shapes.data(), tags.data(),
                              param_.pinfo->p_forward);
        for (index_t i = 0; i < out_data.size(); ++i) {
            CHECK_NE(req[i], kAddTo)
                << "NativeOp doesn't support AddTo for output";
            if (req[i] != kNullOp) {
                std::stringstream ss;
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        s->Wait();
        param_.pinfo->backward(ptrs.size(), ptrs.data(), ndims.data(),
                               shapes.data(), tags.data(),
   ...
   private:
    ...
    std::vector<int> tags;
    std::map<std::string, std::pair<TShape, mshadow::Tensor<cpu, 2> > >
        buffer_map;

    virtual void SyncBuffer(const TBlob &tblob, const std::string &name,
                            mshadow::Stream<xpu> *stream) {
        using namespace mshadow;
        std::map<std::string,
                 std::pair<TShape, mshadow::Tensor<cpu, 2> > >::iterator
            buffer = buffer_map.find(name);
        if (buffer == buffer_map.end() ||
            buffer->second.first != tblob.shape_) {
            if (buffer != buffer_map.end()) {
                FreeSpace<2, real_t>(&(buffer->second.second));
                buffer_map.erase(buffer);
            }
            buffer_map[name] = std::pair<TShape, Tensor<cpu, 2> >(
                tblob.shape_,
                NewTensor<cpu>(tblob.shape_.FlatTo2D(), 0.0f, false));
            buffer = buffer_map.find(name);
        }
        Copy(buffer->second.second, tblob.FlatTo2D<xpu, real_t>(stream),
             stream);
    }

    virtual void SyncVec(const std::vector<TBlob> &vec,
                         const std::string &prefix,
                         mshadow::Stream<xpu> *stream, int tag) {
        size_t size = 0;
        ...
        for (size_t i = 0; i < vec.size(); ++i) {
            ...
            ptr = nnvm::ShapeTypeCast(vec[i].shape_.begin(),
                                      vec[i].shape_.end(), ptr);
            tags.push_back(tag);
        }
    }
};  // NativeOp
...
class NativeOpProp : public OperatorProperty {
   public:
    ...
    int NumOutputs() const override { return param_.num_outputs_; }
    ...
    std::string TypeString() const override { return "_Native"; }
