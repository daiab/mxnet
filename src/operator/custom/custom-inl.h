...
class CustomOp : public Operator {
   public:
    explicit CustomOp(MXCallbackList *op_info) {
        ...
        if (std::string("NaiveEngine") ==
            dmlc::GetEnv("MXNET_ENGINE_TYPE", std::string())) {
            sync_mode_ = true;
        } else {
            sync_mode_ = false;
            destructing_ = false;
    ...
    virtual ExecType exec_type() const { return kAsync; }
   ...
   public:
    static void Register(const std::string &op_type,
        ...
        if (registry_.find(op_type) != registry_.end()) {
            LOG(WARNING)
                << "New registration is overriding existing custom operator "
                << op_type;
        }
    ...
    void Init(const std::vector<std::pair<std::string, std::string> > &kwargs)
        ...
        info_.reset(new MXCallbackList, [](MXCallbackList *ptr) {
            reinterpret_cast<CustomOpDelFunc>(
                ptr->callbacks[kCustomOpPropDelete])(
                ptr->contexts[kCustomOpPropDelete]);
            delete ptr;
        });
        CHECK(creator(param_.op_type.c_str(), keys.size(), keys.data(),
                      vals.data(), info_.get()));
    ...
    std::vector<std::string> ListArguments() const override {
        ...
        CHECK(reinterpret_cast<CustomOpListFunc>(
            info_->callbacks[kCustomOpPropListArguments])(
    ...
    std::vector<std::string> ListOutputs() const override {
        char **args = NULL;
        CHECK(reinterpret_cast<CustomOpListFunc>(
            info_->callbacks[kCustomOpPropListOutputs])(
    ...
    std::vector<std::string> ListAuxiliaryStates() const override {
        char **args = NULL;
        CHECK(reinterpret_cast<CustomOpListFunc>(
            info_->callbacks[kCustomOpPropListAuxiliaryStates])(
    ...
    int NumOutputs() const override { return ListOutputs().size(); }

    std::map<std::string, std::string> GetParams() const override {
        return std::map<std::string, std::string>(kwargs_.begin(),
                                                  kwargs_.end());
    }
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        CHECK(reinterpret_cast<CustomOpInferShapeFunc>(
            info_->callbacks[kCustomOpPropInferShape])(
            shapes.size(), ndims.data(), shapes.data(),
            info_->contexts[kCustomOpPropInferShape]));
        for (unsigned i = 0; i < in_shape->size(); ++i) {
            SHAPE_ASSIGN_CHECK(*in_shape, i,
                               TShape(shapes[i], shapes[i] + ndims[i]));
        }
        out_shape->clear();
    ...
    bool InferType(std::vector<int> *in_type, std::vector<int> *out_type,
        ...
        CHECK(reinterpret_cast<CustomOpInferTypeFunc>(
            info_->callbacks[kCustomOpPropInferType])(
            types.size(), types.data(),
            info_->contexts[kCustomOpPropInferType]));
        ...
        for (unsigned i = 0; i < num_auxs_; ++i) {
            TYPE_ASSIGN_CHECK(*aux_type, i,
                              types[i + num_inputs_ + num_outputs_]);
        }
        return true;
    }
    ...
    std::string TypeString() const override { return "Custom"; }
    ...
    std::vector<int> DeclareBackwardDependency(
        ...
        CHECK(reinterpret_cast<CustomOpBwdDepFunc>(
            ...
            info_->contexts[kCustomOpPropDeclareBackwardDependency]));
        std::vector<int> deps;
