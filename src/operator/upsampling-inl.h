    ...
    DMLC_DECLARE_PARAMETER(UpSamplingParam) {
        ...
        DMLC_DECLARE_FIELD(num_args)
            ...
            .describe(
                ...
                "(scale*h_0,scale*w_0) and all other inputs will be upsampled "
                "to the"
                "same size. For bilinear upsampling this must be 2; 1 input "
                "and 1 weight.");
...
class UpSamplingNearestOp : public Operator {
   public:
    explicit UpSamplingNearestOp(UpSamplingParam p) { this->param_ = p; }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> out =
            out_data[up_enum::kOut].get<xpu, 4, DType>(s);
        if (param_.num_args > 1) {
            ...
            for (int i = 0; i < param_.num_args; ++i) {
                ...
                int scale =
                    out_data[up_enum::kOut].size(2) / in_data[i].size(2);
                if (param_.multi_input_mode == up_enum::kSum) {
                    if (i == 0) {
                        Assign(out, req[up_enum::kOut],
                               upsampling_nearest(data, scale));
                    } else {
                        out += upsampling_nearest(data, scale);
                    }
                } else {
                    Assign(slice<1>(out, begin, end), req[up_enum::kOut],
                           upsampling_nearest(data, scale));
                }
                begin = end;
            }
            ...
            Tensor<xpu, 4, DType> data =
                in_data[up_enum::kData].get<xpu, 4, DType>(s);
            Assign(out, req[up_enum::kOut],
                   upsampling_nearest(data, param_.scale));
        }
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 4, DType> grad =
            out_grad[up_enum::kOut].get<xpu, 4, DType>(s);
        if (param_.num_args > 1) {
            int begin = 0;
            for (int i = 0; i < param_.num_args; ++i) {
                Tensor<xpu, 4, DType> input_grad =
                    in_grad[i].get<xpu, 4, DType>(s);
                mshadow::Shape<2> in_shape =
                    Shape2(input_grad.shape_[2], input_grad.shape_[3]);
                int end = begin + input_grad.size(1);
                int scale = grad.size(2) / in_shape[0];
                if (param_.multi_input_mode == up_enum::kSum) {
                    Assign(input_grad, req[i],
                           pool<mshadow::red::sum>(grad, in_shape, scale, scale,
                                                   scale, scale));
                } else {
                    Assign(input_grad, req[i],
                           pool<mshadow::red::sum>(slice<1>(grad, begin, end),
                                                   in_shape, scale, scale,
                                                   scale, scale));
                }
                begin = end;
            }
            ...
            Tensor<xpu, 4, DType> input_grad =
                in_grad[up_enum::kData].get<xpu, 4, DType>(s);
            mshadow::Shape<2> in_shape =
                Shape2(input_grad.shape_[2], input_grad.shape_[3]);
            Assign(input_grad, req[up_enum::kData],
                   pool<mshadow::red::sum>(grad, in_shape, param_.scale,
                                           param_.scale, param_.scale,
                                           param_.scale));
        }
    }
...
class UpSamplingProp : public OperatorProperty {
   public:
    ...
    bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.sample_type == up_enum::kNearest) {
            ...
            for (auto &shape : *in_shape) {
                CHECK_EQ(shape.ndim(), 4U) << "UpSamplingNearest: Input data "
                                              "should be 4D in (batch, "
                                              "channel, y, x)";
                int oh = dshape[2] * param_.scale,
                    ow = dshape[3] * param_.scale;
                ...
                if (param_.multi_input_mode == up_enum::kSum) {
                    CHECK(oshape[1] == 0 || oshape[1] == shape[1])
                        << "Number of channels must be the same when "
            ...
            CHECK_EQ(dshape.ndim(), 4U) << "UpSamplingBilinear: Input data "
                                           "should be 4D in (batch, channel, "
                                           "y, x)";
            if (dshape.ndim() == 0) return false;
            int kernel = 2 * param_.scale - param_.scale % 2;
            SHAPE_ASSIGN_CHECK(*in_shape, up_enum::kWeight,
    ...
    std::string TypeString() const override { return "UpSampling"; }
    ...
    std::vector<int> DeclareBackwardDependency(
        ...
        if (param_.sample_type == up_enum::kNearest) {
            ...
            return {out_grad[up_enum::kOut], in_data[up_enum::kData],
                    in_data[up_enum::kWeight]};
        }
    }
