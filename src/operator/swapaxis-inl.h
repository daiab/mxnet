...
class SwapAxisOp : public Operator {
   public:
    ...
    void Reshape2Five(mshadow::Shape<5> *inter_shape, const TShape &shape,
    ...
    void SwapAxis(mshadow::Stream<xpu> *s, const std::vector<TBlob> &in_data,
        ...
        Tensor<xpu, 5, DType> inter_data_in =
            data_in.get_with_shape<xpu, 5, DType>(inter_shape, s);

        Shape<5> inter_shape2 = inter_shape;
        std::swap(inter_shape2[1], inter_shape2[3]);

        Tensor<xpu, 5, DType> inter_data_out =
            data_out.get_with_shape<xpu, 5, DType>(inter_shape2, s);
...
class SwapAxisProp : public OperatorProperty {
   public:
    ...
    std::string TypeString() const override { return "SwapAxis"; }
