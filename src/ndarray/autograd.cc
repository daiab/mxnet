...
inline void AGDFSVisit(const std::vector<AGNodeEntry>& heads, FVisit fvisit) {
    ...
    nnvm::PostOrderDFSVisit<GNode, AGNode*>(
        ...
        [](GNode n, uint32_t index) -> GNode {
            return &(*n)->inputs.at(index).ag_node;
        });
}
...
void AutogradRuntime::MarkVariables(const std::vector<NDArray*>& variables,
    ...
    for (uint32_t i = 0; i < variables.size(); ++i) {
        ...
        variables[i]->entry_ =
            std::move(e);  // assign last to prevent cyclic reference
    }
}

void AutogradRuntime::RecordImperativeFCompute(
    const nnvm::Op* op, const nnvm::NodeAttrs& attrs,
    std::vector<NDArray>* p_inputs, std::vector<NDArray>* p_outputs) {
    RecordOp(op, attrs, p_inputs, p_outputs, nullptr);
}

void AutogradRuntime::RecordImperativeOperator(
    const std::shared_ptr<Operator>& opr, const nnvm::Op* op,
    const nnvm::NodeAttrs& attrs, std::vector<NDArray>* p_inputs,
...
AGNodePtr AutogradRuntime::RecordOp(const nnvm::Op* op,
    ...
    for (uint32_t i = 0; i < outputs.size(); ++i) {
        CHECK(outputs[i].entry_.is_none())
            << "Output NDArray is non-empty and already in another computation "
               "graph. "
            << "Assigning to it will cause undefined behavior when evaluating "
               "gradients. "
            << "Please call backward first to clear the graph or do this out "
               "side of "
            << "a train section. ";
    ...
    for (size_t i = 0; i < inputs.size(); ++i) {
        if (inputs[i].entry_.is_none()) {
            ...
            e.ag_node->nn_node->attrs.name =
                "var_" + std::to_string(variable_count_++);
            inputs[i].entry_ =
                std::move(e);  // assign last to prevent cyclic reference
        }
...
void AutogradRuntime::ComputeGradient(const std::vector<NDArray>& outputs,
    ...
    for (const auto& i : outputs) {
        CHECK(!i.entry_.is_none())
            << "Cannot differentiate node because it is not in a computational "
               "graph. "
            << "You need to set is_training to true or use a train_section to "
               "save "
            << "computational graphs for backward. If you want to "
    ...
    AGDFSVisit(heads, [&](const AGNodePtr& n) {
        if (n->nn_node->is_variable()) {
            ...
            if (fmutate_inputs.count(n->nn_node->op())) {
                for (uint32_t i :
                     fmutate_inputs[n->nn_node->op()](n->nn_node->attrs)) {
    ...
    if (args.size()) {
        ...
        exec->Init(sym, args[0].ctx(), ctx_map, args, args_grad, grad_reqs,
        ...
        for (size_t i = 0; i < ograds.size(); ++i) {
            if (ograds[i].is_none()) {
                head_grads.emplace_back(exec->output_arrays_[i].shape(),
                                        exec->output_arrays_[i].ctx(), false,
                                        exec->output_arrays_[i].dtype());
