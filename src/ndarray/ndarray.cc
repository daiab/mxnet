...
NDArray NDArray::Reshape(const TShape &shape) const {
    ...
    if (AutogradRuntime::Get()->IsTraining()) {
        CHECK_GE(shape_.Size(), shape.Size())
            << "NDArray.Reshape: target shape must have must have the same "
               "size as "
        ...
        AutogradRuntime::Get()->RecordImperativeFCompute(op, attrs, &inputs,
                                                         &outputs);
        return outputs[0];
    } else {
        CHECK_GE(shape_.Size(), shape.Size())
...
void NDArray::set_fresh_out_grad(bool state) const {
    CHECK(entry_.ag_node != nullptr) << "NDArray has not been marked as a "
                                        "variable and does not have gradient "
                                        "state";
...
void TernaryOp(const NDArray &lhs, const NDArray &mhs, const NDArray &rhs,
    ...
    if (out->is_none()) {
        *out = NDArray(OP::GetShape(lhs.shape(), mhs.shape(), rhs.shape()),
        ...
        CHECK(out->shape() ==
              OP::GetShape(lhs.shape(), mhs.shape(), rhs.shape()))
    ...
    switch (lhs.ctx().dev_mask()) {
        case cpu::kDevMask: {
            Engine::Get()->PushSync(
                [lhs, mhs, rhs, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::Eval<cpu, OP>(lhs.data(), mhs.data(), rhs.data(),
                                           &tmp, ctx);
                },
        ...
        case gpu::kDevMask: {
            Engine::Get()->PushSync(
                [lhs, mhs, rhs, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::Eval<gpu, OP>(lhs.data(), mhs.data(), rhs.data(),
                                           &tmp, ctx);
...
void CopyFromTo(const NDArray &from, NDArray *to, int priority) {
    ...
    CHECK(from.shape() == to->shape()) << "operands shape mismatch"
    ...
    if (a == cpu::kDevMask && b == cpu::kDevMask) {
        Engine::Get()->PushSync(
            [from, ret](RunContext ctx) {
                TBlob tmp = ret.data();
                ndarray::Copy<cpu, cpu>(from.data(), &tmp, from.ctx(),
                                        ret.ctx(), ctx);
            },
            from.ctx(), const_vars, {ret.var()}, FnProperty::kNormal, priority,
        ...
        if (a == cpu::kDevMask && b == gpu::kDevMask) {
            Engine::Get()->PushSync(
                [from, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::Copy<cpu, gpu>(from.data(), &tmp, from.ctx(),
                                            ret.ctx(), ctx);
                    // Wait GPU kernel to complete
                    ctx.get_stream<gpu>()->Wait();
                },
                ret.ctx(), const_vars, {ret.var()}, FnProperty::kCopyToGPU,
                priority, PROFILER_MESSAGE("CopyCPU2GPU"));
        } else if (a == gpu::kDevMask && b == cpu::kDevMask) {
            Engine::Get()->PushSync(
                [from, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::Copy<gpu, cpu>(from.data(), &tmp, from.ctx(),
                                            ret.ctx(), ctx);
                    // Wait GPU kernel to complete
                    ctx.get_stream<gpu>()->Wait();
                },
            ...
            Engine::Get()->PushSync(
                [from, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::Copy<gpu, gpu>(from.data(), &tmp, from.ctx(),
                                            ret.ctx(), ctx);
                    // Wait GPU kernel to complete
                    ctx.get_stream<gpu>()->Wait();
                },
                from.ctx(), const_vars, {ret.var()},
                from.dtype() != ret.dtype() ? FnProperty::kNormal
    ...
    switch (out->ctx().dev_mask()) {
        case cpu::kDevMask: {
            Engine::Get()->PushSync(
                ...
                out->ctx(), const_vars, {ret.var()}, FnProperty::kNormal,
                priority, PROFILER_MESSAGE_FUNCNAME);
            break;
        }
#if MXNET_USE_CUDA
        case gpu::kDevMask: {
        ...
        case cpu::kDevMask: {
            Engine::Get()->PushSync(
                [a, b, resource, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::EvalRandom<cpu, Distribution>(a, b, resource, &tmp,
                                                           ctx);
                },
                out->ctx(), {}, {ret.var(), resource.var}, FnProperty::kNormal,
                0, PROFILER_MESSAGE_FUNCNAME);
            break;
        }
#if MXNET_USE_CUDA
        case gpu::kDevMask: {
            Engine::Get()->PushSync(
                [a, b, resource, ret](RunContext ctx) {
                    TBlob tmp = ret.data();
                    ndarray::EvalRandom<gpu, Distribution>(a, b, resource, &tmp,
                                                           ctx);
                    // Wait GPU kernel to complete
                    ctx.get_stream<gpu>()->Wait();
                },
...
void NDArray::SyncCopyFromCPU(const void *data, size_t size) const {
    ...
    TBlob src((void *)data, dshape, cpu::kDevMask, this->dtype_,
              0);  // NOLINT(*)
    ...
    if (this->ctx().dev_mask() == cpu::kDevMask) {
        ...
        rctx.stream = nullptr;
        TBlob dst = this->data();
        ndarray::Copy<cpu, cpu>(src, &dst, Context::CPU(), Context::CPU(),
                                rctx);
    } else {
#if MXNET_USE_CUDA
        Engine::Get()->PushSync(
            [&](RunContext rctx) {
                TBlob dst = this->data();
                ndarray::Copy<cpu, gpu>(src, &dst, Context::CPU(), this->ctx(),
                                        rctx);
                // Wait GPU kernel to complete
...
void NDArray::SyncCopyToCPU(void *data, size_t size) const {
    ...
    if (this->ctx().dev_mask() == cpu::kDevMask) {
        ...
        RunContext rctx;
        rctx.stream = nullptr;
        ndarray::Copy<cpu, cpu>(this->data(), &dst, Context::CPU(),
                                Context::CPU(), rctx);
    } else {
#if MXNET_USE_CUDA
        Engine::Get()->PushSync(
...
void Imdecode(NDArray *ret, NDArray mean, size_t index, size_t x0, size_t y0,
    ...
    if (ret->is_none()) {
        *ret = NDArray(
            mshadow::Shape3(n_channels, y1 - y0, x1 - x0), Context::CPU(),
            false, mean.is_none() ? mshadow::default_type_flag : mean.dtype());
    }
    NDArray buff;
    if (ret->shape().ndim() == 3) {
        buff = ret->Reshape(mshadow::Shape4(1, ret->shape()[0], ret->shape()[1],
                                            ret->shape()[2]));
    } else {
    ...
    if (mean.is_none()) {
        MSHADOW_TYPE_SWITCH(buff.dtype(), DType, {
            mshadow::Tensor<cpu, 4, DType> tensor =
                buff.data().get<cpu, 4, DType>();
        ...
        MSHADOW_TYPE_SWITCH(buff.dtype(), DType, {
            mshadow::Tensor<cpu, 4, DType> tensor =
                buff.data().get<cpu, 4, DType>();
            mshadow::Tensor<cpu, 3, DType> tmean =
                mean.data().get<cpu, 3, DType>();
            for (index_t i = 0; i < y1 - y0; i++) {
                uchar *im_data = res.ptr<uchar>(y0 + i) + res.channels() * x0;
                for (index_t j = 0; j < x1 - x0; j++) {
                    for (index_t k = 0; k < n_channels; k++) {
