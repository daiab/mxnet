...
int MXPredCreatePartialOut(const char* symbol_json_str, const void* param_bytes,
    ...
    // load in the symbol.
    {
        nnvm::Graph g;
        g.attrs["json"] =
            std::make_shared<nnvm::any>(std::string(symbol_json_str));
    ...
    if (num_output_nodes != 0) {
        ...
        for (mx_uint i = 0; i < num_output_nodes; ++i) {
            ...
            for (size_t j = 0; j < all_out.size(); ++j) {
                ...
                CHECK_NE(j, all_out.size() - 1) << "didn't find node name: "
                                                << out_key;
            }
        }
    ...
    try {
        ...
        bool infer_complete =
            (g.GetAttr<size_t>("shape_num_unknown_nodes") == 0);
    ...
    // bind
    {
        ...
        ret->exec.reset(Executor::Bind(sym, ctx, ctx_map, arg_arrays,
