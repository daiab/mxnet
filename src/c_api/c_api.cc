...
int MXNDArraySlice(NDArrayHandle handle, mx_uint slice_begin, mx_uint slice_end,
    ...
    NDArray *ptr = new NDArray();
    API_BEGIN();
    *ptr = static_cast<NDArray *>(handle)->Slice(slice_begin, slice_end);
    *out = ptr;
    API_END_HANDLE_ERROR(delete ptr);
}
...
int MXFuncGetInfo(FunctionHandle fun, const char **name,
    ...
    return MXAPIGetFunctionRegInfo(
        static_cast<const NDArrayFunctionReg *>(fun), name, description,
        num_args, arg_names, arg_type_infos, arg_descriptions, return_type);
}
...
int MXKVStoreSetBarrierBeforeExit(KVStoreHandle handle,
    ...
    API_BEGIN();
    static_cast<KVStore *>(handle)->set_barrier_before_exit(
        barrier_before_exit);
    API_END();
}
...
int MXKVStoreRunServer(KVStoreHandle handle,
    ...
    auto ctrl = [controller_temp, controller_handle_temp](
        int head, const std::string &body) {
...
int MXRtcCreate(char *name, mx_uint num_input, mx_uint num_output,
    ...
    for (mx_uint i = 0; i < num_input; ++i) {
        input.push_back(std::pair<std::string, NDArray>(
            input_names[i], *reinterpret_cast<NDArray *>(inputs[i])));
    }
    for (mx_uint i = 0; i < num_output; ++i) {
        output.push_back(std::pair<std::string, NDArray>(
            output_names[i], *reinterpret_cast<NDArray *>(outputs[i])));
    }
...
int MXRtcPush(RtcHandle handle, mx_uint num_input, mx_uint num_output,
    ...
    reinterpret_cast<MXRtc *>(handle)->push(input, output, gridDimX, gridDimY,
                                            gridDimZ, blockDimX, blockDimY,
                                            blockDimZ);
#else
    LOG(FATAL) << "Need to compile with USE_CUDA=1 and USE_NVRTC=1 for MXRtc.";
#endif  // ((MXNET_USE_CUDA) && (MXNET_USE_NVRTC))
