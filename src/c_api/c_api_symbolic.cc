        ...
        for (const auto &k : kHiddenKeys) {
            ...
            if (pos == 0) {
                ...
                break;
                       ...
                       pos == tmp.length() - k.length()) {
                std::ostringstream os;
                os << "setting variable attributes with " << keys[i]
                   << " is deprecated. "
                   << "please instead use\nw = Variable(" << k << "=" << vals[i]
                   << ")\n"
                   << "sym = YourSymbolName(" << tmp.substr(0, pos - 1)
                   << "=w)";
                throw dmlc::Error(os.str());
            }
        }
        if (!flag) kwargs.insert({std::string(keys[i]), std::string(vals[i])});
    }
...
int MXCachedCreateSymbol(CachedOpHandle handle, const char *name,
    ...
    *s = nnvm::Symbol::CreateFunctor(*attrs);
    ...
    s->Compose(parg, std::unordered_map<std::string, const nnvm::Symbol *>(),
               name);
    *out = s;
    API_END_HANDLE_ERROR(delete s;)
}
    ...
    for (const auto &k : kHiddenKeys) {
        ...
        if (pos == 0 && k.length() == skey.length()) {
            ...
            break;
                   ...
                   pos + k.length() == skey.length()) {
            std::ostringstream os;
            os << "setting variable attributes with " << key
               << " is deprecated. "
               << "please instead use\nw = Variable(" << k << "=" << value
               << ")\n"
...
int MXSymbolListAttrShallow(SymbolHandle symbol, mx_uint *out_size,
    ...
    MXAPIThreadLocalEntry *ret = MXAPIThreadLocalStore::Get();
    API_BEGIN();
    std::unordered_map<std::string, std::string> attr = s->ListAttrs(
        static_cast<nnvm::Symbol::ListAttrOption>(1));  // NOLINT(*)
...
void MatchArguments(
    ...
    if (nmatched != known_arg_attrs.size()) {
        ...
        for (const auto &kv : known_arg_attrs) {
            ...
            if (keys.count(key) == 0) {
                LOG(FATAL) << source << "Keyword argument name " << key
                           << " not found." << msg.str();
            }
        }
    }
...
int MXSymbolInferShape(SymbolHandle sym, mx_uint num_args, const char **keys,
    ...
    if (keys == nullptr && num_args != 0) {
        ...
        for (mx_uint i = 0; i < num_args; ++i) {
            arg_shapes[read_only_args[i]] =
                nnvm::ShapeTypeCast(arg_shape_data + arg_ind_ptr[i],
...
int MXSymbolInferType(SymbolHandle sym, mx_uint num_args, const char **keys,
    ...
    if (keys == nullptr && num_args != 0) {
        ...
        mxnet::MatchArguments(g.indexed_graph(), kwargs, &arg_types,
                              "InferType");
    }
