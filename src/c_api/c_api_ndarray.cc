...
void SetOpAttrs(const nnvm::Op* op, nnvm::NodeAttrs* p_attrs,
                const int& num_inputs, const int& num_params,
...
void SetNumOutputs(const nnvm::Op* op, const nnvm::NodeAttrs& attrs,
                   const int& num_inputs, int* infered_num_outputs,
                   int* num_visible_outputs) {
    static auto& visible_out =
...
void SetNDInputsOutputs(const nnvm::Op* op, std::vector<NDArray>* p_ndinputs,
                        std::vector<NDArray>* p_ndoutputs,
                        const int& num_inputs, const NDArrayHandle* inputs,
                        int* num_outputs, const int& infered_num_outputs,
                        const int& num_visible_outputs, NDArray** outarray) {
    ...
    if (outarray == nullptr) {
        *num_outputs = num_visible_outputs;
        ...
        CHECK(!AutogradRuntime::Get()->IsTraining())
            << "Inplace operations (+=, -=, op(..., out=x) etc.) and "
               "assignment are "
            << "not supported when you are inside a train_section using "
               "autograd.";
...
void SetContext(Context* p_ctx, const nnvm::NodeAttrs& attrs,
                const int& num_inputs, const std::vector<NDArray>& ndinputs,
...
void SetShapeType(const nnvm::Op* op, const nnvm::NodeAttrs& attrs,
                  const Context& ctx, const std::vector<NDArray>& ndinputs,
                  const int& infered_num_outputs,
                  std::vector<NDArray>* p_ndoutputs) {
    std::vector<NDArray>& ndoutputs = *p_ndoutputs;
    static auto& infershape =
        nnvm::Op::GetAttr<nnvm::FInferShape>("FInferShape");
    ...
    CHECK(infershape.count(op)) << "Operator " << op->name
                                << " is missing FInferShape attribute";
    ...
    CHECK(infertype.count(op)) << "Operator " << op->name
                               << " is missing FInferType attribute";
...
void SetDependency(std::vector<engine::VarHandle>* p_read_vars,
                   ...
                   std::vector<uint32_t>* p_auxidx, const nnvm::Op* op,
                   const nnvm::NodeAttrs& attrs, const Context& ctx,
                   const std::vector<NDArray>& ndinputs,
                   const std::vector<NDArray>& ndoutputs) {
    static auto& mutate =
        nnvm::Op::GetAttr<nnvm::FMutateInputs>("FMutateInputs");
    static auto& tmp_resource =
...
void PushFCompute(const FCompute& fn, const nnvm::Op* op,
                  const nnvm::NodeAttrs& attrs, const Context& ctx,
    ...
    Engine::Get()->PushAsync(
        [ctx, attrs, fn, ndinputs, ndoutputs, requested, is_train](
            ...
            OpContext opctx{is_train, rctx, engine::CallbackOnComplete(),
                            requested};
...
void PushOperator(std::shared_ptr<Operator> opr, const nnvm::Op* op,
                  const nnvm::NodeAttrs& attrs, const Context& ctx,
                  const std::vector<engine::VarHandle>& read_vars,
                  const std::vector<engine::VarHandle>& write_vars,
                  const std::vector<Resource>& requested,
            ...
            OpContext opctx{is_train, rctx,
                            Engine::Get()->CreateCallback(
                                [](Engine* engine, void* cpt_handle) {
                                    Capture* cpt =
                                        static_cast<Capture*>(cpt_handle);
                                    cpt->on_complete();
                                    delete cpt;
                                },
...
void ImperativeInvokeImpl(const nnvm::NodeAttrs& attrs, int num_inputs,
    ...
    int num_visible_outputs;
    SetNumOutputs(op, attrs, num_inputs, &infered_num_outputs,
                  &num_visible_outputs);

    std::vector<NDArray> ndinputs, ndoutputs;
    SetNDInputsOutputs(op, &ndinputs, &ndoutputs, num_inputs, inputs,
                       num_outputs, infered_num_outputs, num_visible_outputs,
                       outarray);
    ...
    if (ndfunc.count(op)) {
        ...
        Context ctx;
        SetContext(&ctx, attrs, num_inputs, ndinputs, infered_num_outputs,
                   ndoutputs);
        ...
        SetDependency(&read_vars, &write_vars, &requested, &auxidx, op, attrs,
                      ctx, ndinputs, ndoutputs);
        ...
        if (fn) {
            if (AutogradRuntime::Get()->IsTraining()) {
                AutogradRuntime::Get()->RecordImperativeFCompute(
                    op, attrs, &ndinputs, &ndoutputs);
            }
            PushFCompute(fn, op, attrs, ctx, read_vars, write_vars, requested,
                         ndinputs, ndoutputs);
            ...
            if (AutogradRuntime::Get()->IsTraining()) {
                AutogradRuntime::Get()->RecordImperativeOperator(
                    opr, op, attrs, &ndinputs, &ndoutputs);
            }
            PushOperator(opr, op, attrs, ctx, read_vars, write_vars, requested,
                         auxidx, ndinputs, ndoutputs);
        } else {
            LOG(FATAL) << "Operator " << op->name
                       << " cannot be run; requires at least one of"
                       << " FCompute<xpu>, NDArrayFunction, FCreateOperator be "
                          "registered";
        }
    }
    ...
    if (outarray == nullptr) {
        ...
        for (int i = 0; i < num_visible_outputs; ++i) {
            ret->ret_handles.push_back(reinterpret_cast<NDArrayHandle>(
                new NDArray(std::move(ndoutputs[i]))));
        }
...
int MXCachedInvoke(CachedOpHandle handle, int num_inputs, NDArrayHandle* inputs,
