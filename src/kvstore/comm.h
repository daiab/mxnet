...
class Comm {
   public:
    Comm() {
        ...
        pinned_ctx_ =
            (ret == 0 && gpu_num > 0) ? Context::CPUPinned(0) : Context::CPU();
#else
...
class CommCPU : public Comm {
   public:
    CommCPU() {
        nthread_reduction_ =
            dmlc::GetEnv("MXNET_KVSTORE_REDUCTION_NTHREADS", 4);
        bigarray_bound_ =
    ...
    const NDArray& Reduce(int key, const std::vector<NDArray>& src,
        ...
        Engine::Get()->PushSync(
            [reduce, this](RunContext rctx) { ReduceSumCPU(reduce); },
   ...
   private:
    ...
    inline static void ReduceSumCPU(const std::vector<DType*>& dptr,
        ...
        for (size_t i = 1; i < dptr.size(); i += 4) {
            switch (dptr.size() - i) {
                ...
                case 2: {
                    Tensor<cpu, 1, DType> in_1(dptr[i] + offset, Shape1(size));
                    Tensor<cpu, 1, DType> in_2(dptr[i + 1] + offset,
                                               Shape1(size));
                    in_0 += in_1 + in_2;
                    break;
                }
                case 3: {
                    ...
                    Tensor<cpu, 1, DType> in_2(dptr[i + 1] + offset,
                                               Shape1(size));
                    Tensor<cpu, 1, DType> in_3(dptr[i + 2] + offset,
                                               Shape1(size));
                    in_0 += in_1 + in_2 + in_3;
                    break;
                }
                default: {
                    ...
                    Tensor<cpu, 1, DType> in_3(dptr[i + 2] + offset,
                                               Shape1(size));
                    Tensor<cpu, 1, DType> in_4(dptr[i + 3] + offset,
                                               Shape1(size));
    ...
    inline void ReduceSumCPUImpl(std::vector<DType*> dptr, size_t total) {
        const size_t step =
            std::min(bigarray_bound_, static_cast<size_t>(4 << 10));
   ...
   public:
    ...
    const NDArray& Reduce(int key, const std::vector<NDArray>& src,
        ...
        if (buf.copy_buf.empty()) {
            ...
            // ndarray,
            // such as the largest fullc in VGG. consider to do segment reduce
            // with
            // NDArray.Slice or gpu direct memory access. for the latter, we
            // need to
