...
class KVStoreDist : public KVStoreLocal {
   public:
    explicit KVStoreDist(bool use_device_comm)
        ...
        bigarray_bound_ =
            dmlc::GetEnv("MXNET_KVSTORE_BIGARRAY_BOUND", 1000 * 1000);
    }
    ...
    void Push(const std::vector<int>& keys, const std::vector<NDArray>& values,
              int priority) override {
        Push_(keys, values, priority, true);
    }

    void Pull(const std::vector<int>& keys, const std::vector<NDArray*>& values,
              int priority) override {
        ...
        for (size_t i = 0; i < uniq_keys.size(); ++i) {
            int key = uniq_keys[i];
            // use the same array for merging to guarantee that pull always
            // happens
            // after the previous push on this key
            auto& recv_buf = comm_buf_[key];
            if (recv_buf.is_none()) {
                // it may happen for the first time a no-rank-0 worker pull the
                // weight.
            ...
            auto pull_from_servers = [this, key, data, size](
                ...
                CHECK_NOTNULL(ps_worker_)
                    ->ZPull(pskv.keys, vals, &pskv.lens, 0, [vals, cb]() {
                        delete vals;
                        cb();
                    });
            };
            ...
            CHECK_NOTNULL(Engine::Get())
                ->PushAsync(pull_from_servers, pinned_ctx_, {},
                            {recv_buf.var()}, FnProperty::kNormal, priority,
    ...
    void SendCommandToServers(int cmd_id,
        ...
        CHECK_NOTNULL(ps_worker_);
        ps_worker_->Wait(
            ps_worker_->Request(cmd_id, cmd_body, ps::kServerGroup));
    }
    ...
    int get_num_dead_node(int node_id, int timeout) const override {
        ...
        std::unordered_set<int> watch_set(watch_nodes.begin(),
                                          watch_nodes.end());
   ...
   private:
    void Push_(const std::vector<int>& keys, const std::vector<NDArray>& values,
               int priority, bool do_merge) {
        ...
        for (size_t i = 0; i < uniq_keys.size(); ++i) {
            ...
            NDArray merged =
                do_merge ? comm_->Reduce(key, vals, priority) : vals[0];
            ...
            if (merged.ctx().dev_mask() == cpu::kDevMask) {
                ...
                if (send_buf.is_none()) {
                    send_buf = NDArray(merged.shape(), pinned_ctx_, false,
                                       merged.dtype());
                }
            ...
            auto push_to_servers = [this, key, data, size](
                RunContext rctx, Engine::CallbackOnComplete cb) {
                // convert to ps keys
                PSKV& pskv = EncodeKey(key, size);
                ...
                CHECK_NOTNULL(ps_worker_)
                    ->ZPush(pskv.keys, vals, pskv.lens, 0, [cb]() { cb(); });
            };
            Engine::Get()->PushAsync(push_to_servers, pinned_ctx_,
                                     {send_buf.var()}, {}, FnProperty::kNormal,
                                     priority,
    ...
    inline PSKV& EncodeKey(int key, size_t size) {
        ...
        if (!pskv.keys.empty()) {
            CHECK_EQ(static_cast<size_t>(pskv.size), size)
                << "The value size cannot be changed";
        } else {
            ...
            if (size < bigarray_bound_) {
                ...
                for (int i = 0; i < num_servers; ++i) {
                    size_t part_size =
                        static_cast<size_t>(round(static_cast<double>(size) /
                                                  num_servers * (i + 1))) -
                        static_cast<size_t>(
                            round(static_cast<double>(size) / num_servers * i));
