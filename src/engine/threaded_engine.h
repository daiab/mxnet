...
class ThreadedVar final : public Var,
   ...
   public:
     ...
     *             need to be initialized with next==nullptr and
     * trigger=nullptr.
     */
     ...
     *  This function may trigger subsequent waiting operations on this
     * variable.
     *
     ...
     *            when all of its dependencies are satiesfied.
     * \tparam Dispatcher the function called to trigger an operation.
     * \return to_delete, whether this Variable can be deleted after this
     * functin.
     */
    template <typename Dispatcher>
...
class ThreadedEngine : public Engine {
   public:
    ...
    ThreadedEngine() {
        ...
        objpool_varblk_ref_ =
            common::ObjectPool<VersionedVarBlock>::_GetSharedRef();
        ...
        if (!shutdown_phase_) {
            try {
                ...
                if (what.find("driver shutting down") == std::string::npos &&
                    ...
                    LOG(FATAL)
                        << e.what() << "\n"
                        << "An fatal error occurred in asynchronous engine "
                           "operation. "
                           "If you do not know what caused this error, "
                           "you can try set environment variable "
                           "MXNET_ENGINE_TYPE "
                           "to NaiveEngine and run with debugger (i.e. gdb). "
                           "This will force all operations to be synchronous "
                           "and "
                           "backtrace will give you the series of calls that "
                           "lead "
                           "to this error. Remember to set MXNET_ENGINE_TYPE "
                           "back to "
