...
void ThreadedEngine::CheckDuplicate(
    ...
    for (std::size_t i = 0; i < use_size; ++i) {
        ...
        if (mutate.at(j) == use.at(i)) {
            LOG(FATAL) << "duplicate items found between `const_vars` and "
                          "`mutable_vars`";
        }
    }
}
...
void ThreadedEngine::Push(OprHandle op, Context exec_ctx, int priority,
    ...
    opr_block->wait.store(static_cast<int>(threaded_opr->const_vars.size() +
                                           threaded_opr->mutable_vars.size() +
                                           1));
...
void ThreadedEngine::PushAsync(AsyncFn fn, Context exec_ctx,
                               ...
                               const char* opr_name) {
    ThreadedOpr* opr =
        NewOperator(std::move(fn), const_vars, mutable_vars, prop, opr_name);
    opr->temporary = true;
...
void ThreadedEngine::DeleteVariable(SyncFn delete_fn, Context exec_ctx,
                                    VarHandle var) {
...
void ThreadedEngine::WaitForVar(VarHandle var) {
        ...
        PROFILER_MESSAGE("WaitForVar"));
    {
        std::unique_lock<std::mutex> lock{finished_m_};
        finished_cv_.wait(
            lock, [this, &done]() { return done.load() || kill_.load(); });
    }
}
...
void ThreadedEngine::WaitForAll() {
    std::unique_lock<std::mutex> lock{finished_m_};
    finished_cv_.wait(
        lock, [this]() { return pending_.load() == 0 || kill_.load(); });
}
