...
class ThreadedEnginePerDevice : public ThreadedEngine {
   public:
    ...
    ThreadedEnginePerDevice() noexcept(false) {
        ...
        // create CPU task
        int cpu_priority_nthreads =
            dmlc::GetEnv("MXNET_CPU_PRIORITY_NTHREADS", 4);
        ...
        cpu_priority_worker_->pool.reset(new ThreadPool(
            ...
            [this] { this->CPUWorker(cpu_priority_worker_.get()); }));
        ...
        if (opr_block->opr->prop == FnProperty::kAsync && pusher_thread) {
            ...
            if (ctx.dev_mask() == cpu::kDevMask) {
                if (opr_block->opr->prop == FnProperty::kCPUPrioritized) {
                    cpu_priority_worker_->task_queue.Push(opr_block,
                                                          opr_block->priority);
                } else {
                    ...
                    cpu_normal_workers_
                        .Get(dev_id,
                             [this, dev_id, nthread]() {
                                 auto blk =
                                     new ThreadWorkerBlock<kWorkerQueue>();
                                 blk->pool.reset(new ThreadPool(
                                     nthread,
                                     [this, blk]() { this->CPUWorker(blk); }));
                                 return blk;
                             })
                ...
                if (is_copy) {
                    gpu_copy_workers_
                        .Get(dev_id,
                             [this, dev_id, is_copy, nthread]() {
                                 auto blk = new ThreadWorkerBlock<kCopyQueue>();
                                 blk->pool.reset(new ThreadPool(
                                     nthread, [this, dev_id, is_copy, blk]() {
   ...
   private:
    ...
    struct ThreadWorkerBlock {
        ...
        // destructor
        ~ThreadWorkerBlock() noexcept(false) { task_queue.SignalForKill(); }
    };
