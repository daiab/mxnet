    ...
    if (v.size() == 1) {
        ...
        if (v.size() < inplace_sum_cap) {
            ...
            for (size_t i = 1; i < v.size(); ++i) {
                ...
                // NOTE: adding control dependency can be dangerous and cause
                // cycle in the dep.
                // The curent usage is correct, because of the following
                // invariant:
                // assert: v[i-1] do not depend on v[i]
                // To put in plain text: v is gradient vector that get pushed in
                // the order
                // that can generate them, which means if v[i] is not yet
                // pushed,
...
nnvm::Graph GraphExecutor::InitFullGraph(
    nnvm::Symbol symbol, const std::vector<OpReqType>& grad_req_type,
    ...
    // take gradient
    nnvm::Graph g_grad =
        nnvm::pass::Gradient(g, symbol.outputs, xs, head_grad_entry_,
                             AggregateGradient, need_mirror, nullptr, zero_ops);
...
Graph AssignContext(
    Graph g, const Context& default_ctx,
    const std::map<std::string, Context>& ctx_map,
    const std::vector<NDArray>& in_args,
    const std::vector<std::pair<OpReqType, NDArray> >& grad_store,
    const std::vector<NDArray>& aux_states, size_t num_forward_inputs,
    ...
    if (ctx_map.size() == 0) {
        ...
        for (const auto& x : grad_store) {
            CHECK(x.second.ctx() == default_ctx)
                << "Gradient array is in " << x.second.ctx()
                << " while binding with ctx=" << default_ctx
...
void GraphExecutor::Init(nnvm::Symbol symbol, const Context& default_ctx,
    ...
    }
    {
        ...
        for (size_t i = num_forward_inputs_; i < idx.input_nodes().size();
             ++i) {
...
Graph GraphExecutor::InitGraph(nnvm::Symbol symbol, const Context& default_ctx,
    ...
    g = AssignContext(g, default_ctx, ctx_map, in_args, grad_store_, aux_states,
...

    {
        ...
        nnvm::StorageVector arg_storage_id(idx.num_node_entries(),
                                           kBadStorageID);
        for (size_t j = num_forward_outputs_; j < idx.outputs().size(); ++j) {
        ...
        g.attrs["storage"] =
            std::make_shared<dmlc::any>(std::move(arg_storage_id));
...
void GraphExecutor::InitDataEntryMemory(std::vector<NDArray>* shared_pool) {
    ...
    if (shared_pool != nullptr) {
        for (const NDArray& nd : *shared_pool) {
            size_t bytes =
                nd.shape().Size() * mshadow::mshadow_sizeof(nd.dtype());
    ...
    for (size_t i : sorted_pool_index) {
        ...
        for (auto it = free_pool.lower_bound(bytes); it != free_pool.end();
             ++it) {
...
void GraphExecutor::InitCachedOps() {
    ...
    for (uint32_t nid = 0; nid < idx.num_nodes(); ++nid) {
        ...
        // setup exec vars
        Engine::Get()->PushSync(
            [exec](RunContext rctx) { exec->Setup(); }, Context::CPU(), {},
            all_vars, FnProperty::kNormal, 0, PROFILER_MESSAGE("SetupExec"));
    ...
    if (prefer_bulk_exec > 0) {
        ...
        for (size_t nid = 0; nid < num_forward_nodes_; nid++) {
            ...
            if (node->is_variable() || nid - topo_start > num_nodes_threshold ||
                ...
                // create a new segment for the previous nodes if the current
                // one cannot be bulked
                cached_seg_opr_[topo_start] =
                    this->CreateCachedSegOpr(topo_start, nid);
                topo_start = nid + 1;
            }
        }
        // the last segmenet
    ...
    if (prefer_bulk_exec) {
        ...
        for (size_t nid = num_forward_nodes_; nid < total_num_nodes; nid++) {
            ...
            if (idx[nid].source->is_variable() ||
                ...
                // If it produces output gradient, don't include it in the
                // segment
...
#if MXNET_USE_PROFILER
            bool profiling = engine::Profiler::Get()->GetState() ==
                             engine::Profiler::kRunning;
#else
