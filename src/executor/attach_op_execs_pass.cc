...
class ForwardOpExecutor : public OpExecutor {
   public:
    ...
    void Setup() override {
        ...
        std::transform(out_array.begin(), out_array.end(), out_data_.begin(),
                       [](const NDArray& nd) { return nd.data(); });
    }
    Operator::ExecType exec_type() const override { return op_->exec_type(); }
...
class BackwardOpExecutor : public OpExecutor {
   public:
    void Run(RunContext rctx) override {
        op_ctx.run_ctx = rctx;
        op_->Backward(op_ctx, out_grad_, in_data_, out_data_, req, in_grad_,
                      aux_data_);
#if MKL_EXPERIMENTAL == 1
    ...
    void Setup() override {
        ...
        std::transform(out_array.begin(), out_array.end(), in_grad_.begin(),
                       [](const NDArray& nd) { return nd.data(); });
    }
    Operator::ExecType exec_type() const override { return op_->exec_type(); }
    explicit BackwardOpExecutor(std::shared_ptr<Operator> op,
        ...
        arg_data_ptr_ =
            prop->BackwardInputs(out_grad_ptr, in_data_ptr, out_data_ptr);
    }
...
class FComputeExecutor : public OpExecutor {
   public:
    ...
    void Setup() override {
        ...
        out_data_.resize(out_array.size());
        auto get_blob = [](const NDArray& nd) { return nd.data(); };
        std::transform(in_array.begin(), in_array.end(), in_data_.begin(),
                       get_blob);
        std::transform(out_array.begin(), out_array.end(), out_data_.begin(),
                       get_blob);
    }
    Operator::ExecType exec_type() const override { return Operator::kSync; }
    ...
    static FCompute GetFCompute(const Op* op, Context ctx) {
        static auto& fcompute_cpu =
            nnvm::Op::GetAttr<FCompute>("FCompute<cpu>");
        static auto& fcompute_gpu =
            nnvm::Op::GetAttr<FCompute>("FCompute<gpu>");
...
Graph AttachOpExecs(Graph g) {
    ...
    auto& fcreate_layer_op =
        nnvm::Op::GetAttr<FCreateLayerOp>("FCreateLayerOp");
    ...
    const auto& saved_opr = g.GetAttr<
        std::unordered_map<const nnvm::Node*, std::shared_ptr<Operator>>>(
        "saved_opr");
    ...
    for (size_t i = 0; i < idx.num_nodes(); ++i) {
        ...
        if (fmutate_inputs.count(inode.source->op())) {
            mutate_index =
                fmutate_inputs[inode.source->op()](inode.source->attrs);
        }
        FCompute fcompute =
            FComputeExecutor::GetFCompute(inode.source->op(), vctx[i]);
        if (fcreate_layer_op.count(inode.source->op())) {
            ...
            ret[i] = std::make_shared<FComputeExecutor>(fcompute,
                                                        inode.source->attrs);
        } else {
