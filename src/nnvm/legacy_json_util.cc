...
Graph UpgradeJSON_FixParsing(Graph g) {
    nnvm::DFSVisit(g.outputs, [](const std::shared_ptr<Node>& n) {
        static auto& flist_inputs =
            Op::GetAttr<FListInputNames>("FListInputNames");
        ...
        for (auto it = n->attrs.dict.begin(); it != n->attrs.dict.end();) {
            ...
            for (const auto key : kHiddenKeys) {
                ...
                if (pos == 0 || (pos != std::string::npos &&
                                 pos == it->first.length() - key.length())) {
        ...
        for (const auto& kv : hidden_keys) {
            ...
            for (const auto& key : kHiddenKeys) {
                ...
                if (pos == 0 && key.length() == kv.first.length()) {
                    ...
                    auto it =
                        std::find(arg_names.begin(), arg_names.end(), name);
                    if (it != arg_names.end()) {
                        int idx = it - arg_names.begin();
                        if (n->inputs[idx].node->is_variable()) {
                            n->inputs[idx].node->attrs.dict["__" + key + "__"] =
                                kv.second;
                            flag = true;
                        }
                    }
...
Graph UpgradeJSON_Parse(Graph g) {
    nnvm::DFSVisit(g.outputs, [](const std::shared_ptr<Node>& n) {
        if (n->op() != nullptr) {
            ...
            n->attrs.parsed = nnvm::Symbol::CreateVariable(n->attrs.name)
                                  .outputs[0]
                                  .node->attrs.parsed;
        }
    });
        ...
        if (n->inputs.size() < n->num_inputs()) {
            ...
            for (size_t i = n->inputs.size(); i < n->num_inputs(); ++i) {
                auto var = Symbol::CreateVariable(
                               DefaultVarName(n->attrs.name, arg_names[i]))
                               .outputs[0];
...
Graph UpgradeJSON_000903_000904(Graph g) {
    nnvm::DFSVisit(g.outputs, [](const std::shared_ptr<Node>& n) {
        static auto& fset_attrs = Op::GetAttr<nnvm::FSetInputVarAttrOnCompose>(
            "FSetInputVarAttrOnCompose");

        if (n->op() != nullptr) {
            nnvm::FSetInputVarAttrOnCompose fn =
                fset_attrs.get(n->op(), nullptr);
            if (fn != nullptr) {
...
Graph UpgradeJSON_000904_000905(Graph g) {
    nnvm::DFSVisit(g.outputs, [](const std::shared_ptr<Node>& n) {
        ...
        if (n->attrs.dict.find("axis") == n->attrs.dict.end() ||
            n->attrs.dict["axis"] != "-1")
            return;
...
static std::vector<std::pair<int, std::function<Graph(Graph)> > >
    upgrader_list = {
...
Graph LoadLegacyJSONPass(Graph g) {
    ...
    if (version > MXNET_VERSION) {
        LOG(INFO) << "Warning: loading symbol saved by MXNet version "
                  << version << " with lower version of MXNet v"
                  << MXNET_VERSION << ". May cause undefined behavior. "
        ...
        LOG(INFO) << "Loading symbol saved by previous version v"
                  ...
                  << version % 100 << ". Attempting to upgrade...";
        upgrading = true;
    }
...
NNVM_REGISTER_PASS(LoadLegacyJSON)
    .describe(
        "Return a new Graph, loaded from src.attrs[\"json\"] and upgraded to "
        "current version")
