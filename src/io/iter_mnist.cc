...
class MNISTIter : public IIterator<TBlobBatch> {
   public:
    ...
    virtual void Init(
        ...
        this->LoadLabel();
        if (param_.flat) {
            batch_data_.shape_ = mshadow::Shape4(param_.batch_size, 1, 1,
                                                 img_.size(1) * img_.size(2));
        } else {
            batch_data_.shape_ = mshadow::Shape4(param_.batch_size, 1,
                                                 img_.size(1), img_.size(2));
        }
        out_.data.clear();
    ...
    virtual void BeforeFirst(void) { this->loc_ = 0; }
   ...
   private:
    inline void GetPart(int count, int* start, int* end) {
        ...
        *start = static_cast<int>(static_cast<double>(count) /
                                  param_.num_parts * param_.part_index);
        *end = static_cast<int>(static_cast<double>(count) / param_.num_parts *
                                (param_.part_index + 1));
    }
    ...
    inline void Shuffle(void) {
        std::shuffle(inst_.begin(), inst_.end(),
...
#else
        return reinterpret_cast<int>(buf[0] << 24 | buf[1] << 16 | buf[2] << 8 |
                                     buf[3]);
#endif
    }
