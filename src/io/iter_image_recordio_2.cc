...
#pragma omp parallel num_threads(param_.preprocess_threads)
    ...
    if (param_.path_imglist.length() != 0) {
        label_map_.reset(new ImageLabelMap(
    ...
    if (param_.verbose) {
        LOG(INFO) << "ImageRecordIOParser2: " << param_.path_imgrec << ", use "
    ...
    if (!std::is_same<DType, uint8_t>::value) {
        ...
        if (normalize_param_.mean_img.length() != 0) {
            std::unique_ptr<dmlc::Stream> fi(dmlc::Stream::Create(
                normalize_param_.mean_img.c_str(), "r", true));
            if (fi.get() == nullptr) {
                this->CreateMeanImg();
            } else {
                fi.reset(nullptr);
                if (param_.verbose) {
                    LOG(INFO) << "Load mean image from "
                              << normalize_param_.mean_img;
                }
                ...
                {
                    std::unique_ptr<dmlc::Stream> fi(dmlc::Stream::Create(
                        normalize_param_.mean_img.c_str(), "r"));
                    NDArray::Load(fi.get(), &data, &keys);
                }
                CHECK_EQ(data.size(), 1) << "Invalid mean image file format";
                data[0].WaitToRead();
                mshadow::Tensor<cpu, 3> src =
                    data[0].data().get<cpu, 3, real_t>();
    ...
    while (current_size < batch_param_.batch_size) {
        ...
        if (n_parsed_ == 0) {
            if (source_->NextChunk(&chunk)) {
                ...
                n_to_copy =
                    std::min(n_read, batch_param_.batch_size - current_size);
                ...
                CHECK(!overflow) << "number of input images must be bigger "
                                    "than the batch size";
            ...
            n_to_copy =
                std::min(n_parsed_, batch_param_.batch_size - current_size);
            n_parsed_ -= n_to_copy;
        }
        ...
        if (out->data.size() == 0 && n_to_copy != 0) {
            ...
            for (size_t i = 0; i < out->data.size(); ++i) {
                ...
                out->data.at(i) =
                    NDArray(dst_shape, Context::CPU(), false, src_type_flag);
...
template <typename DType>
inline void ImageRecordIOParser2<DType>::ParseChunk(
    dmlc::InputSplit::Blob* chunk) {
...
#pragma omp parallel num_threads(param_.preprocess_threads)
    {
        ...
        while (reader.NextRecord(&blob)) {
            ...
            bool is_mirrored =
                (normalize_param_.rand_mirror && coin_flip(*(prnds_[tid]))) ||
                normalize_param_.mirror;
            float contrast_scaled;
            float illumination_scaled;
            if (!std::is_same<DType, uint8_t>::value) {
                contrast_scaled =
                    (rand_uniform(*(prnds_[tid])) *
                         normalize_param_.max_random_contrast * 2 -
                     normalize_param_.max_random_contrast + 1) *
                    normalize_param_.scale;
                illumination_scaled =
                    (rand_uniform(*(prnds_[tid])) *
                         normalize_param_.max_random_illumination * 2 -
            ...
            for (int i = 0; i < res.rows; ++i) {
                ...
                for (int j = 0; j < res.cols; ++j) {
                    ...
                    if (!std::is_same<DType, uint8_t>::value) {
                        ...
                        if (normalize_param_.mean_r > 0.0f ||
                            normalize_param_.mean_g > 0.0f ||
                            normalize_param_.mean_b > 0.0f ||
                            normalize_param_.mean_a > 0.0f) {
                            ...
                            for (int k = 0; k < n_channels; ++k) {
                                RGBA[k] = RGBA[k] * contrast_scaled +
                                          illumination_scaled;
                            }
                        } else if (!meanfile_ready_ ||
                                   normalize_param_.mean_img.length() == 0) {
            ...
            if (label_map_ != nullptr) {
                ...
                CHECK_EQ(param_.label_width, rec.num_label)
                    ...
                    << param_.label_width;
                mshadow::Copy(label,
                              mshadow::Tensor<cpu, 1>(
                                  rec.label, mshadow::Shape1(rec.num_label)));
            } else {
    ...
    while (source_->NextChunk(&chunk)) {
        ...
        for (unsigned i = 0; i < inst_order_.size(); ++i) {
            ...
            mshadow::Tensor<cpu, 3> outimg =
                temp_[place.first][place.second]
                    .data[0]
                    .template get<cpu, 3, real_t>();
            if (imcnt == 0) {
            ...
            if (imcnt % 10000L == 0 && param_.verbose) {
                LOG(INFO) << imcnt << " images processed, " << elapsed
                          << " sec elapsed";
            }
        }
    }
...
class ImageRecordIter2 : public IIterator<DataBatch> {
   public:
    ...
    virtual ~ImageRecordIter2(void) { iter_.Destroy(); }
    ...
    virtual void BeforeFirst(void) { iter_.BeforeFirst(); }
...
Example::
  ...
  data_iter.reset() # To restart the iterator from the beginning.
    ...
    .set_body([]() { return new ImageRecordIter2<real_t>(); });
