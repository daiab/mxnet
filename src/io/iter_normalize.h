...
class ImageNormalizeIter : public IIterator<DataInst> {
   public:
    ...
    virtual void Init(
        ...
        if (param_.mean_img.length() != 0) {
            ...
            if (fi.get() == nullptr) {
                ...
                {
                    std::unique_ptr<dmlc::Stream> fi(
                        dmlc::Stream::Create(param_.mean_img.c_str(), "r"));
    ...
    virtual void BeforeFirst(void) { base_->BeforeFirst(); }
   ...
   private:
    ...
    inline void SetOutImg(const DataInst &src) {
        ...
        float contrast = rand_uniform(rnd_) * param_.max_random_contrast * 2 -
                         param_.max_random_contrast + 1;
        float illumination =
    ...
    inline void CreateMeanImg(void) {
        ...
        TBlob tmp = meanimg_;
        {
            std::unique_ptr<dmlc::Stream> fo(
                dmlc::Stream::Create(param_.mean_img.c_str(), "w"));
