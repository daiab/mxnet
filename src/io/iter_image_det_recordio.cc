...
class ImageDetLabelMap {
   public:
     ...
     */
    explicit ImageDetLabelMap(const char *path_imglist, int label_width,
                              bool silent) {
        image_index_.clear();
        label_.clear();
        idx2label_.clear();
        dmlc::InputSplit *fi =
            dmlc::InputSplit::Create(path_imglist, 0, 1, "text");
        ...
        while (fi->NextRecord(&rec)) {
            ...
            if (label_width > 0) {
                ...
                CHECK_GT(label_.size(), start_pos)
                    << "Bad ImageList format: empty label";
            }
    ...
    inline std::vector<float> FindCopy(size_t imid) const {
        std::unordered_map<size_t, std::pair<size_t, size_t>>::const_iterator
            it = idx2label_.find(imid);
        CHECK(it != idx2label_.end()) << "fail to find imagelabel for id "
                                      << imid;
...
struct ImageDetRecParserParam : public dmlc::Parameter<ImageDetRecParserParam> {
    ...
    /*! \brief pad label to specified length, -1 for auto estimate in whole
     * dataset */
    int label_pad_width;
    ...
    DMLC_DECLARE_PARAMETER(ImageDetRecParserParam) {
        ...
        DMLC_DECLARE_FIELD(aug_seq)
            ...
            .describe(
                ...
                "augmenters."
                " Make sure you don't use normal augmenters for detection "
                "tasks.");
        DMLC_DECLARE_FIELD(label_width)
            .set_default(-1)
            .describe(
                "Dataset Param: How many labels for an image, -1 for variable "
                "label size.");
        ...
        DMLC_DECLARE_FIELD(shuffle_chunk_seed)
            .set_default(0)
...
class ImageDetRecordIOParser {
   public:
    ...
    inline void BeforeFirst(void) { return source_->BeforeFirst(); }
...
#pragma omp parallel num_threads(param_.preprocess_threads)
    ...
    if (param_.path_imglist.length() != 0) {
        label_map_.reset(new ImageDetLabelMap(
...
#pragma omp parallel num_threads(param_.preprocess_threads)
            {
                ...
                while (reader.NextRecord(&blob)) {
                    ...
                    if (rec.label != NULL) {
                        ...
                        LOG(FATAL) << "Not enough label packed in img_list or "
                                      "rec file.";
                    }
                }
#pragma omp critical
...
template <typename DType>
inline bool ImageDetRecordIOParser<DType>::ParseNext(
    std::vector<InstVector<DType>> *out_vec) {
...
class ImageDetRecordIter : public IIterator<DataInst> {
   public:
    ...
    virtual const DataInst &Value(void) const { return out_; }
...
MXNET_REGISTER_IO_ITER(ImageDetRecordIter)
    ...
    .set_body([]() {
        return new PrefetcherIter(new BatchLoader(
            new ImageDetNormalizeIter(new ImageDetRecordIter<real_t>())));
    });
}  // namespace io
}  // namespace mxnet
