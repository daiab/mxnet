...
class ImageLabelMap {
   public:
    ...
    explicit ImageLabelMap(const char *path_imglist,
                           mshadow::index_t label_width, bool silent) {
        ...
        for (size_t i = 0; i < image_index_.size(); ++i) {
            idx2label_[image_index_[i]] =
                dmlc::BeginPtr(label_) + i * label_width;
        }
        if (!silent) {
    ...
    inline mshadow::Tensor<cpu, 1> Find(size_t imid) const {
        ...
        CHECK(it != idx2label_.end()) << "fail to find imagelabel for id "
                                      << imid;
        return mshadow::Tensor<cpu, 1>(it->second,
                                       mshadow::Shape1(label_width));
    }
    ...
    DMLC_DECLARE_PARAMETER(ImageRecParserParam) {
        DMLC_DECLARE_FIELD(path_imglist)
            ...
            .describe(
                ...
                "<index of record>\t<one or more labels>\t<relative path from "
                "root folder>.");
        ...
        DMLC_DECLARE_FIELD(shuffle_chunk_size)
            .set_default(0)
            .describe(
                "The data shuffle buffer size in MB. Only valid if shuffle is "
                "true.");
