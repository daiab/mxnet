...
bool get_jpeg_size(const uint8_t* data, uint32_t data_size, int64_t* width,
                   int64_t* height) {
    ...
    if (data[i] == 0xFF && data[i + 1] == 0xD8 && data[i + 2] == 0xFF &&
        ...
        if (data[i + 2] == 'J' && data[i + 3] == 'F' && data[i + 4] == 'I' &&
            ...
            while (i < data_size) {
                i += block_length;  // Increase the file index to get to the
                                    // next block
                if (i >= data_size)
                    return false;  // Check to protect against segmentation
                                   // faults
                if (data[i] != 0xFF)
                    return false;  // Check that we are truly at the start of
                                   // another block
                uint8_t m = data[i + 1];
                if (m == 0xC0 || (m >= 0xC1 && m <= 0xCF && m != 0xC4 &&
                                  m != 0xC8 && m != 0xCC)) {
                    // 0xFFC0 is the "Start of frame" marker which contains the
                    // file size
                    // The structure of the 0xFFC0 block is quite simple
                    // [0xFFC0][ushort length][uchar precision][ushort x][ushort
                    // y]
                    ...
                    *width = data[i + 7] * 256 + data[i + 8];
                    ...
                    block_length =
                        data[i] * 256 + data[i + 1];  // Go to the next block
                }
            }
...
bool get_png_size(const uint8_t* data, uint32_t data_size, int64_t* width,
                  int64_t* height) {
...
struct ImdecodeParam : public dmlc::Parameter<ImdecodeParam> {
    ...
    DMLC_DECLARE_PARAMETER(ImdecodeParam) {
        ...
        DMLC_DECLARE_FIELD(to_rgb)
            .set_default(true)
            .describe(
                "Whether to convert decoded image to mxnet's default RGB "
                "format "
...
void Imdecode(const nnvm::NodeAttrs& attrs, const std::vector<NDArray>& inputs,
              std::vector<NDArray>* outputs) {
#if MXNET_USE_OPENCV
    const auto& param = nnvm::get<ImdecodeParam>(attrs.parsed);

    CHECK_EQ(inputs[0].ctx().dev_mask(), cpu::kDevMask)
        << "Only supports cpu input";
    CHECK_EQ(inputs[0].dtype(), mshadow::kUint8)
        << "Input needs to be uint8 buffer";
    ...
    if (get_jpeg_size(str_img, len, &oshape[1], &oshape[0])) {
        ...
        cv::Mat dst(ndout.shape()[0], ndout.shape()[1],
                    param.flag == 0 ? CV_8U : CV_8UC3, ndout.data().dptr_);
        res.copyTo(dst);
    ...
    Engine::Get()->PushSync(
        [ndin, ndout, param](RunContext ctx) {
            cv::Mat buf(1, ndin.shape().Size(), CV_8U, ndin.data().dptr_);
            cv::Mat dst(ndout.shape()[0], ndout.shape()[1],
                        param.flag == 0 ? CV_8U : CV_8UC3, ndout.data().dptr_);
        ...
        ndout.ctx(), {ndin.var()}, {ndout.var()}, FnProperty::kNormal, 0,
        PROFILER_MESSAGE("Imdecode"));
    (*outputs)[0] = ndout;
#else
...
inline void Imresize(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                     const std::vector<TBlob>& inputs,
                     const std::vector<OpReqType>& req,
                     const std::vector<TBlob>& outputs) {
#if MXNET_USE_OPENCV
    CHECK_NE(inputs[0].type_flag_, mshadow::kFloat16)
        << "imresize doesn't support fp16";
    const int DTYPE[] = {CV_32F, CV_64F, -1, CV_8U, CV_32S};
    int cv_type = CV_MAKETYPE(DTYPE[inputs[0].type_flag_], inputs[0].shape_[2]);
    const auto& param = nnvm::get<ResizeParam>(attrs.parsed);
    cv::Mat buf(inputs[0].shape_[0], inputs[0].shape_[1], cv_type,
                inputs[0].dptr_);
    cv::Mat dst(outputs[0].shape_[0], outputs[0].shape_[1], cv_type,
                outputs[0].dptr_);
...
struct MakeBorderParam : public dmlc::Parameter<MakeBorderParam> {
    ...
    DMLC_DECLARE_PARAMETER(MakeBorderParam) {
        ...
        DMLC_DECLARE_FIELD(value).set_default(0.0).describe("Fill with value.");
    }
};
...
inline bool MakeBorderShape(const nnvm::NodeAttrs& attrs,
    ...
    oshape->clear();
    oshape->push_back(mshadow::Shape3(
        (*ishape)[0][0] + param.top + param.bot,
        (*ishape)[0][1] + param.left + param.right, (*ishape)[0][2]));
    return true;
}

inline void copyMakeBorder(const nnvm::NodeAttrs& attrs, const OpContext& ctx,
                           const std::vector<TBlob>& inputs,
                           const std::vector<OpReqType>& req,
                           const std::vector<TBlob>& outputs) {
#if MXNET_USE_OPENCV
    ...
    cv::Mat dst(outputs[0].shape_[0], outputs[0].shape_[1], cv_type,
                outputs[0].dptr_);
    cv::copyMakeBorder(buf, dst, param.top, param.bot, param.left, param.right,
