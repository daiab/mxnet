...
#pragma omp parallel num_threads(param_.preprocess_threads)
    ...
    if (param_.verbose) {
        LOG(INFO) << "ImageRecordIOParser: " << param_.path_imgrec << ", use "
...
template <typename DType>
inline bool ImageRecordIOParser<DType>::ParseNext(
...
files.
    ...
    .set_body([]() {
        return new PrefetcherIter(new BatchLoader(
            new ImageNormalizeIter(new ImageRecordIter<real_t>())));
    });
...
the data type instead of ``float``.
    ...
    .set_body([]() {
        return new PrefetcherIter(
            new BatchLoader(new ImageRecordIter<uint8_t>()));
    });
}  // namespace io
}  // namespace mxnet
