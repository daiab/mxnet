...
struct DefaultImageDetAugmentParam
    : public dmlc::Parameter<DefaultImageDetAugmentParam> {
    ...
    /*! \brief maximum trials for cropping, skip cropping if fails exceed this
     * number */
    ...
    int fill_value;
    /*! \brief interpolation method 0-NN 1-bilinear 2-cubic 3-area 4-lanczos4
     * 9-auto 10-rand  */
    int inter_method;
    ...
    DMLC_DECLARE_PARAMETER(DefaultImageDetAugmentParam) {
        ...
        DMLC_DECLARE_FIELD(rand_crop_prob)
            .set_default(0.0f)
            .describe(
                "Augmentation Param: Probability of random cropping, <= 0 to "
                "disable");
        ...
        DMLC_DECLARE_FIELD(emit_overlap_thresh)
            .set_default(0.3f)
            .describe(
                "Augmentation Param: Emit overlap thresh for emit mode overlap "
                "only.");
        ...
        DMLC_DECLARE_FIELD(max_random_hue)
            .set_default(0)
            .describe(
                "Augmentation Param: Maximum random value of H channel in HSL "
                "color space.");
        ...
        DMLC_DECLARE_FIELD(max_random_saturation)
            .set_default(0)
            .describe(
                "Augmentation Param: Maximum random value of S channel in HSL "
                "color space.");
        ...
        DMLC_DECLARE_FIELD(max_random_illumination)
            .set_default(0)
            .describe(
                "Augmentation Param: Maximum random value of L channel in HSL "
                "color space.");
        ...
        DMLC_DECLARE_FIELD(inter_method)
            .set_default(1)
            .describe(
                "Augmentation Param: 0-NN 1-bilinear 2-cubic 3-area 4-lanczos4 "
                "9-auto 10-rand.");
        ...
        DMLC_DECLARE_FIELD(resize_mode)
            ...
            .describe(
                ...
                "ratio; "
                "shrink: ensure each side fit in data_shape, preserve aspect "
                "ratio; "
                "fit: fit image to data_shape, preserve ratio, will upscale if "
                "applicable.");
    }
};
...
class ImageDetLabel {
   public:
    ...
    struct ImageDetObject {
        ...
        float bottom;
        std::vector<float>
            extra;  // store extra info other than id and coordinates
     ...
     */
    bool TryCrop(const Rect crop_box, const float min_crop_overlap,
                 ...
                 const float max_crop_object_coverage, const int crop_emit_mode,
        ...
        if (min_crop_overlap > 0.f && max_crop_overlap < 1.f &&
            ...
            for (auto &obj : objects_) {
                ...
                if (min_crop_sample_coverage > 0.f ||
                    max_crop_sample_coverage < 1.f) {
                    float c = (crop_box & gt_box).area() / crop_box.area();
                    if (c < min_crop_sample_coverage ||
                        c > max_crop_sample_coverage) {
                        continue;
                    }
                }
                if (min_crop_object_coverage > 0.f ||
                    max_crop_object_coverage < 1.f) {
                    float c = (crop_box & gt_box).area() / gt_box.area();
                    if (c < min_crop_object_coverage ||
                        c > max_crop_object_coverage) {
                        continue;
                    }
                }
                valid = true;
...
class DefaultImageDetAugmenter : public ImageAugmenter {
   public:
    ...
    void Init(const std::vector<std::pair<std::string, std::string> > &kwargs)
        ...
        ValidateCropParameters(&param_.min_crop_scales,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_scales,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.min_crop_aspect_ratios,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_aspect_ratios,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.min_crop_overlaps,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_overlaps,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.min_crop_sample_coverages,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_sample_coverages,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.min_crop_object_coverages,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_object_coverages,
                               param_.num_crop_sampler);
        ValidateCropParameters(&param_.max_crop_trials,
                               param_.num_crop_sampler);
        for (int i = 0; i < param_.num_crop_sampler; ++i) {
            ...
            CHECK_GE(param_.max_crop_aspect_ratios[i],
                     param_.min_crop_aspect_ratios[i]);
            CHECK_GE(param_.max_crop_overlaps[i], param_.min_crop_overlaps[i]);
            CHECK_GE(param_.max_crop_sample_coverages[i],
                     param_.min_crop_sample_coverages[i]);
            CHECK_GE(param_.max_crop_object_coverages[i],
                     param_.min_crop_object_coverages[i]);
        }
        CHECK_GE(param_.emit_overlap_thresh, 0.0f);
    }
    /*!
     * \brief get interpolation method with given inter_method, 0-CV_INTER_NN
     * 1-CV_INTER_LINEAR 2-CV_INTER_CUBIC
     * \ 3-CV_INTER_AREA 4-CV_INTER_LANCZOS4 9-AUTO(cubic for enlarge, area for
    ...
    void ValidateCropParameters(nnvm::Tuple<DType> *param,
                                const int num_sampler) {
        if (num_sampler == 1) {
            ...
            if (param->ndim() == 1) {
                ...
                CHECK_EQ(param->ndim(), num_sampler)
                    << "# of parameters/crop_samplers mismatch ";
            }
        }
    }

    /*! \brief Generate crop box region given cropping parameters */
    Rect GenerateCropBox(const float min_crop_scale, const float max_crop_scale,
        ...
        float new_scale = std::uniform_real_distribution<float>(
                          ...
                          1e-12f;
        float min_ratio = std::max<float>(
            min_crop_aspect_ratio / img_aspect_ratio, new_scale * new_scale);
        float max_ratio =
            std::min<float>(max_crop_aspect_ratio / img_aspect_ratio,
                            1. / new_scale * new_scale);
        float new_ratio = std::sqrt(
            std::uniform_real_distribution<float>(min_ratio, max_ratio)(*prnd));
        float new_width = std::min(1.f, new_scale * new_ratio);
        float new_height = std::min(1.f, new_scale / new_ratio);
        float x0 =
            std::uniform_real_distribution<float>(0.f, 1 - new_width)(*prnd);
        float y0 =
            std::uniform_real_distribution<float>(0.f, 1 - new_height)(*prnd);
        return Rect(x0, y0, new_width, new_height);
    }

    /*! \brief Generate padding box region given padding parameters */
    Rect GeneratePadBox(const float max_pad_scale, common::RANDOM_ENGINE *prnd,
                        const float threshold = 1.05f) {
        float new_scale =
            std::uniform_real_distribution<float>(1.f, max_pad_scale)(*prnd);
        if (new_scale < threshold) return Rect(0, 0, 0, 0);
        auto rand_uniform =
            std::uniform_real_distribution<float>(0.f, new_scale - 1);
    ...
    cv::Mat Process(const cv::Mat &src, std::vector<float> *label,
        ...
        if (param_.resize != -1) {
            ...
            int interpolation_method =
                GetInterMethod(param_.inter_method, src.cols, src.rows,
                               new_width, new_height, prnd);
            cv::resize(src, res, cv::Size(new_width, new_height), 0, 0,
        ...
        if (param_.random_hue_prob > 0.f ||
            param_.random_saturation_prob > 0.f ||
            param_.random_illumination_prob > 0.f ||
            param_.random_contrast_prob > 0.f) {
        ...
        // random mirror logic
        if (param_.rand_mirror_prob > 0 &&
            rand_uniform(*prnd) < param_.rand_mirror_prob) {
                ...
                if (pad_box.area() > 0) {
                    if (det_label.TryPad(pad_box)) {
                        ...
                        int right = static_cast<int>(
                            (pad_box.width + pad_box.x - 1) * res.cols);
                        int bot = static_cast<int>(
                            (pad_box.height + pad_box.y - 1) * res.rows);
                        cv::copyMakeBorder(
                            temp_, res, top, bot, left, right,
                            cv::BORDER_ISOLATED,
                            cv::Scalar(param_.fill_value, param_.fill_value,
                                       param_.fill_value));
                    }
                }
            }
        ...
        if (param_.rand_crop_prob > 0 && param_.num_crop_sampler > 0) {
            if (rand_uniform(*prnd) < param_.rand_crop_prob) {
                // random crop sampling logic: randomly pick a sampler, return
                // if success
                ...
                for (auto idx : indices) {
                    ...
                    for (int t = 0; t < param_.max_crop_trials[idx]; ++t) {
                        Rect crop_box = GenerateCropBox(
                            param_.min_crop_scales[idx],
                        ...
                        if (det_label.TryCrop(
                                crop_box, param_.min_crop_overlaps[idx],
                            ...
                            int width =
                                static_cast<int>(crop_box.width * res.cols);
                            int height =
                                static_cast<int>(crop_box.height * res.rows);
        ...
        if (image_det_aug_default_enum::kForce == param_.resize_mode) {
            ...
            int interpolation_method =
                GetInterMethod(param_.inter_method, res.cols, res.rows,
                               new_width, new_height, prnd);
            cv::resize(res, res, cv::Size(new_width, new_height), 0, 0,
                       interpolation_method);
