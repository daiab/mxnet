   ...
   public:
    ...
    inline mshadow::Tensor<cpu, dim, DType> operator[](size_t i) const {
        ...
        return mshadow::Tensor<cpu, dim, DType>(
            (DType*)dmlc::BeginPtr(content_) + offset_[i],
            shape_[i]);  // NOLINT(*)
    }
...
class InstVector {
   public:
    ...
    // get index
    inline unsigned Index(unsigned i) const { return index_[i]; }
    // instance
    ...
    inline DataInst Back() const { return (*this)[Size() - 1]; }
    inline void Clear(void) {
        index_.clear();
        data_.Clear();
        label_.Clear();
     ...
     */
    inline void Push(unsigned index, mshadow::Shape<3> dshape,
    ...
    inline const TensorVector<3, DType>& data() const { return data_; }
    /*! \return the label content */
    inline const TensorVector<1, real_t>& label() const { return label_; }
...
struct TBlobBatch {
   public:
    ...
    /*! \brief number of padding elements in this batch,
         this is used to indicate the last elements in the batch are only padded
       up to match the batch, and should be discarded */
...
class TBlobContainer : public mshadow::TBlob {
   public:
    TBlobContainer(void) : mshadow::TBlob(), tensor_container_(nullptr) {}
    ~TBlobContainer() {
   ...
   private:
    void create() {
        ...
        MSHADOW_TYPE_SWITCH(this->type_flag_, DType, {
            auto tensor_container =
                new mshadow::TensorContainer<mshadow::cpu, 1, DType>(false);
    ...
    void resize() {
        MSHADOW_TYPE_SWITCH(this->type_flag_, DType, {
            auto tensor_container =
                (mshadow::TensorContainer<mshadow::cpu, 1, DType>*)
                    tensor_container_;
            tensor_container->Resize(mshadow::Shape1(shape_.Size()));
        });
    }
