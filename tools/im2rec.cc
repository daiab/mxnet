 ...
 *shrink, bilinear for others) 10-RAND(0-4)
 */
int GetInterMethod(int inter_method, int old_width, int old_height,
                   int new_width, int new_height, std::mt19937 &prnd) {
    if (inter_method == 9) {
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        printf(
            "Usage: <image.lst> <image_root_dir> <output.rec> [additional "
            "parameters in form key=value]\n"
            "Possible additional parameters:\n"
            "\tcolor=USE_COLOR[default=1] Force color (1), gray image (0) or "
            "keep source unchanged (-1).\n"
            "\tresize=newsize resize the shorter edge of image to the newsize, "
            "original images will be packed by default\n"
            "\tlabel_width=WIDTH[default=1] specify the label_width in the "
            "list, by default set to 1\n"
            "\tpack_label=PACK_LABEL[default=0] whether to also pack multi "
            "dimenional label in the record file\n"
            "\tnsplit=NSPLIT[default=1] used for part generation, logically "
            "split the image.list to NSPLIT parts by position\n"
            "\tpart=PART[default=0] used for part generation, pack the images "
            "from the specific part in image.list\n"
            "\tcenter_crop=CENTER_CROP[default=0] specify whether to crop the "
            "center image to make it square.\n"
            "\tquality=QUALITY[default=95] JPEG quality for encoding (1-100, "
            "default: 95) or PNG compression for encoding (1-9, default: 3).\n"
            "\tencoding=ENCODING[default='.jpg'] Encoding type. Can be '.jpg' "
            "or '.png'\n"
            "\tinter_method=INTER_METHOD[default=1] NN(0) BILINEAR(1) CUBIC(2) "
            "AREA(3) LANCZOS4(4) AUTO(9) RAND(10).\n"
            "\tunchanged=UNCHANGED[default=0] Keep the original image "
            "encoding, size and color. If set to 1, it will ignore the others "
            "parameters.\n");
        return 0;
    }
    int label_width = 1;
...
#ifdef _MSC_VER
        effct_len =
            sscanf_s(argv[i], "%[^=]=%s", key, sizeof(key), val, sizeof(val));
#else
    ...
    if (inter_method != 1) {
        switch (inter_method) {
            ...
            case 4:
                ...
                break;
            case 9:
                LOG(INFO) << "Use inter_method mod auto(cubic for enlarge, "
                             "area for shrink)";
                break;
            case 10:
                LOG(INFO) << "Use inter_method mod "
                             "rand(nn/bilinear/cubic/area/lanczos4)";
                break;
            default:
    ...
    dmlc::InputSplit *flist =
        dmlc::InputSplit::Create(argv[1], partid, nsplit, "text");
    ...
    while (flist->NextRecord(&line)) {
        ...
        for (int k = 1; k < label_width; ++k) {
            CHECK(is >> label_buf[k]) << "Invalid ImageList, did you provide "
                                         "the correct label_width?";
        }
        ...
        if (pack_label) {
            ...
            memcpy(BeginPtr(blob) + bsize, BeginPtr(label_buf),
        ...
        if (unchanged != 1) {
            ...
            if (new_size > 0) {
                if (center_crop) {
                    if (img.rows > img.cols) {
                        ...
                        img = img(cv::Range(margin, margin + img.cols),
                                  cv::Range(0, img.cols));
                    } else {
                        int margin = (img.cols - img.rows) / 2;
                        img = img(cv::Range(0, img.rows),
                                  cv::Range(margin, margin + img.rows));
                    }
                }
                ...
                if (img.rows > img.cols) {
                    if (img.cols != new_size) {
                        interpolation_method = GetInterMethod(
                            inter_method, img.cols, img.rows, new_size,
                    ...
                    if (img.rows != new_size) {
                        interpolation_method = GetInterMethod(
                            inter_method, img.cols, img.rows,
                            new_size * img.cols / img.rows, new_size, prnd);
                        cv::resize(
                            img, res,
            ...
            memcpy(BeginPtr(blob) + bsize, BeginPtr(encode_buf),
                   encode_buf.size());
        } else {
            size_t bsize = blob.size();
            blob.resize(bsize + decode_buf.size());
            memcpy(BeginPtr(blob) + bsize, BeginPtr(decode_buf),
                   decode_buf.size());
        }
        ...
        ++imcnt;
        if (imcnt % 1000 == 0) {
            LOG(INFO) << imcnt << " images processed, " << GetTime() - tstart
                      << " sec elapsed";
        }
    }
    LOG(INFO) << "Total: " << imcnt << " images processed, "
              << GetTime() - tstart << " sec elapsed";
    delete fo;
    delete flist;
    return 0;
}
